...
.controller('LlamadaRecibidaController', ['$scope', '$rootScope', '$http', 'afiliacionHttp', 'ngDialog',
    function ($scope, $rootScope, $http, afiliacionHttp, ngDialog) {
        ...
        $scope.RegistrarLLamada = function (estado,data) {
            if (estado == 'R') {
                $scope.info.documento = data.nit;
                $scope.tipodocumento = data.tipo_documento;
                $scope.ConsultarAportante();
            }
            if (estado == 'N') {
                $scope.info.documento = null;
                $scope.tipodocumento = '0';
            }
        ...
        $scope.ConsultarSedeAportante = function () {
            $http({
                ...
                $scope.DetalleAfiliadoLLamada($scope.info.documento);
            })
        }
        ...
        $scope.CantidadPorPersona = function () {
            $http({
                ...
                url: "php/cartera/funcartera.php",
                data: {
                    function: 'CantidadxPersona', documento: sessionStorage.getItem('cedula')}   //  '1140857421'
            }).then(function (respose) {
        ...
        $scope.ListarXLlamar = function () {
            ...
            $http({
                method: 'POST',
                url: "php/cartera/funcartera.php",
                data: { function: 'ListarXLlamar' }
            }).then(function (response) {
                if (response.data.length > 0) {
                    if ($scope.length == response.data.length) {
                        $scope.llamadas.destroy();
                    }
                    $scope.llamadaxpendiente = response.data;
                    $scope.length = response.data.length;
                    setTimeout(function () {
                        $scope.llamadas = $('#llamadas').DataTable({
                            dom: 'lBsfrtip',
                            select: true,
                            buttons: ['csv', 'excel'],
        ...
        $scope.ListadoXLlamar = function () {
            ...
            $http({
                method: 'POST',
                url: "php/cartera/funcartera.php",
                data: { function: 'ListadoXLlamar', documento:sessionStorage.getItem('cedula') } // documento: '1140857421' } //documento:sessionStorage.getItem('cedula') }
            }).then(function (response) {
                if (response.data.length > 0) {
                    if ($scope.status == 'L') {
        ...
        $scope.ListarGestionCartera = function () {
            $http({
                method: 'POST',
                url: "php/cartera/funcartera.php",
                data: { function: 'ListarGestionCartera', documento: sessionStorage.getItem('cedula') }  //'1140857421'
            }).then(function (response) {
        ...
        $scope.ObtenerGestionLlamada = function (tipo_gestion, titulo) {
            $http({
                method: 'POST',
                url: "php/cartera/funcartera.php",
                data: { function: 'ObtenerGestionLlamada',tipo_gestion:tipo_gestion, documento:sessionStorage.getItem('cedula')}  //'1140857421' 
            }).then(function (response) {
                if (response.data.length != 0) {
