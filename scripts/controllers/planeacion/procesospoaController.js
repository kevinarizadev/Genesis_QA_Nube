  ...
  .controller('procesospoaController', ['$scope', '$http', '$filter',
    function ($scope, $http, $filter) {
      ...
      $scope.Inicio = function () {
        ...
        // document.querySelector('#theme-settings').style.display = "none"
      ...
      $scope.hojaIndicadoresLimpiar = function () {
        $scope.hojaIndicadores = {
          ...
          listadoUnidadMedida: [
            ...
            { codigo: '10', nombre: 'Semanas' },
            { codigo: '11', nombre: 'Minutos' },
          ],
          listadoFrecuencia: [
      ...
      $scope.hojaIndicadoresListar = function (x) {
        ...
        $http({
          ...
          if (data.length) {
            ...
            if (x === undefined) {
              $scope.hojaIndicadores.filtros.anio = $scope.SysDay.getFullYear().toString()
              // $scope.hojaIndicadores.filtros.anio = '2026'
              $scope.hojaIndicadoresAplicarFiltro()
            }
            // $scope.configPages('hojaIndicadores');
            setTimeout(() => { $scope.$apply(); }, 500);
          }
        });
        ///////////////////////////////////
      ...
      $scope.modalEditarIndicador = function (x) {
        ...
        $scope.hojaIndicadores.formulario.marca_pedt = x.REGC_MARCA_PEDT;
        $scope.hojaIndicadores.formulario.marca_altocosto = x.REGC_MARCA_ALTOCOSTO;
        $scope.hojaIndicadores.formulario.marca_2335 = x.REGC_MARCA_2335;
        $scope.hojaIndicadores.formulario.marca_incentivo = x.REGC_MARCA_INCENTIVO;
      ...
      $scope.validarFormIndicadores = function () {
        return new Promise((resolve) => {
          ...
          if (!$scope.hojaIndicadores.formulario.marca_pedt) resolve(false);
          if (!$scope.hojaIndicadores.formulario.marca_altocosto) resolve(false);
          if (!$scope.hojaIndicadores.formulario.marca_2335) resolve(false);
          if (!$scope.hojaIndicadores.formulario.marca_incentivo) resolve(false);
      ...
      $scope.guardarFormIndicadores = function () {
        $scope.validarFormIndicadores().then(res => {
          ...
          const dato = [{
            ...
            pmarca_pedt: $scope.hojaIndicadores.formulario.marca_pedt,
            pmarca_altocosto: $scope.hojaIndicadores.formulario.marca_altocosto,
            pmarca_2335: $scope.hojaIndicadores.formulario.marca_2335,
            pmarca_incentivo: $scope.hojaIndicadores.formulario.marca_incentivo,
              ...
              $http({
                ...
                if (data.Codigo == 0) {
                  ...
                  if ($scope.hojaIndicadores.formulario.idIndicadorSeleccionado == '') {
                    ...
                    $scope.actualizaUnicoIndicadores($scope.hojaIndicadores.formulario.idIndicadorSeleccionado)
                  }
                  swal({
                    title: data.Nombre,
      ...
      $scope.actualizaUnicoIndicadores = function (codigoId) {
        let datos = $scope.hojaIndicadores.listadoTabla.find(e => e.CODIGO == codigoId)
      ...
      $scope.hojaIndicadoresModalFiltro = function () {

      }

      // $scope.hojaIndicadoresBorrarFiltro = function () {
      //   $scope.openModal('modalFiltroIndicadores');
      // }

      $scope.hojaIndicadoresLimpiarFiltro = function () {
        $scope.hojaIndicadores.filtros.proceso = '';
        $scope.hojaIndicadores.filtros.regional = '';
        ...
        // $scope.hojaIndicadores.filtros.responsables = '';
        $scope.hojaIndicadores.filtros.fenix = '';
        $scope.hojaIndicadores.filtros.pedt = '';
        $scope.hojaIndicadores.filtros.altocosto = '';
        $scope.hojaIndicadores.filtros.marca2335 = '';
        $scope.hojaIndicadores.filtros.incentivo = '';

        $scope.hojaIndicadores.filtros.activo = false;
        $scope.initPaginacion('hojaIndicadores', $scope.hojaIndicadores.listadoTabla);
      }

      $scope.hojaIndicadoresQuitarFiltro = function () {
        $scope.hojaIndicadores.filtros.activo = false;
        // $scope.hojaIndicadores.filtros.proceso = '';
        // $scope.hojaIndicadores.filtros.dependencia = '';
        // $scope.hojaIndicadores.filtros.periodicidadAnalisis = '';
      ...
      $scope.hojaIndicadoresAplicarFiltro = function () {
        // let listadoTabla;
        $scope.hojaIndicadores.filtros.activo = true;
        ...
        if ($scope.hojaIndicadores.filtros.regional != '') {
          if ($scope.hojaIndicadores.filtros.regional == '999') {
            listadoTabla = listadoTabla.filter(x => x.REGN_NIVEL.split('-')[0] == 1);
          } else {
            listadoTabla = listadoTabla.filter(x => x.REGN_REGIONAL === $scope.hojaIndicadores.filtros.regional)
          }
        }
        ...
        if ($scope.hojaIndicadores.filtros.anio != '') {
          listadoTabla = listadoTabla.filter(x => x.REGN_ANNO === $scope.hojaIndicadores.filtros.anio)
        }
        if ($scope.hojaIndicadores.filtros.fenix != '') {
          listadoTabla = listadoTabla.filter(x => x.REGC_FENIX === $scope.hojaIndicadores.filtros.fenix)
        }
        if ($scope.hojaIndicadores.filtros.pedt != '') {
          listadoTabla = listadoTabla.filter(x => x.REGC_MARCA_PEDT === $scope.hojaIndicadores.filtros.pedt)
        }
        if ($scope.hojaIndicadores.filtros.altocosto != '') {
          listadoTabla = listadoTabla.filter(x => x.REGC_MARCA_ALTOCOSTO === $scope.hojaIndicadores.filtros.altocosto)
        }
        if ($scope.hojaIndicadores.filtros.marca2335 != '') {
          listadoTabla = listadoTabla.filter(x => x.REGC_MARCA_2335 === $scope.hojaIndicadores.filtros.marca2335)
        }
        if ($scope.hojaIndicadores.filtros.incentivo != '') {
          listadoTabla = listadoTabla.filter(x => x.REGC_MARCA_INCENTIVO === $scope.hojaIndicadores.filtros.incentivo)
        }
        $scope.initPaginacion('hojaIndicadores', listadoTabla);
        setTimeout(() => { $scope.$apply(); }, 500);
        if (listadoTabla.length) {
          Materialize.toast(`✅ Registros filtrados ${listadoTabla.length}`, 2000); return
        }
        Materialize.toast('❎ Sin Registros', 2000);
      }
      ...
      $scope.cargarMasivoGestionIndicador = function () {
        swal({
          title: "Cargar Gestión",
          html: `
             ...
             <div class="right">
               <input type="file" id="file" name="file" accept=".xlsx" onchange="myFunction()">
             </div>
          ...
          if (reader.readAsBinaryString) {
            reader.onload = function (e) {
              ...
              datos.forEach(element => {
                if ($scope.modalDatosCambio.CODIGO != element.CODIGO) {
                  swal("Error", 'Corregir la columna (CODIGO), no corresponde al indicador seleccionado', "warning").catch(swal.noop);
                  varControl = true;
                  return
                }
                datosJSON.push({
                  gestion_id: element.CODIGO,
                  anio: element.AÑO,
                  periodo: element.PERIODO,
                  numerador: element.NUMERADOR,
                  denominador: element.DENOMINADOR,
                  constante: element.CONSTANTE,
                  observacion: element.ANALISIS
                })
                console.log(element)
              });
              console.table(datosJSON)
              if (!varControl) {
                $scope.guardarMasivoGestionIndicador(datosJSON)
              }
            };
          ...
          // reader.onload = () => {
          //   const linea = reader.result.split('\r\n');
          //   console.log(linea)
          //   linea.forEach(element => {
          //     const campo = element.split('|');
          //     console.log(campo)
          //     if ($scope.modalDatosCambio.CODIGO != campo[0]) {
          //       swal("Error", 'Corregir la columna (CODIGO)', "warning").catch(swal.noop);
          //       // break
          //       return
          //     }
          //     datosJSON.push({
      ...
      $scope.guardarMasivoGestionIndicador = function (datosJSON) {
        swal({
          title: 'Confirmar',
          text: '¿Desea cargar la gestión?',
          type: 'question',
          showCancelButton: true,
          confirmButtonColor: '#3085d6',
          cancelButtonColor: '#d33',
          ...
          if (result) {
            ...
            $http({
              ...
              data: {
                ...
                responsable: $scope.Rol_Cedula,
                datos: JSON.stringify(datosJSON)
              }
            }).then(function ({ data }) {
              if (data.toString().substr(0, 3) == '<br' || data == 0) {
                swal("Error", 'Sin datos', "warning").catch(swal.noop); return
              ...
              if (data.Codigo == 0) {
                swal("Mensaje", data.Nombre, "success").catch(swal.noop);
                $scope.modalDatosCorresp($scope.modalDatosCambio)
                $scope.actualizaUnicoIndicadores($scope.itemSeleccionado)
              }
              if (data.Codigo == 1) {
                swal("Mensaje", data.Nombre, "warning").catch(swal.noop);
              }
      ...
      $scope.filterColumn = function (hoja, columna, val) {
        $scope[hoja].listadoTablaTemp = $scope[hoja].listadoTabla.filter(el => {
          if (!val) return true; // Si 'val' está vacío, no filtramos nada
          const cellValue = el[columna];
          // Condición para 'CODIGO' o cualquier otra columna
          return columna === 'CODIGO' ? cellValue == val : cellValue.toLowerCase().includes(val.toLowerCase());
        });
        if ($scope[hoja].listadoTablaTemp.length > 0) {
          $scope.setPage(hoja, 1);
        }
      ...
      $scope.calcular = function (hoja, i, fin) {
        ...
        console.log($scope[hoja].varsTabla.pages)
        $scope[hoja].varsTabla.pages = [];
        for (i; i <= fin; i++) {
          $scope[hoja].varsTabla.pages.push({
            no: i
