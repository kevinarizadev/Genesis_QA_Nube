'use strict';
angular.module('GenesisApp').controller('modaldatoscontactoautController', ['$scope', 'ngDialog', '$http', 'afiliacionHttp', function ($scope, ngDialog, $http, afiliacionHttp) {
    $scope.test = "modaldatoscontactoautController";
    $http({
        method: 'POST',
        ...
        $scope.municipiologueo = sessionStorage.getItem('municipio');
        $scope.permisoactualizar = sessionStorage.getItem('actualizardatos');
        ...
        var myInputcomplemento = document.getElementById("barrio");
        myInputcomplemento.onpaste = function (e) {
          e.preventDefault();
    ...
    $scope.actualizar_gestantes = function () {
        $http({
            ...
            data: { function: 'obtenerafiliados', responsable: $scope.responsable, tipodocumento: $scope.tipoDoc, documento: $scope.contacto.AFIC_DOCUMENTO }
        }).then(function () {
        });
    }
