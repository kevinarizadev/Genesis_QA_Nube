        ...
        $scope.diasgestacion = "";
            ...
            $scope.guardar_devolucion = function() {
                ...
                if (subir_adjunto_devolcuiones == true) {
                    var numeroradicado = $scope.liquidacion.radicado.split("-");
                    var ubicacionradicado = numeroradicado[1];
                    ...
                    $http({
                        ...
                        data: {
                            ...
                            ubicacion: ubicacionradicado,
                            numero: $scope.liquidacion.numero
                        }
                    }).then(function(response) {
                        $scope.validarradicado(ubicacionradicado, $scope.liquidacion.numero);
                        if (response.data == 1) {
                ...
                $http({
                            ...
                            if(response.data.codigo){
                                ...
                                if($scope.tieneprorroga == 'S'){
                                    if($scope.datoliquidacionibcprorroga == ''){
...
//  de nacimiento prematuro;   =< 36 semanas de gestacion
                ...
                if (concepto == 0) {
                    ...
                    $http({
                        ...
                        if (response.data.datos.length != 0) {
                            ...
                            $scope.liquidacion.dias_prematuros = response.data.datos[0].prematuro + 1;
            ...
            $scope.buscar_historial = function() {
                //if($scope.donde_guardo.valmed=='S' && !$scope.EDITAR_MEDICO_FORMULARIO){
                    $http({
                        ...
                        if (response.data.length != 0) {
                            var datoshistorial = response.data;
                               $scope.listar_historial = datoshistorial.filter(function(item) {
                                return item.RADICADO.trim() !== $scope.liquidacion.radicado;
                                });
                            $scope.dialogNewAfil = ngDialog.open({
            ...
            $scope.escoger_prorroga = function(ind) {
                ...
                // yordis
                var numeroradicado = $scope.liquidacion.radicado.split("-");
                var tipo = $scope.proronga.RADICADO.split("-");
                if((numeroradicado[0] == 'EG' || numeroradicado[0] == 'TT') && (tipo[0] == 'EG' || tipo[0] == 'TT' || tipo[0] == 'LA')){
            ...
            }else if(numeroradicado[0] == 'AT' && tipo[0] == 'AT'){
                $scope.liquidacion.prorroga_descripcion = $scope.proronga.RADICADO + " - " + $scope.proronga.DIAGNOSTICO_NOM;
                $scope.liquidacion.proroga_numero = $scope.proronga.NUMERO;
                $scope.liquidacion.proroga_ubicacion = $scope.proronga.UBICACION;
                var dias_acumulados=($scope.proronga.DIAS_PRORROGA=="")||($scope.proronga.DIAS_PRORROGA=='0')?$scope.proronga.DURACION:$scope.proronga.DIAS_PRORROGA;
                ...
                $("#DM" + $scope.proronga.NUMERO).addClass('eleacti');
                $('#DM' + $scope.proronga.NUMERO).siblings().removeClass('eleacti');
            } else if((numeroradicado[0] == 'LA' || numeroradicado[0] == 'LP' || numeroradicado[0] == 'LM') && (tipo[0] == 'LA' || tipo[0] == 'LP' || tipo[0] == 'LM')){
                $scope.liquidacion.prorroga_descripcion = $scope.proronga.RADICADO + " - " + $scope.proronga.DIAGNOSTICO_NOM;
                $scope.liquidacion.proroga_numero = $scope.proronga.NUMERO;
                $scope.liquidacion.proroga_ubicacion = $scope.proronga.UBICACION;
                var dias_acumulados=($scope.proronga.DIAS_PRORROGA=="")||($scope.proronga.DIAS_PRORROGA=='0')?$scope.proronga.DURACION:$scope.proronga.DIAS_PRORROGA;
                ...
                $('#DM' + $scope.proronga.NUMERO).siblings().removeClass('eleacti');
                    ...
                    swal('Advertencia', 'No Puede Seleccionar esta Como Prorroga', 'warning');
            }
            }
            // $scope.escoger_prorroga = function(ind) {
            //     $scope.proronga = $scope.listar_historial[ind];
               ...
               if ($scope.liquidacion.documento ==  $scope.liquidacion.doc_medico ) { Encontrar_Vacios = true; }
                ...
                switch (tipo) {
                    ...
                    case 'M':
                        ...
                        // } else {
                            // if($scope.liquidacion.dias_prorroga < 30){
                    ...
                    // }else{
                    //     swal('Información', "Validar Dias de Prorroga", 'info');
                    // }
                        break;
                    case 'L':
