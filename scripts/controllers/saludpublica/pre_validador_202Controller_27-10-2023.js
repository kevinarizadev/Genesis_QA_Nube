...
angular.module("GenesisApp").controller("pre_validador_202_controller", [
  ...
  function ($scope, $http, $q) {
    ...
    $scope.actualizaPlaceHolder = function (anno, periodo) {
      $scope.XnombreArch = "";
      ...
      if ($scope.inputFile.files[0].name.split(".")[0] == $scope.XnombreArch) {
        if (
          ...
          if ($scope.inputFile.files.length != 0) {
            ...
            if (
              $scope.inputFile.files[0].size > 0 &&
              $scope.inputFile.files[0].size <= 10485760
            ) {
              ...
              swal(
                "TAMAÑO EXCEDIDO",
                "Archivo no debe superar los 10 megabytes",
                "error"
              );
    ...
    $scope.ValidarArchivo = function (tipo, anno, periodo) {
