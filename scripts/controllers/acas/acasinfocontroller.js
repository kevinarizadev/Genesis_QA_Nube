...
angular.module('GenesisApp')
    .controller('acasinfocontroller', ['$scope', '$http', 'acasHttp', '$rootScope', 'communication', 'ngDialog', 'notification',
        function ($scope, $http, acasHttp, $rootScope, communication, ngDialog, notification) {
            ...
            $scope.asuntoacas = $scope.asuntoacas;
            ...
            $scope.insertarComentario = function (Cerrar_acas) {
                ...
                return new Promise(resolve => {
                    ...
                    if ($scope.validateTransfer == true) {
                        acasHttp.tranferirTercero('RE',$scope.cod_acas,$scope.ubicacionacas,$scope.terceros).then(function (response) {
                           if(response.data.codigo == 1){
                               notification.getNotification('success', response.data.mensaje + $scope.nombreMessage,'Notificación');
                           }else{
                               notification.getNotification('error',"Error al transferir el servicio "+ $scope.numeroacas +"  ",'Notificación');
                           }
                       })
                       ngDialog.closeAll();
                    }
                        $scope.validateTransfer = false;

                });
            }

            $scope.tranferir = async function (nombre){
                if ($scope.codigo_concepto == 'RS') {
                    $scope.nombreMessage = nombre;
                    if ($scope.asuntoacas == 'AUTORIZACION' || $scope.asuntoacas == 'ASIGNACION DE CITAS' || $scope.asuntoacas == 'MEDICAMENTOS' || 
                        $scope.asuntoacas == 'PROGRAMACION QUIRURGICA') {
                            $scope.terceros = 22511077;
                    }else{
                            $scope.terceros = 22511916;
                    }
                }
