...
angular.module('GenesisApp')
    .controller('acascontroller', ['$scope', '$http', 'notification', 'acasHttp', 'ngDialog', 'communication', '$rootScope',
        function ($scope, $http, notification, acasHttp, ngDialog, communication, $rootScope) {
            ...
            $scope.habilitarCampodeCriticidad = function(){
                if ($scope.filtroarea == '2' && $scope.filtroconcepto == 'EE') {
                $scope.OcultarCriticidad = true;
                acasHttp.obtenerCriticidad().then(function(response){
                    $scope.datosCriticidad = response.data
                })
                if ($scope.filtroconcepto != "EE")  $scope.obtenerprioridad();
                } else {
                $scope.OcultarCriticidad = false;
                }
            }
            ...
            $scope.obtenermotivo = function () {
                if ($scope.filtroconcepto != '0' && $scope.filtroconcepto != '? undefined:undefined ?') {
                    ...
                    $scope.HabilitarCampoCartera();
                    $scope.habilitarCampodeCriticidad();
                    $scope.motivos = [];
            ...
            $scope.AsuntoPrioridad = function () {
                if ($scope.filtroconcepto != "EE") {
                    $scope.obtenerprioridad();
                }
            }
            ...
            $scope.validainsertaaca = function () {
                ...
                if ($scope.filtro.tercero == true) {
                    ...
                    acasHttp.insertarAcas($scope.docval,
                        ...
                        $scope.filtroconcepto,
                        $scope.filtromotivo, '',
                        $scope.descripcion.toString().toUpperCase(), communication.data.tercero_cedula, $scope.filtroasunto, $scope.criticidad,
                        // $scope.prioridad.substr(0, 1), 
                        $scope.barrio,$scope.tipoaportante, $scope.numaportante).then(function (response) {
                            swal.close();
                            if (response.data.codigo == 1) {
                    ...
                    // if ($scope.tipoaportante == '0') {$scope.tipoaportante = ''}
                    acasHttp.insertarAcas($scope.docval, $scope.ubicacion, $scope.filtroconcepto, $scope.filtromotivo, '', $scope.descripcion.toString().toUpperCase(), $scope.cedula, $scope.filtroasunto, $scope.criticidad, $scope.barrio, $scope.tipoaportante, $scope.numaportante).then(function (response) {
                        swal.close();
                        console.log(response);
                        if (response.data.codigo == 1) {
                            $scope.numeroacascel = response.data.mensaje;
            ...
            $scope.insertaracas = function (id) {
                ...
                if (validador_alerta == false) {
                    ...
                    //debugger
                    if ($scope.filtroconcepto != '0' && $scope.filtroconcepto != '0' && $scope.filtroconcepto != '0' && $scope.descripcion != '' && $scope.filtroasunto != '' && $scope.filtroasunto != '0' || ($scope.OcultarCriticidad != false && $scope.criticidad != "")) {
            ...
            $scope.obtenerprioridad = function () {
                // document.getElementById("prioridadID").style.display = 'none';
                ...
                acasHttp.obtenerPrioridad($scope.docval, $scope.filtroconcepto, $scope.filtromotivo, $scope.filtroasunto,$scope.criticidad == "" ? '' : $scope.criticidad).then(function (response) {
                    ...
                    if ($scope.filtroconcepto != "0" && $scope.filtromotivo != "0" && $scope.filtroasunto != "0" || $scope.criticidad != "") {
                        //document.getElementById("prioridadID").style.display = 'inline-block';
                    }
