		...
		function ($scope, $http, consultaHTTP, $filter, ngDialog, cfpLoadingBar) {
			$(document).ready(function () {
				...
				$.getJSON("php/obtenersession.php").done(function (respuesta) {
					...
					if ($scope.sesdata.cedula == 22515239 || $scope.sesdata.cedula == 1096211358 || $scope.sesdata.cedula == 1045739217) {
			...
			$scope.restart = function () {
				...
				$scope.dsbCumplimientomensual = false;
				$scope.btnfechafallo = true;
			...
			$scope.habilitarInfo = function () {
				$.getJSON("php/obtenersession.php").done(function (respuesta) {
					$scope.sesdata = respuesta;
					if ($scope.sesdata.cedula == 1096211358 || $scope.sesdata.cedula == 1045739217) {
						$scope.dsbRegistro = false;
					} else {
						$scope.dsbRegistro = true;
					}
			...
			$scope.deshabilitarInfoTutela = function () {
				if ($scope.sesdata.cedula != 1096211358) {
					$scope.dsbRegistro = false;
					document.getElementById('cons_juzgado').disabled = false;
					document.getElementById('aniocons_juzgado').disabled = false;
			...
			$('#resultTutelas tbody').on('click', 'tr', function () {
				// $scope.lstMotivos();
				// console.log($scope.registro.motivo);
				// console.log($scope.registro.motivolbl);
				$scope.despl1 = true;
				...
				$http({
					...
					// if (parseInt($scope.registro.f_recepcion.split('/')[2]) < 2019) {//-- VALIDAR TAMBIEN QUE SEAN DE OTRAS EPS
					// 	$scope.BuscarOmitirEtapasTut();//Buscar Etapas para habilitar
					// }
					// VALIDA MOSTRAR O NO CAMPOS DE FALLO TUTELA CNVU 12/12/2019
					if ($scope.registro.f_fallo != "" || $scope.registro.plazo_fallo != "" || $scope.registro.f_fallo_plazo != "") {
						$scope.hdeFechaFallo = true;
					} else {
						$scope.hdeFechaFallo = false;
					}
					//console.log($scope.hdeIdafiliado);
					//console.log($scope.registro.usuariotraeps);
					...
					if (response.data.rutarecibidorespuestatutela == null) {
						...
						setTimeout(function () { $(document).scrollTop($('#panelListado1').height() + 400); }, 500);
						$scope.despl2 = true;
						// $scope.despl1 = false;
						return;
					} else {
					...
					if ($scope.registro.status == "2") {
						...
						// $scope.despl1 = false;
						$scope.despl3 = true;
						setTimeout(function () { $(document).scrollTop($('#panelListado2').height() + 1200); }, 500);
						// setTimeout(function() {$(document).scrollTop($('#panelListado2').height()+100);}, 1000);
						return;
					}
			...
			$scope.registraTutela = function () {
				console.log($scope.registro.medioRecepcionAdmisionTut);
				...
				if (
					($scope.hdeMotivo == false && ($scope.registro.motivo == "" || $scope.registro.motivo === undefined)) ||
			...
			$scope.ActualizarTutela = function () {
				// $scope.registro.motivo = $scope.registro.motivolbl;
				// $scope.lstMotivos();
						...
						if (result) {
							$http({
								...
								if (response.data.codigo == "1") {
									...
									console.log($scope.registro.medioRecepcionRespuesta);
									$http({
										method: 'POST',
										url: "php/upload_file/upload.php",
										data: {
			...
			// $scope.changeFalloSegInst = function () {
			// 	$scope.detalles.tratamientointegral = false;
			// 	$scope.detalles.seguimiento = false;
			// 	// if ($scope.detalles.fallotutela == true) {
			// 	// 	$scope.hdeDetallesAccionSeg = false;
											...
											if (response.data.codigo == "1") {
												...
												$http({
													...
													if (response.data.codigo == "1") {
														...
														if ($scope.detalles.fallotutela == true) {
															setTimeout(function () { $(document).scrollTop($('#panelListado3').height() + 400); }, 500);
															// $scope.despl1 = false;
															// $scope.despl2 = false;
															$scope.despl3 = true;
															$scope.hdeDetallesAccionSeg = false;
															...
															$scope.despl3 = false;
															$scope.despl4 = true;
															setTimeout(function () { $(document).scrollTop($('#panelListado4').height() + 400); }, 500);
															$scope.hdeDetallesAccionSeg = true;
															$scope.dsbDetallesSegInst = true;
															// fallotutela
						...
						if (result) {
							$http({
								...
								data: {
									...
									typefile: '14',
									ori: true,
									fallo_seg: $scope.detallesetpdos.fallotutelainstdos
								}
							}).then(function (response) {
								if (response.data.codigo == "1") {
									...
									$http({
										...
										if (response.data.codigo == "1") {
											...
											$scope.hdeAdjuntarFalloSegInst = true;
											$scope.dsbDetallesSegInst = true;
											if ($scope.detallesetpdos.fallotutelainstdos == true) {
												setTimeout(function () { $(document).scrollTop($('#panelListado7').height() + 700); }, 800);
												$scope.despl7 = false;
											} else {
												$scope.hdeImpugnacion = false;
												$scope.despl7 = false;
												$scope.despl4 = true;
												setTimeout(function () { $(document).scrollTop($('#panelListado4').height() + 700); }, 800);
											}
											swal('Completado', response.data.observacion_err, 'success')
										} else {
											swal('Error', response.data.observacion_err, 'warning')
			...
			$scope.registraImpugnacion = function () {
				if ($scope.registro.medioRecepcionImpugnacion == true && document.getElementById("medioRecepcionImpugnacionAdjunto").files.length == 0 && $scope.Validar_Fecha($("#dteMedioRecepcionImpugnacion").val()) == 'si') {
					swal('Información', 'Digite Fecha y Adjunte el soporte de Medio de Recepción', 'info');
				} else {
					// var xFecha_Medio_Fisico = new Date();
					// var dia = (xFecha_Medio_Fisico.getDate() <= 9) ? '0' + xFecha_Medio_Fisico.getDate() : xFecha_Medio_Fisico.getDate();
					// var mes = ((xFecha_Medio_Fisico.getMonth() + 1) <= 9) ? '0' + (xFecha_Medio_Fisico.getMonth() + 1) : (xFecha_Medio_Fisico.getMonth() + 1);
					// var Fecha_Medio_Fisico = dia + '/' + mes + '/' + xFecha_Medio_Fisico.getFullYear();
					swal({
						title: 'Confirmar',
							...
							$http({
								...
								if (response.data.codigo == "1") {
									if ($scope.detalles.impugnado == true) {
											...
											if (response.data.codigo == "1") {
												...
												$http({
													...
													if (response.data.codigo == "1") {
														...
														$scope.hdeIncidentes = false;
														$scope.hderequerimiento = false; $(paso1).addClass('active');
														$scope.hderequerimientores = true; $(paso2).removeClass('active');
														$scope.hdeproincdes = true; $(paso3).removeClass('active');
														$scope.hderesincdes = true; $(paso4).removeClass('active');
														$scope.hdefalincdes = true; $(paso5).removeClass('active');
														$scope.hdeconinc = true; $(paso6).removeClass('active');
														$scope.hdeautcieinc = true; $(paso7).removeClass('active');
														$scope.tabselect = 1;
										...
										$scope.hdeIncidentes = false;
										$scope.hderequerimiento = false;
										$scope.hderequerimientores = true;
										$scope.hdeproincdes = true;
										$scope.hderesincdes = true;
										$scope.hdefalincdes = true;
										$scope.hdeconinc = true;
										$scope.hdeautcieinc = true;
										$scope.hdeCumplimiento = false;
										$scope.despl5 = true;
										$scope.despl4 = false;
										$scope.archivoimpgtut = '';
								...
								if (response.data.codigo == "1") {
									...
									$http({
										...
										if (response.data.codigo == "1") {
											...
											$scope.archivofalloimpug2 = '';
											$("#FechaFallImp").val('');
											$scope.hdeReqPrevio = false;
											$scope.hderequerimiento = false;
											$scope.despl5 = true;
											$scope.despl6 = true;
											setTimeout(function () { $(document).scrollTop($('#panelListado6').height() + 500); }, 500);
										} else {
											swal('Error', response.data.observacion_err, 'warning')
										}
									});
