      ...
      $scope.mostrarAfiliado = function (type, id, carne, gpoblacion,tipo_afiliado) {
        ...
        for (var i = 0; i <= $scope.afildata.length - 1; i++) {
          if ($scope.afildata[i].DOCUMENTO == id) {
            ...
            $scope.municipioportabilidad = $scope.afildata[i].M_PORTABILIDAD;
            $scope.condiciondiscapacidad = $scope.afildata[i].CONDICION;
            $scope.nombre_subgrupo_p = $scope.afildata[i].NOMBRE_SUBGRUPO_P;
            $scope.estadobdua = $scope.afildata[i].ESTADOBDUA;
            $scope.claveactual = $scope.afildata[i].CLAVE;
            ...
            console.log("municipioportabilidad: " + $scope.municipioportabilidad);
            console.log("condiciondiscapacidad: " + $scope.condiciondiscapacidad);
