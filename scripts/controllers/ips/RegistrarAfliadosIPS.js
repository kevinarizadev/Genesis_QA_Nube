...
angular.module('GenesisApp')
  .controller('RegistrarAfliadosIPS', ['$scope', '$http', 'consultaHTTP', '$filter', 'ngDialog', 'cfpLoadingBar', 'afiliacionHttp',
    function ($scope, $http, consultaHTTP, $filter, ngDialog, cfpLoadingBar, afiliacionHttp) {
        ...
        return new Promise((resolve) => {
          $http({
            ...
            data: {
              ...
              tipo: $scope.datos.tipodocumento,
              numero: $scope.datos.numero,
              tipo_CN: $scope.datos.tipodocumento == 'RC' ? $scope.datos.tipodocumento_CN : '',
              numero_CN: $scope.datos.tipodocumento == 'RC' ? $scope.datos.numero_CN : ''
            }
          }).then(function ({ data }) {
            resolve(data)//
          });
      ...
      $scope.Siguiente = function (id) {
        switch (id) {
          case '0':
            ...
            break;
          case '1':
            // if ($scope.datos.tipodocumento == 'RC') {
            // if($scope.datos.tipodocumento == 'RC' && ($scope.datos.numero_CN == null || $scope.datos.numero_CN == ''))
            if ($scope.datos.numero == '0' || $scope.datos.numero == null || $scope.datos.numero == undefined ||
              ($scope.datos.tipodocumento == 'RC' && ($scope.datos.numero_CN == null || $scope.datos.numero_CN == ''))) {
              swal('Notificacion', 'Debe Completar Todos Los Campos', 'info');
            } else {
      ...
      $scope.ValidoInformacion = function () {
        if ($scope.info.reportes == 'N' && $scope.TipoReporte == false) {
          ...
          } else if ($scope.datos.correo == '' || $scope.datos.correo == undefined || $scope.datos.correo == null) {
            swal('Notificacion', "Campo Obligatorio Correo Electronico. Favor digitarlo", 'error')
          } else if ($scope.datos.telefono == '' || $scope.datos.telefono == undefined || $scope.datos.telefono == null) {
            swal('Notificacion', "Campo Obligatorio  Telefono. Favor digitarlo", 'error')
          } else if ($scope.datos.dirrecion == '' || $scope.datos.dirrecion == undefined || $scope.datos.dirrecion == null) {
            swal('Notificacion', "Campo Obligatorio Dirreción. Favor digitarlo", 'error')
          } else {
            $scope.validacion = '0';
          }
        } else if ($scope.info.reportes == 'I' && $scope.TipoReporte == false) {
            ...
            if ($scope.responsable == '32856342' || $scope.responsable == '8521697' || $scope.responsable == '32612938' || $scope.responsable == '32612938' || $scope.responsable == '1143450658') {
              ...
              $http({
                ...
                data: {
                  ...
                  tipodocumento_CN: $scope.datos.tipodocumento == 'RC' ? $scope.datos.tipodocumento_CN : '',
                  numero_CN: $scope.datos.tipodocumento == 'RC' ? $scope.datos.numero_CN : ''
                }
              }).then(function (response) {
      ...
      $scope.RegistrarIncosistencia = function () {
        var formattedDate = moment($scope.json.nacimiento).format('DD/MM/YYYY');
        $http({
          method: 'POST',
          url: "php/ips/func3047.php",
          data: {
            function: 'ReporteIncosistencia',
            tipo_documento: $scope.tipodocumento_cabeza,
            documento: $scope.numero_cabeza,
