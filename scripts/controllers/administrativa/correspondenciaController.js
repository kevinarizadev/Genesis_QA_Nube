...
angular.module('GenesisApp')
  .controller('correspondenciaController', ['$scope', '$timeout', 'CorrespondenciaHttp', 'ngDialog', '$filter', '$http', function ($scope, $timeout, CorrespondenciaHttp, ngDialog, $filter, $http) {
    //Objects
    ...
    $scope.pqrData = {
      selectedtipoSolicitud: 'E', selectedRv: 'N', selectedRdS: "", selectedCriteriObjetivo: null, selectedCriterioSubjetivo: null,
      ...
      selectedTramite: 'S', selectedCorrespondencia: 'E', seccional: null, numfolio: null,
      tipo_tercero: null, tipo_documento_rad: null, documento_rad: null, tipoEnvio: null,
    ...
    // CorrespondenciaHttp.getPrivideliosPQR().then(function (response) {
    //   $scope.privilegios = response;
    // })


    // CorrespondenciaHttp.getSolicitud().then(function (response) {
    ...
    $scope.usuariosPermisos = function(){
      let cedula = sessionStorage.getItem("cedula");
          $http({
              ...
              if (data != undefined) {
                  $scope.permisoRadicar = data.rol == "" ? false : true;
                  if ($scope.permisoRadicar) {
                    $scope.tabs.tab ='R';
                  }else{
                    $scope.tabs.tab ='C';
                        setTimeout(()=>{
                          $scope.getCorrespondencia('A');
                        },700)
                  }
              }
            })
    }
    $scope.usuariosPermisos();
    ...
    $scope.TiposdeSolicitud =  {
      ...
      $scope.obtenerAreasCorresp = function(dato, tipo){
        if (tipo == 'R') {
          if (dato != "") {
            CorrespondenciaHttp.get_areas_corresp(dato).then(({data})=>{
              $scope.pqrData.seccional = dato;
              $scope.visible = false;
              $scope.areaspqr = data;
            })
          }
    ...
    // CorrespondenciaHttp.getRiesgoSolicitud().then(function (response) {
    //   $scope.rv = response;

    // })
    // CorrespondenciaHttp.getRiesgo().then(function (response) {
    //   $scope.rv = response;
    // })

    // CorrespondenciaHttp.getOtrosEntesDeControl().then(function (response) {
    //   $scope.otrosEntesDeControl = response;
    // })

    CorrespondenciaHttp.getDocumento().then(function (response) {
      $scope.documento = response.Documento;
    })
    CorrespondenciaHttp.getSexo().then(function (response) {
      $scope.generos = response.Sexo;
    })
    // CorrespondenciaHttp.getCriteriObjetivo().then(function (response) {
    //   $scope.criteriObjetivo = response;
    // })
    // CorrespondenciaHttp.getCriterioSubjetivo().then(function (response) {
    //   $scope.criterioSubjetivo = response;
    // })
    // CorrespondenciaHttp.getCriterioComplementario().then(function (response) {
    //   $scope.criterioComplementario = response;
    // })
    // CorrespondenciaHttp.getSujetosproteccionespecial().then(function (response) {
    //   $scope.sujetosProteccionEspecial = response;
    // })
    // CorrespondenciaHttp.getServicios().then(function (response) {
    //   $scope.servicios = response;
    // })
    // CorrespondenciaHttp.getMedicamentos().then(function (response) {
    //   $scope.medicamentos = response;
    // })
    // CorrespondenciaHttp.getAcudiente().then(function (response) {
    //   $scope.acudiente = response;
    // })
    // CorrespondenciaHttp.getRespuestas().then(function (response) {
    //   $scope.respuestas = response;
    // })
    // CorrespondenciaHttp.getDias().then(function (response) {
    //   $scope.dias = response;
    // })

    CorrespondenciaHttp.getMediosRecepcionTipo('C').then(function (response) {
    ...
    $scope.getPQRS = function (state) {
      ...
      CorrespondenciaHttp.getPQRS(state, sessionStorage.getItem('cedula')).then(function (response) {
    ...
    CorrespondenciaHttp.get_regionales_corresp(sessionStorage.getItem('municipio')).then(function (response) {
      $scope.sedecorrespondencia = response.data;
    })

    $scope.getCorrespondencia = function () {
      CorrespondenciaHttp.get_rol_correspondencia(sessionStorage.getItem('cedula')).then(function (response) {
        ...
        setTimeout(() => {
          ...
          CorrespondenciaHttp.getCorrespondencia($scope.tempstatecorrepondencia, sessionStorage.getItem('cedula'), $scope.temptipocorrepondencia, $scope.filterOptionsPqr).then(function (response) {
    ...
    $scope.cancelPqr = function () {//Funcion para hacer reset al formulario
      swal({
        ...
        $timeout(function () {
          $scope.pqrData.selectedtipoSolicitud = 'E'; $scope.data.inactiveSolicitud = true; $scope.resetDataForm(); $scope.pqrData.macromotivo = null;
        }, 100);
      }).catch(swal.noop);
    }//Fin
    ...
    $scope.insert = function () {//Hace la request al servidor para insertar el PQR
      ...
      $scope.json = JSON.stringify({
        ...
        'tipo_envio': $scope.pqrData.tipoEnvio,
      ...
      swal({ title: 'Cargando datos' }).catch(swal.noop);
      swal.showLoading();
      if (sessionStorage.getItem('cedula')) {
        CorrespondenciaHttp.postPqr($scope.json, $scope.pqrData.pqrFile, null, 'I', $scope.pqrData.gradfile).then(function (res) {
          // console.log(res);
    ...
    $scope.resetDataForm = function () {//Funcion para reset data form
      ...
      $scope.pqrData.selectedTramite = 'S'; $scope.pqrData.selectedCorrespondencia = 'E'; $scope.pqrData.seccional = null; $scope.pqrData.numfolio = null; $scope.pqrData.tipo_tercero = null;
      ...
      $scope.pqrData.tipo_documento_rad = null; $scope.pqrData.documento_rad = null; pqrData.tipoEnvio = null;
      $scope.tabAfiliadoTemp = '1';
    ...
    $scope.getEnteControl = function (ente) {//Funcion para obtener el ente de control y la sede
      if (ente) {
        ...
        if ($scope.pqrData.sede == 'S') {
          CorrespondenciaHttp.getEntidades($scope.pqrData.enteCodigo).then(function (response) {
    ...
    $scope.changeMediosRecepcion = function (medio) {//Funcion para validar SelectOtrosEntes y opciones
      ...
      if (medio != '13' && $scope.pqrData.selectedtipoSolicitud == 'E') {
        $scope.nomarea = "";
          // CorrespondenciaHttp.get_areas().then(function (response) {
        ...
        CorrespondenciaHttp.getTipoRadicacion('C').then(function (response) {
    ...
    $scope.changeTipoRadicacion = function (afectado) {//Funcion  para validar select tipo radicación
      ...
      if (afectado == '') {
        ...
        if (afectado == 'P') {//Valida tipo de radicacion Proceso Admin
          ...
          CorrespondenciaHttp.get_tipo_correspondencia().then(function (response) {
    ...
    $scope.descargarSoporte = function(){
        if (!$scope.fechaInicio || !$scope.fechaFin) {
          swal('Campos Requeridos', "Por favor se necesita completar los campos vacios", 'warning');
        }else{
          swal({
            html: '<div class="loading"><div class="default-background"></div><div class="default-background"></div><div class="default-background"></div></div><p style="font-weight: bold;">Cargando...</p>',
            width: 200,
          ...
          $http({
            ...
            data: {
                ...
                inicio: $scope.formatDate2($scope.fechaInicio), 
                fin: $scope.formatDate2($scope.fechaFin),   
               }
        }).then( function({data}){
          if (data.length) {
            ...
            XLSX.writeFile(wb, "Correspondencia.xlsx");
    ...
    $scope.descargarConsulta = function(){
      if (!$scope.tipocorrespondenciaExcel || !$scope.estadoConsulta || !$scope.InicioConsulta || !$scope.FinConsulta) {
        swal('Campos Requeridos', "Por favor se necesita completar los campos vacios", 'warning');
      }else{
        ...
        $http({
          ...
          data: {
              ...
              tipo: $scope.tipocorrespondenciaExcel, 
              estado: $scope.estadoConsulta, 
              inicio: $scope.formatDate2($scope.InicioConsulta), 
              fin: $scope.formatDate2($scope.FinConsulta),   
          }
    ...
    $scope.searchAfiliado = function () {//Funcion para obtener la data para buscar al afiliado y valida que no hayan campos vacios
      ...
      if ($scope.pqrData.User.selectedDocumento && $scope.pqrData.User.documento) {
        ...
        CorrespondenciaHttp.postSearchAfiliado($scope.dataSearch).then(function (response) {
          ...
          if ($scope.afiliado && $scope.afiliado.NOMBRECOMPLETO) {//Si el nombre existe asigna los datos de la request
            ...
            if ($scope.pqrData.selectedtipoSolicitud == 'R' && $scope.pqrData.selectedtipoRadicacion == 'A') {
              CorrespondenciaHttp.validapqr_gestionriesgo($scope.dataSearch.selectedDocumento, $scope.dataSearch.documento).then(result => {
    ...
    $scope.searchIps = function () {//Funcion para obtener data de la ips
      if ($scope.pqrData.Ips.nit) {
        ...
        CorrespondenciaHttp.postSearchIps($scope.pqrData.Ips.nit).then(function (response) {
    ...
    $scope.searchEmpeador = function () {//Funcion para obtener data de la empleador
      if ($scope.pqrData.Empleador.tipodocumento && $scope.pqrData.Empleador.documento) {
        ...
        CorrespondenciaHttp.postSearchEmpleador({
    ...
    $scope.validateDias = function () {//Valida los dias habiles para respuesta del pqr
      CorrespondenciaHttp.getvalidapqr_responsable(sessionStorage.getItem('cedula')).then(function (response) {
        console.log(response);

        if (response.Nombre == 'S') {
          CorrespondenciaHttp.getDiasEntes().then(function (response) {
            $scope.dias = response;
          })
        } else {
          CorrespondenciaHttp.getDias().then(function (response) {
    ...
    $scope.$watch('filterPQRS', function (val) {
      ...
      if ($scope.tabs.tab == 'G') {
        if ($scope.filterPQRS == null || $scope.filterPQRS == '') {
          ...
          CorrespondenciaHttp.getPQRS($scope.check_option ? 'G' : 'A', sessionStorage.getItem('cedula')).then(function (response) {
            $scope.filterVencidas = 'N';
            // if()
            if ($scope.cedulausuario == '1140861113') {
              $scope.listadoCompleto = response.sort((a, b) => {
    ...
    $scope.$watch('filterC', function (val) {
      ...
      if ($scope.tabs.tab == 'C') {
        if ($scope.filterC == null || $scope.filterC == '') {
          CorrespondenciaHttp.getCorrespondencia($scope.check_optionc ? 'R' : 'A', sessionStorage.getItem('cedula')).then(function (response) {
            $scope.pqrs = response;
            $scope.inicio = 1;
            $scope.sum_records = 0;
            $scope.pages = Array($scope.numPages()).fill((x, i) => i).map((x, i) => i);
    ...
    $scope.descargafile = function (ruta, ftp) {
      CorrespondenciaHttp.dowloadfile(ruta, ftp).then(function (response) {
    ...
    $scope.uploadImage = function (e) {
      ...
      $timeout(function () {//Usado para validar el file en tiempo real
        ...
        if ($scope.soportepq.filetemp) {//Valida si existe el archivo en el input file
          if ($scope.soportepq.filetemp.size <= 10000000) {//valida que el size del file sea <= 5 mb
    ...
    $scope.getPQRSExcel = function (estado) {
      ...
      CorrespondenciaHttp.postObtenerPqrExcel().then(function (response) {
    ...
    $scope.buscarIps = function (params) {
      if(params.length > 3){
        ...
        CorrespondenciaHttp.get_Ips(params).then(function (response) {
          // console.log(response);
    ...
    $scope.loadPQRS = function () {
      if ($scope.jsonExport.length == 0) {
        ...
        $scope.jsonExport.forEach(element => {
          ...
          swal({ title: 'Cargando Informacion' }); swal.showLoading();
          CorrespondenciaHttp.postCarguePqr(JSON.stringify(element)).then(function (res) {
            console.log(res);
            $scope.sumUpload++;
            $timeout(function () {//Usado para validar el file en tiempo real
    ...
    $scope.openmodals = function (tipo, pqr) {
      ...
      switch (tipo) {
        ...
        case 'trazapqr':
          if (pqr) {
            CorrespondenciaHttp.getInfoAseguramientoPQR(pqr.CODIGO).then(function (response) {
              $scope.dpqr = response.data[0];
            console.log($scope.dpqr);

            })
          }
          CorrespondenciaHttp.p_mostrar_traza(pqr.CODIGO).then(function (response) {
        ...
        case 'modalcorrespondencia':
          ...
          CorrespondenciaHttp.get_detalle_correspondencia(pqr.CONSECUTIVO).then(function (response) {
            console.log(response);
            $scope.detallecorrespondencia = response;

            CorrespondenciaHttp.get_func_areas($scope.detallecorrespondencia.area_correspondencia, 0).then(function (response) {
        ...
        case 'modaltranferircorresp':
          ...
          CorrespondenciaHttp.get_pqr_seccionales().then(function (response) {
            console.log(response);
        ...
        case 'modalpqcorrespondencia':
          $scope.pqcorrespondencia = [];
          //  CorrespondenciaHttp.get_pq_correspondencia(sessionStorage.getItem('cedula')).then(function (response) {
          //    $scope.getPQ('A');
          ...
          break;
        case 'historicopqr':
          CorrespondenciaHttp.get_historico_pqr(sessionStorage.getItem('cedula')).then(function (res) {
            if (res.length > 0) {
        ...
        case 'modaldevolucion':
          $scope.dpqr = pqr;
          CorrespondenciaHttp.get_motivosacciones('D', 'P').then(function (response) {
            console.log(response.data);
    ...
    $scope.getdocumentosrad = function () {
      CorrespondenciaHttp.get_documentos_radicar().then(function (response) {
        console.log(response);
    ...
    $scope.getPQ = function (state) {
      ...
      CorrespondenciaHttp.get_pq_pqr(temptipocorrepondencia, state, sessionStorage.getItem('cedula')).then(function (response) {
    ...
    $scope.getArea = function (nomarea, tipo) {
      ...
      if (nomarea) {
        ...
        // CorrespondenciaHttp.get_func_areas(nomarea, 'N').then(function (response) {
          if (tipo == 'R') {
            let prueba = $scope.areaspqr.find((element)=> element.CODIGO_AREA ==  nomarea)
            console.log(prueba);
            $scope.visible = true;
            $scope.responsablesareas = {
              Nombre: prueba.RESPONSABLE1,
              Nombre1: prueba.RESPONSABLE2,
              Nombre2: prueba.RESPONSABLE3,
            };  
            }

          if (tipo == 'T') {
            let area = nomarea == 0 ? 1 : nomarea;
            CorrespondenciaHttp.get_areas_corresp($scope.vcorrespondencia.ubicacion).then(function (response) {

              $scope.areaspqr = response.data;     
            })
            CorrespondenciaHttp.get_correspon_ubicacion(area, $scope.vcorrespondencia.ubicacion).then(function (response) {
              $scope.vcorrespondencia.area_correspondencia = response[0].CODIGO_AREA;
              $scope.responsablesareas = {
                Nombre: response[0].RESPONSABLE1,
                Nombre1: response[0].RESPONSABLE2,
                Nombre2: response[0].RESPONSABLE3,
              };
            })
    ...
    $scope.getResSeccional = function (params, tab) {
      if (params == 'S') {
        ...
        CorrespondenciaHttp.get_func_areas(sessionStorage.getItem('municipio'), 'S').then(function (response) {
          if (tab == 'R') {
        ...
        $scope.pqrData.t_seccional = 'N';
        CorrespondenciaHttp.get_func_areas(0, 'N').then(function (response) {
          console.log(response);
          if (tab == 'R') {
            $scope.responsablesareas = response;
          }
    ...
    $scope.getResSeccionalCod = function (params) {
      console.log(params);
      if (params) {
        CorrespondenciaHttp.get_func_areas(0, params).then(function (response) {
          $scope.responsablesareast = response;
    ...
    $scope.transferirCorrespondencia = function () {
      CorrespondenciaHttp.postTransferirCorrespondencia($scope.area.Codigo, $scope.dpqr.CODIGO, sessionStorage.getItem('cedula')).then(function (response) {
    ...
    $scope.getStatistics = function () {
      CorrespondenciaHttp.get_pqr_estadisticas(sessionStorage.getItem('cedula'), $scope.check_optione == false ? 'A' : 'H').then(function (response) {
        if (response.data.length == 0) {
          setTimeout(() => {
            $scope.hidegraficapie = true;
          }, 100);
    ...
    $scope.getpqr_tiposol = function (params) {
      CorrespondenciaHttp.getPQRTIPOSOL(params, sessionStorage.getItem('cedula'), $scope.check_optione == false ? 'A' : 'H').then(function (response) {
        console.log(response);
    ...
    $scope.showpqrs = function (params) {
      CorrespondenciaHttp.get_PQR_TIPO_SOL($scope.tiposol, 'A', params).then(function (response) {
    ...
    $scope.adjuntarpq = function () {
      ...
      CorrespondenciaHttp.post_update_ruta_pq($scope.temppq, 'pqfile' + $scope.temppq, sessionStorage.getItem('cedula'), $scope.soportepq.file, $scope.soportepq.nombre, '3').then(function (response) {
        console.log(response);
        if (response.codigo == '0') {
          CorrespondenciaHttp.get_pq_pqr().then(function (res) {
          ...
          swal('PQ CORESPONDENCIA', response.mensaje, 'success').then(function () {
            ...
            CorrespondenciaHttp.post_actualiza_estado_pq($scope.temppq, 'pqfile' + $scope.temppq).then(function (response) {
              console.log(response);
    ...
    $scope.showsoportepq = function (params) {
      ...
      setTimeout(() => { $scope.$apply(); }, 500);
      console.log(params);
      CorrespondenciaHttp.dowloadfile(params.ruta, params.ftp).then(function (response) {
    ...
    $scope.seleccionarOperador = function (item) {
      ...
      if ($scope.tipocallCourrier == 'G') {
        ...
        CorrespondenciaHttp.post_actualizar_courier(JSON.stringify($scope.tempcorrespenviada), $scope.tempcorrespenviada.length).then(function (res) {
          console.log(res);
    ...
    $scope.showAddOperador = function () {
      ...
      CorrespondenciaHttp.getServicioCorrepondencia().then(function (response) {
        $scope.tempservio = response;
      })

      CorrespondenciaHttp.getAmbitoCorrepondencia().then(function (response) {
        $scope.tempambito = response;
      })
      CorrespondenciaHttp.get_ciudades_pqr().then(function (response) {
    ...
    $scope.inserta_operador = function () {
      if ($scope.pqrData.NewOperador.documento && $scope.pqrData.NewOperador.nombre &&
        ...
        CorrespondenciaHttp.post_insertar_operador_corresp(JSON.stringify($scope.pqrData.NewOperador)).then(function (response) {
          console.log(response);
          if (response.Codigo == '0') {
    ...
    $scope.saveguia = function () {
      ...
      CorrespondenciaHttp.post_actualizar_guia(JSON.stringify($scope.tempcorrespenviada),
    ...
    $scope.searchTercero = function (param, paramt) {//Funcion para obtener data de la ips
      ...
      if (param && paramt) {
        ...
        CorrespondenciaHttp.get_tercero(param, paramt).then(function (response) {
          console.log(response);
          if (response.Codigo == '0') {
            $scope.inactivetercero = true;
    ...
    $scope.get_sedes = function () {
      CorrespondenciaHttp.get_sedes().then(function (response) {
    ...
    $scope.get_areas_sede = function (argument) {
      CorrespondenciaHttp.get_areas_sede(argument).then(function (response) {
    ...
    $scope.get_personas_area = function (argument) {
      if (argument) {
        CorrespondenciaHttp.p_obtener_funcionario_eps(argument).then(function (response) {
          if (response.Codigo == 0) {
            $scope.tempresponsables = [];
            $scope.inactivefuncionario = true;
    ...
    $scope.creatpqcorresp = function (pq, param) {
      console.log(pq);
      CorrespondenciaHttp.post_ipq_correspondencia(pq.length, JSON.stringify(pq), sessionStorage.getItem('cedula')).then(function (res) {
        console.log(res);
        if (res.Codigo == '1') {
          if (param == 'R') {
            console.log($scope.tempcorresprecibida);
    ...
    $scope.inserta_tercero = function () {
      ...
      if ($scope.pqrData.NewTercero.documento && $scope.pqrData.NewTercero.nombre &&
        ...
        CorrespondenciaHttp.post_insertar_tercero_pqr(JSON.stringify($scope.pqrData.NewTercero)).then(function (response) {
          console.log(response);
    ...
    $scope.openCourier = function (tipocall) {
      ...
      CorrespondenciaHttp.get_operadores_correspondencia().then(function (res) {
        console.log(res);
    ...
    $scope.showUpdateCorresp = function (pqr) {
      ...
      if (pqr.TIPOCORRESPOND == 'R') {
        CorrespondenciaHttp.get_areas_corresp(sessionStorage.getItem("municipio")).then(function (response) {
          $scope.areaspqr = response.data;
        })
      }
      if (sessionStorage.getItem('municipio') == '1') {
      ...
      CorrespondenciaHttp.get_detalle_correspondencia(pqr.CONSECUTIVO).then(function (response) {
        ...
        if ($scope.vcorrespondencia.area_correspondencia) {
          ...
          if (sessionStorage.getItem('municipio') == '1') {
            $scope.areasecnacional = 'N';
            $scope.vcorrespondencia.area_correspondencia == 0 ? 1 : $scope.vcorrespondencia.area_correspondencia
            $scope.getArea($scope.vcorrespondencia.area_correspondencia, 'T');

          } else {
            $scope.areasecnacional = 'S';
            $scope.obtenerAreasCorresp($scope.areasecnacional, 'U');
          }
    ...
    $scope.updateCorrespondencia = function () {
      ...
      CorrespondenciaHttp.post_actualizar_correspondencia(JSON.stringify($scope.pqrData.EditCorresp), 1).then(function (response) {
        console.log(response);
        if (response.Codigo == 0) {
          $scope.showtablecorresp = false;
    ...
    $scope.showform = function () {
      $scope.showmsj = true;
      $timeout(function () {
        $scope.pqrData.selectedCorrespondencia = 'E'; $scope.data.inactiveSolicitud = true; $scope.resetDataForm();
      }, 100);
    }
    ...
    $scope.validarDocumentoRadicar = function (doctercero, tipodocrad, docrad) {
      ...
      if (doctercero && tipodocrad && docrad) {
        CorrespondenciaHttp.p_validar_documento_radicar(doctercero, tipodocrad, docrad).then(function (response) {
          console.log(response);
    ...
    $scope.saveDevolucion = function (motivo, observacion) {
      CorrespondenciaHttp.postDevolverCorrespondencia($scope.dpqr.CONSECUTIVO,
    ...
    $scope.getPQR = function (state) {
      ...
      CorrespondenciaHttp.getPQR($scope.filterPQRS, state, sessionStorage.getItem('cedula')).then(function (response) {
        if (response.Codigo == '1') {
    ...
    //   if (cargo == 11 || cargo == 44 || cargo == 60 || cargo == 94 || cargo == 58 || cargo == 24 || cargo == 0 || cargo == 91) {
    ...
    $scope.Optener_Motivos_Especificos = function (macro_motivo, motivo_general, motivo_especifico) {
      ...
      CorrespondenciaHttp.get_Tipos_Motivos_Especificos(macro_motivo, motivo_general, motivo_especifico).then(function (response) {
        console.log(response);
    ...
    $scope.Optener_Sub_Motivos_Especificos = function (tipo_motivo_especifico) {
      $scope.pqrData.submotivosEspecificos = "";
      CorrespondenciaHttp.get_Sub_Motivos_Especificos($scope.macro_motivo, $scope.motivo_general, $scope.motivo.codigo, tipo_motivo_especifico).then(function (response) {
    ...
    // -- --------------------------------yordis 16/06/2023------------------------------------------------------ --
    // CorrespondenciaHttp.p_obtener_pqr_tecno_altocosto().then(result => {
    //   $scope.motivosAltoCosto = result;
    // })

    // CorrespondenciaHttp.p_obtener_pqr_patologia().then(result => {
