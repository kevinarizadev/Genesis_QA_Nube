  ...
  .controller('riesgosycontrolesController', ['$scope', 'ngDialog', 'consultaHTTP', 'notification', 'cfpLoadingBar', '$http', function ($scope, ngDialog, consultaHTTP, notification, cfpLoadingBar, $http) {
    $scope.AbrirModalGestion = function (id) {   
      ...
      swal({
        ...
        if (dismiss == 'cancel') {
          $("#modalInhabilitar").modal("open");
          $scope.modal.estado = "IN";
          setTimeout(() => {
            $scope.$apply();
          }, 50);
        }
    ...
    $scope.mostrarSeleccion = function (tipoId, implementacionId) {
      if (tipoId && implementacionId) {
        let encontrado = $scope.listados.atributoscontroltipo.find(element => element.Codigo == tipoId);
        $scope.tipoct = encontrado.Probabilidad;
        encontrado = $scope.listados.implementacion.find(element => element.Codigo == implementacionId);
    ...
    $scope.EliminarControl = function (data) {
      console.log(data);
      $http({
        method: 'POST',
        url: "php/riesgos/riesgos.php",
        data: {
          ...
          P_VNUMERORIESGO: data.NumeroRiesgo,
          P_N_NUMEROCONTROL: data.Codigo,
        }
      }).then(({ data }) => {
        console.log(data);
   ...
   //$scope.iniciarFormaulario();
      switch (vista) {
        ...
        case "control":
          ...
          $scope.ListadoControlDetalle($scope.riesgoElegido.ID)
          break
        case "controlform":
    ...
    $scope.ListadoRiesgos = function () {
      ...
      $http({
        ...
        //console.log($scope.listadosRiesgos);
      });
    }
   ...
   // $scope.ListadoRiesgos();
    ...
    $scope.ListadoRiesgosId = function (id) {
      ...
      $http({
        ...
        $scope.riesgos = {
          ...
          objetivosEstrategicos: response.data.ObjetivosEstrategicoRelacionados,
    ...
    $scope.elegirTipoRiesgo = function (i) {
      //cons
      if ($scope.listados.tipodeRiesgo[i].Estado == true) {
        $scope.tipoRiesgos.push($scope.listados.tipodeRiesgo[i].Codigo);
        if ($scope.tipoRiesgos.length > 3) {
          $scope.listados.tipodeRiesgo[i].Estado = false;
        ...
        var index = $scope.tipoRiesgos.findIndex(function (objeto) {
          return objeto === $scope.listados.tipodeRiesgo[i].Codigo;
        });
      ...
      for (let index = 0; index < $scope.listados.tipodeRiesgo.length; index++) {
        $scope.tipoRiesgos.forEach(element => {
          if ($scope.listados.tipodeRiesgo[index].Codigo == element) {
            $scope.listados.tipodeRiesgo[index].Estado = true;
          }
        });
      }
     // console.log($scope.tipoRiesgos);
...
//      console.log($scope.campana)
    ...
    $scope.guardadoRiesgos = function (seguir) {
      ...
      $http({
        ...
        data: {
          ...
          P_V_OBJETIVOS_ESTRATEGICOS_RELACIONADO: $scope.riesgos.objetivosEstrategicos,
        }
      }).then(function (response) {
        if (response.data.codigo == 0) {
          ...
          if (seguir) {
            ...
            $scope.riesgoElegido.ID = response.data.numeroId
    ...
    $scope.validacionCamposRiesgos = function () {
      if ($scope.riesgos.proceso == "" || $scope.riesgos.objetivosEstrategicos == "" || $scope.riesgos.proceso == undefined || $scope.riesgos.proceso == null
    ...
    $scope.siguiente = function () {
      ...
      $scope.guardadoRiesgos(true,$scope.riesgos)
      $scope.menu = "tabla-controles";
    }
    ...
    $scope.guardadoControl = function (P_N_NUMERO_CONTROL, P_V_ESTADO, P_V_DESCRIPCION_CONTROL, P_I_RESPONSABLE_ID, P_I_AC_TIPO_ID, P_I_AC_IMPLEMENTACION_ID, P_I_AC_DOCUMENTACION_ID, P_I_AC_FRECUENCIA_ID, P_I_AC_EVIDENCIA_ID, P_I_VALORACION_CONTROL) {
      $http({
        ...
        data: {
          ...
          P_V_NUMERO_RIESGO:$scope.riesgoElegido.ID,
