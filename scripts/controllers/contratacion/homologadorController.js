...
angular.module('GenesisApp')
  .controller('homologadorController', ['$scope', 'afiliacionHttp', 'notification', 'cfpLoadingBar', '$http', 'ngDialog', 
  function ($scope, afiliacionHttp, notification, cfpLoadingBar, $http, ngDialog) {
    ...
    $(document).ready(function () {
        ...
        $scope.vernombre = false;
        $scope.verresultado = false;
        
    });
    $http({
        method: 'POST',
        url: "php/contratacion/homologador.php",
        data: { function: 'listar_tarifas' }
      }).then(function (response) {
        $scope.listadetarifas = response.data;
      });
      ...
      $scope.setTab = function (x) {
            ...
            $scope.BuscarProducto = function (productocodigo) {
                    swal({ title: 'Cargando...' });
                    swal.showLoading();
                    $http({
                      method: "POST",
                      ...
                      data: {
                        ...
                        // productocodigo: '890202',
                      },
                    }).then(function (response) {
                      swal.close();
                        if (response.data[0]) {
                            $scope.vernombre = true;
                            $scope.nombreproducto = response.data[0].NOMBRE;
                            $scope.numeroproducto = response.data[0].NUMERO;
                      } else {
                        $scope.vernombre = false;
                        swal({
                          title: "¡IMPORTANTE!",
              ...
              $scope.BuscarHomologos = function () {
                    swal({ title: 'Cargando...' });
                swal.showLoading();
                    $http({
                      method: "POST",
                      ...
                      data: {
                        ...
                        numeroproducto: $scope.numeroproducto,
                        tarifaseleccionar: $scope.check_option == false ? $scope.tarifaseleccionar :'',
                        ajusterealizar: $scope.check_option == false ? $scope.ajusterealizar :'',
                        porcentaje: $scope.check_option == false ? $scope.porcentaje :'',
                        valorenviar: $scope.check_option == false ? '' : $scope.valorenviar
                      },
                    }).then(function (response) {
                        $scope.verresultado = true;
                      $scope.datosjson = response.data.varJson;
                      $scope.datoscursor = response.data.varCursor[0];
                      swal.close();
