"use strict";angular
  ...
  .controller("supervisiondecontratoController", ["$scope","$http","$window",function ($scope, $http, $window) {
      ...
      $scope.Inicio = function () {
        ...
        // $scope.numeroidProceso = '';
        $scope.control_inicio_Seccion();
        $scope.p_obtener_consol();
      ...
      $scope.controlVista = function (vista){
      ...
      $scope.p_obtener_consol = function () {
        $http({
          method: "POST",
          url: "php/contratacion/supervisiondecontrato.php",
          data: {
      ...
      $scope.p_obtener_ips_supervision = function (data) {
          ...
          if ($scope.nuevasuper.fecha_Vista == "" || $scope.nuevasuper.fecha_Vista == undefined || $scope.nuevasuper.fecha_Vista == null|| $scope.nuevasuper.selecRegimen == "" || $scope.nuevasuper.prestadorSupervisado == "" || $scope.nuevasuper.representante_prestador == "" || $scope.nuevasuper.cargo_repredentante == "" ) {
            ...
            swal({
              ...
              if (result) {
                ...
                $scope.vistanuevaSupervision = false;
                setTimeout(() => {
                  $scope.p_obtener_preguntas_inicial();
                  $scope.$apply();
                }, 50);
              }
            });
      ...
      $scope.p_obtener_preguntas_inicial = function () {
            ...
            if (result) {
                $http({
                  ...
                  data: {
                    ...
                    vpidproceso: $scope.numeroidProceso,
                  ...
                  if (data && data.toString().substr(0, 3) != "<br") {
                    if( data.Proceso == 7 && $scope.tipodeRol == 'SUPLENTE' ){
                      ...
                      $scope.numeroidProceso = data.Renglon;
                    $scope.infoRenglon = data.Renglon;
                    $scope.generar_Preguntas(data.Proceso,data.Renglon);
                    swal({
     ...
     return;
      ...
      $scope.guardarpreguntasUpdate = function(info){
      let datos = [];
      let vacioEncontrados = 0;
        ...
        if (result) {
            $http({
              ...
              if( $scope.numeroproceso != info){
                ...
                setTimeout(() => {
                  $scope.editarSupervision($scope.infoRenglon);
                  $scope.$apply();
                 },1000)
              } else {
                swal({
      ...
      $scope.generar_Preguntas = function (accion,info) {
        let numeroProceso = info;
        let numero = '1';
        $scope.secuencia =  Number.parseInt(accion) + Number.parseInt(numero);
        if($scope.secuencia == 8 && $scope.tipodeRol == 'SUPERVISOR'){
          // swal({
          //   title: "INFORMACION",
          //   text: 'Has terminado el proceso de supervison',
          //   type: "success",
          // }).catch(swal.noop);
          // setTimeout(() => {
          //   $scope.Inicio();
        ...
        if(numeroProceso == '8' && $scope.tipodeRol != 'SUPERVISOR'){
          swal({
            title: "INFORMACION",
            text: 'Has terminado el proceso de supervison por favor informar a su supervisor',
            type: "success",
      ...
      $scope.guardarpreguntasUpdate2 = function(info){
      let datos = [];
      let vacioEncontrados = 0;
      //$scope.listaprocesosedit2 = [];
        ...
        if (result) {
          if (vacioEncontrados == 0) {
            $http({
              ...
              var numero_idProceso =  data.Renglon;
              $scope.editar_Supervision(numero_idProceso);
              $scope.generar_PreguntasUpdate(info);
              if (data && data.toString().substr(0, 3) != "<br") {
                $scope.infoRenglon = data.Renglon;
                swal({
                  title: "Proceso de supervision",
      ...
      $scope.editar_Supervision = function (info) {
        $scope.listaprocesosedit1 = [];
         let idProceso = info;
             $http({
               method: "POST",
      ...
      $scope.editarSupervision = function (info) {
        ...
        $http({
          ...
          $scope.vistaConsulta = false;
          $scope.vistapreguntasUpdate = true;
          $scope.generar_PreguntasUpdate($scope.procesoActual);
      ...
      $scope.generar_PreguntasUpdate = function (accion) {
        var numero1 = '1';
        var numero2 = accion;
      ...
      $scope.calcula_Resultado = function (accion) {
        var idproceso = $scope.numeroidProceso !== undefined ? $scope.numeroidProceso : $scope.id_proceso_Editar;
        ...
        if (accion == "U") {
          ...
          $http({
            ...
            if (
              ...
              $scope.edit.observacionesResult2 =
                response.data[0].RECOMENDACIONES;
            } else {
              swal({
                title: "¡Ocurrio un error!",
                text: data,
          ...
          if ($scope.form1.observacionesResult1 == "" || $scope.form1.observacionesResult2 == "") {
            ...
            $http({
              ...
              if (
                ...
                if ($scope.infoRenglon == undefined) {
                  $window.open(
                    "views/contratacion/formatos/supervisiondeloscontratos.php?&vprenglon=" +
                    idproceso
                  );
      ...
      $scope.obtener_supervision_detalle = function (data) {
      ...
      $scope.eliminarSupervcion = function (info){
        let numeroid_Proceso = info.ID_PROCESO;
          swal({
            title: "IMPORTANTE",
                      ...
                      if(data.Codigo == 0){
                        swal({title: "SUPERVISION",
                              ...
                              setTimeout(() => {
                                      $scope.Inicio();
                                      $scope.$apply();
                                      $scope.P_OBTENER_IPS_SUPERVISOR
                                    }, 1000);
      ...
      $scope.bucarSupervision = function (dato) {
        $scope.limpiar("form2");
        $scope.consulta_Contrato(dato);
        $scope.vistaConsulta = false;
        $scope.vistaconsultaContrato = true;
      };
      $scope.nuevaSupervision = function (dato) {
        $scope.limpiar("form1");
        $scope.p_obtener_ips_supervision(dato);
        $scope.listContrato = [];
        $scope.lista_pregunta_procesos = [];
        $scope.vistaConsulta = false;
      ...
      $scope.lista_supervisores = function () {
        $http({
          method: "POST",
          url: "php/contratacion/supervisiondecontrato.php",
          data: {
      ...
      $scope.obtener_funcionarios = function (info) {
        if(info == 'I'){
      ...
      $scope.supervisorSeleccionado = function (info,accion){
