...
angular.module('GenesisApp')
    .controller('gestionprescripcioncontroller', ['$scope', '$http', 'notification', 'mipresHTTP', 'ngDialog', 'Popeye', '$filter',
        function ($scope, $http, notification, mipresHTTP, ngDialog, Popeye, $filter) {
...
$scope.Obtener_Tipos_Documentos();
            ...
            $scope.consulta_mipres_afi = function(){
                $http({
                   ...
                   swal('Error','No se encontraron MIPRES correspondientes al afiliado','error');
                   }
        ...
        $scope.consolidado = function(tipo){
           ...
           //  }).then(function (res) {
                ...
                switch  (tipo){
                    ...
                    break;
                    default:
                        swal('Pendiente','Nos encontramos actualizando el modulo, agradecemos la espera','warning');
                    break;
                }
            ...
            $scope.buscar_pres = function () {
                ...
                mipresHTTP.consultarpresxfecha(formatDate($scope.fecha_i),formatDate($scope.fecha_f), $scope.regimen).then(function (response) {
                    if (response !== 0) {
                    ...
                    datosFiltrados.forEach(dia => {
                        dia.forEach(pr => {
                            ...
                            pr.prescripcion.FPrescripcion = formatDate(pr.prescripcion.FPrescripcion);
                    ...
                    setTimeout(function () { document.getElementById("table_presc").scrollIntoView({ block: 'start', behavior: 'smooth' }); }, 300);
        ...
        $scope.inserta_prescripcion_cabeza = function(datos){
            ...
            $scope.buscar_pres_x_doc = function () {
                if ($scope.tipodoc == '' || $scope.doc == '' || $scope.tipodoc == null || $scope.doc == null || $scope.tipodoc == undefined || $scope.doc == undefined || $scope.fecha_id == '' || $scope.fecha_id == undefined) {
                    ...
                    //     }
                    // });
                    mipresHTTP.buscar_pres_x_doc($scope.regimen, $scope.doc, $scope.tipodoc, formatDate($scope.fecha_id)).then(function (response) {
               ...
               $scope.buscar_pres_x_num = function () {
                ...
                if ($scope.no_pres == '' || $scope.regimen == '' || $scope.no_pres == null || $scope.regimen == null) {
                    swal('Error', 'Favor completar los campos', 'warning')
                } else {
                    mipresHTTP.buscar_pres_x_num($scope.regimen, $scope.no_pres).then(function (res) {
                        ...
                        if (res.length == 0) {
                            ...
                            var promise = $scope.inserta_prescripcion_cabeza($scope.resultado2);
            ...
            $scope.exportar = function () {
                $http({
                    method: 'POST',
                    url: $scope.select_tecnologia=''?"php/recobro/genera_xls.php":"php/recobro/genera_xls.php/?tipo=" + $scope.select_tecnologia,
            ...
            $scope.configPages = function () {
                ...
                if (ini < 1) {
                    ini = 1;
                    if (Math.ceil($scope.listaPRESTemp.length / $scope.pageSize) > $scope.valmaxpag)
                        fin = 10;
                    else
                        fin = Math.ceil($scope.listaPRESTemp.length / $scope.pageSize);
                } else {
                    if (ini >= Math.ceil($scope.listaPRESTemp.length / $scope.pageSize) - $scope.valmaxpag) {
                        ini = Math.ceil($scope.listaPRESTemp.length / $scope.pageSize) - $scope.valmaxpag;
                        fin = Math.ceil($scope.listaPRESTemp.length / $scope.pageSize);
                    }
                }
                if (ini < 1) ini = 1;
