"use strict";
angular.module("GenesisApp").controller("mipresTutelacontroller", ["$scope", "$http", "mipresHTTP", "notification", "$timeout", "$rootScope", "$window", "ngDialog", "Popeye", "FileProcessor",
  function ($scope, $http, mipresHTTP, notification, $timeout, $rootScope, $window, ngDialog, Popeye, FileProcessor) {
    ...
    $scope.search_tutela = function () {
      ...
      if ($scope.vw_dir==false) {
        ...
        $http({
          ...
          if (response.data.length == 0) {
              ...
              $scope.newarray = response.data.map(pr => {
                  ...
                  pr.tutela.EstPres = pr.tutela.EstPres === 4 ? 'Activo' : 'No Activo';
                  pr.tutela.Ftutela = formatDate(pr.tutela.Ftutela);
                  const object = pr.tutela;
                  object.dispositivos = pr.dispositivos;
                  object.actdispositivos = pr.dispositivos.length === 0 ? true : false;
                  ...
                  if (pr.medicamentos != null) {
                      object.medicamentos.forEach(medicament => {
                          medicament.tipo_tec = 'M';
                          medicament.NoTutela = object.NoTutela;
                          medicament.TipoIDPaciente = object.TipoIDPaciente;
                          medicament.NroIDPaciente = object.NroIDPaciente;
                  ...
                  if (pr.procedimientos != null) {
                      object.procedimientos.forEach(procedimient => {
                          procedimient.tipo_tec = 'P';
                          procedimient.NoTutela = object.NoTutela;
                          procedimient.TipoIDPaciente = object.TipoIDPaciente;
                          procedimient.NroIDPaciente = object.NroIDPaciente;
                  ...
                  if (pr.productosnutricionales != null) {
                      object.productosnutricionales.forEach(productosnutricional => {
                          productosnutricional.tipo_tec = 'N';
                          productosnutricional.NoTutela = object.NoTutela;
                          productosnutricional.TipoIDPaciente = object.TipoIDPaciente;
                          productosnutricional.NroIDPaciente = object.NroIDPaciente;
                  ...
                  if (pr.serviciosComplementarios != null) {
                      object.serviciosComplementarios.forEach(serviciosComplementari => {
                          serviciosComplementari.tipo_tec = 'S';
                          serviciosComplementari.NoTutela = object.NoTutela;
                          serviciosComplementari.TipoIDPaciente = object.TipoIDPaciente;
                          serviciosComplementari.NroIDPaciente = object.NroIDPaciente;
                  ...
                  if (pr.dispositivos != null) {
                      object.dispositivos.forEach(dispositivo => {
                          ...
                          dispositivo.NoTutela = object.NoTutela;
        ...
        //           object.medicamentos.forEach(medicament => {
        //             medicament.tipo_tec = 'M';
        //             medicament.Notutela = object.NoPrescripcion;
        //             medicament.TipoIDPaciente = object.TipoIDPaciente;
        //             medicament.NroIDPaciente = object.NroIDPaciente;
    ...
    $scope.search_jp = function () {
      ...
      $http({
        method: 'POST',
        url: "php/recobro/mipres.php",
        // url: "json/recobro/juntas.json",
        data: {
          function: $scope.vw_jp == false ? 'get_jp' : 'get_jp_no',
          'dato': $scope.vw_jp == false ? formatDate($scope.fechajp) : $scope.no_presjp,
          'regimen': $scope.regimen
        }
      }).then(function (response) {
        if (response.data !== "1") {
          ...
          $scope.verjp = false; 
          swal.close();
          $scope.junta = response.data;

          $scope.animacion = 'slideInRight';

          $scope.modificacion = response.data[0].junta_profesional;
          ...
          $scope.newarray = response.data.map(pr => {
            ...
            const newItem = {};
            Object.keys(pr.junta_profesional).forEach(key => {
              if (textMap[key]) {
                if (pr.junta_profesional[key] === null) {
                  newItem[key] = 'No Aplica'
                } else {
                  newItem[key] = textMap[key][pr.junta_profesional[key]]
                }
              } else {
                newItem[key] = pr.junta_profesional[key]
              }
            });


            // const estadojm = pr.map(item => {

            //   return newItem;
            // })
           
             const object = {};
             object.junta_profesional = newItem;
            // object.EstJM = newItem;
          ...
          $scope.listModificaciones = $scope.newarray;
          ...
          $scope.verModificaciones = function (actual, opcion) {
            ...
            if (opcion == 'next') {
              if ($scope.total == actual + 1) {
                ...
                $scope.modificacion = $scope.listModificaciones[actual].junta_profesional;
    ...
    $scope.open_modal = function (modal, datos_dir,vw) {
