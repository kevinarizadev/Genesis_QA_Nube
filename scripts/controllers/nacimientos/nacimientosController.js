...
angular.module('GenesisApp')
  .controller('nacimientosController', ['$scope', '$filter', 'consultaHTTP', 'notification', '$timeout', '$rootScope', '$http', '$window', 'ngDialog', 'notify',
    function ($scope, $filter, consultaHTTP, notification, $timeout, $rootScope, $http, $window, ngDialog, notify) {
      $scope.contar = 0;
      $scope.mostrar = 1;
      $scope.color = "orange";
      var x = new Date()
      $scope.ano = x.getUTCFullYear();
      $scope.cantidad = "100";
      ...
      $scope.mostrarMunicipios = () => {
        // $scope.nombre_depart = nombre;
        // $scope.cantidad_depart = cantidad;
        // $scope.depaprincipal = departamento;

        $scope.filterMunicipio($scope.depaprincipal, $scope.nombre_depart, $scope.cantidad_depart);
      }
      ...
      $scope.mostrardiagnosticos = function () {
        ...
        $http({
          ...
          $scope.lista_diag = response.data.map((item) => {
            return  {
              nombre: item.NOMBRE,
              codigo: item.CODIGO
            }
      ...
      // CNVU - FILTRAR ESTADOS
      $scope.myFilter = function (value) {
        $scope.iconGestion = value;
        if (value) {
          $scope.mesasayudasTemp = $scope.mesasayudas.filter(function (el) {
            return el.estado == 'N' || el.estado == 'L';
          })
        }else{
          $scope.mesasayudasTemp = $scope.mesasayudas.filter(function (el) {
            return el.estado == 'A';
      ...
      // CNVU - CAMBIO DE ESTADO DEL REGISTRO
      $scope.cambiarEstado = function (tipodocumento, documento,ano,estado){
        swal({
          title: 'Confirmar',
          text: '¿Desea cambiar el estado de este Nacimiento?',
          type: 'question',
          ...
          if (result) {
            $http({
              ...
              if (response.data.codigo == 0) {
                ...
                $scope.filterMunicipio($scope.depaprincipal, $scope.nombre_depart, $scope.cantidad_depart);
                $scope.filtertabla($scope.munprincipal, $scope.municipio_n, $scope.cantidad_mun);
                $scope.cantidad_mun = $scope.cantidad_mun-1;
                $scope.municipio_nombre = $scope.municipio_n + " (TOTAL " + ($scope.cantidad_mun) + ")";
              } else {
      ...
      // CNVU - FIN CAMBIO DE ESTADO DEL REGISTRO

      $scope.mostrar_panel = function () {
        $scope.check_option_estado = false;
        $scope.verfiltrar = false;
      ...
      $scope.tendencia2 = function (id, nombre) {
        ...
        $http({
          ...
          method: 'POST',
          url: "php/nacimientos/nacimientos.php",
          data: { function: 'graficaseccionales', anno: String($scope.ano), seccional: id }
        }).then(function (response) {
          console.log(response.data);
          notify.close()
          ...
          // CNVU
          var pendientes = (parseInt(response.data[0].total) - parseInt(response.data[0].afiliaciones));
          var porcentajePendientes = (100 - parseInt(response.data[0].porcentajes));
          ...
          Highcharts.chart(id, {
            ...
            xAxis: [{
              categories: ['Afiliaciones', 'Pendientes','Nacimientos']
              // crosshair: true
            }],
            plotOptions: {
              series: {
            ...
            series: [{
              name: 'Nacimientos',
              type: 'column',
              data: [{ y: afiliaciones, color: '#1a2e63', dataLabels: { enabled: true, format: porcentajes_secional_grafica + "%" } }
                , { y: pendientes, color: '#F44336', dataLabels: { enabled: true, format: porcentajePendientes + "%" }}
                ,{ y: total, color: '#7cb5ec' } ],
              tooltip: {
                valueSuffix: ''
              }
      ...
      $scope.filterMunicipio = function (departamento, nombre, cantidad) {
        ...
        $http({
          ...
          console.log(response);
          if (response) {
            //CNVU - SOLUCIONA ERROR AL GESTIONAR
            //$scope.municipio = "";
            $scope.Municipios = response.data.map((item) => {
              item.CANTIDAD_TOTAL = item.CANTIDAD;
              return item;
            });
            //$scope.munprincipal = "";
            $scope.mostrar = 2;
      ...
      //CNVU - PORCENTAJES GESTIONADOS
      $scope.porcentajesGestionados = function (cantidad, total) {
      ...
      $scope.filter = function (val) {
        $scope.myFilter($scope.check_option_estado);
        $scope.mesasayudasTemp = $filter('filter')($scope.mesasayudasTemp, val);
        $scope.configPages();
      }
