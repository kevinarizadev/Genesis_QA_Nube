...
.controller('infopersonalcontroller', ['$scope', '$http', 'ngDialog', 'afiliacionHttp', 'notification', '$timeout', '$q', 'upload', 'communication', '$controller', 'validationParams','$rootScope','$localStorage','$window',
function ($scope, $http, ngDialog, afiliacionHttp, notification, $timeout, $q, upload, communication, $controller, validationParams, $rootScope,$localStorage,$window) {
  var self = this;
...
$scope.mostrar4 = function(){
  ...
  }else { $scope.inactive4 = true}
}
$scope.mostrar5 = function() {
  if ($scope.filtro.selecttramite=='C'){
    if ($scope.filtro.escenario != "SELECCIONAR") {
...
var uploadField = document.getElementById("adjuntoAfiliado");

uploadField.onchange = function() {
    if(this.files[0].size > 8388608){
       swal('Notificación','La imagen es muy pesada','warning');
       this.value = "";
    };
};
  ...
  $http({
    ...
    $localStorage.fuarbef = fuarbef;
  	...
  	$timeout(function() {
                    ...
                    //$window.open('views/formularios/fuar.php', '_blank', "width=1080,height=1100");
                    $window.open('views/consultaafiliados/soportes/fuar.php?tipo='+fuar.TIPODOCUMENTO   +'&id='+fuar.DOCUMENTO,'_blank', "width=1080,height=1100");
                    $window.open('views/formularios/carnet.html', '_blank', "width=1080,height=1100");
                    for (var i = 0; i <= $scope.datafuar.length - 1; i++) {
                       $window.open('views/consultaafiliados/soportes/acta.php?tipo=' + $scope.datafuar[i].TIPODOCUMENTO + '&id=' + $scope.datafuar[i].DOCUMENTO, '_blank', "width=1080,height=1100");
                    }
                 }, 1000);
  }, function errorCallback(response) {
    swal('Mensaje',response.data,'error')
  ...
  loadingButton("guarda");
    if ($scope.filtro.selectZona === undefined || $scope.filtro.selectZona == "" || $scope.filtro.selectZona == " ") {
      ...
      $http({
        method:'POST',
        url:"php/aseguramiento/Cafiliacion.php",
        data: {function:'insertarafiliado',estado: $scope.estado_afiliado,tipo_documento:$scope.parameters.tipo_documento, documento:$scope.parameters.documento, cbf_tipo_documento:'', cbf_documento:'', cbf_tipo:'F',
...
// *** VENTANAS MODALES ***
self.AgregarBeneficiario = function () {
...
self.MostrarBeneficiario = function () {
  ngDialog.open({
