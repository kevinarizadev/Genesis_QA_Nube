...
$scope.ValidoFuncionario = function () {
   ...
   if ($scope.ubicacion == '1') {
      ...
      $scope.Habilitar = true;
         ...
         $scope.buscarDocumentosRevisar = function (fechainicio, fechafin,numerodocumento) {
            swal({ title: 'Buscando...' });
            swal.showLoading();
            ...
            if($scope.check_option == false){
              ...
              var numerodocumento = "";
               fechainicio = parsedia(fechainicio)
               fechafin = parsedia(fechafin)
            }
            $http({
              method: 'POST',
              url: "php/digitalizacion/funcdigitalizacion.php",
              data: { function: 'BusquedaRevision', 
              documento:numerodocumento,
              fechainicio: fechainicio, 
              fechafin: fechafin}
            }).then(function (response) {
               $scope.OcultarInformacion = false;
               $scope.VerDocumentosPendientes = false;
               swal.close();
              $scope.listarDocumentosPendiente = response.data;
              setTimeout(function () {
                $scope.table = $('#tablaverpendientes').DataTable({
                  language: { "url": "//cdn.datatables.net/plug-ins/1.10.16/i18n/Spanish.json" },
                  lengthMenu: [[10, 20, 50, -1], [10, 20, 50, 'Todas']],
                  order: [[0, "asc"]]
                });
                $scope.table.draw();
                document.getElementById('tablaverpendientes').scrollIntoView({ block: 'start', behavior: 'smooth' });
              }, 100);
            })
          }
         // $scope.buscarDocumentosRevisar = function (fechainicio, fechafin) {
         //    $('#tablaverpendientes').DataTable().destroy();
         //    swal({ title: 'Cargando Información Del Afiliado', allowEscapeKey: false, allowOutsideClick: false });
         //    swal.showLoading();
         //    var table = $("#tablaverpendientes").DataTable({
         //       "aProcessing": true,
         //       "aServerSide": true,
         //       "language": {
         ...
         //       ajax: {
         //          method: 'POST',
         //          url: "php/digitalizacion/datosderevision.php",
         //          data: { 'function': 'BusquedaRevision', fechainicio: parsedia(fechainicio), fechafin: parsedia(fechafin)}
         //       },
         //       "columns": [
         //           { "data": "tipo_doc" },
         //           { "data": "documento" },
         ...
         //       swal.close();
         //  }
         $scope.VerDocumentosAfiliado = function(documento,tipodocumento) {
            $scope.VerDocumentosRevisar(documento,tipodocumento);
            swal({ title: 'Cargando Información Del Afiliado', allowEscapeKey: false, allowOutsideClick: false });
            swal.showLoading();
            $scope.escenario = '';
            $scope.info = {};
            $http({
               method: 'POST',
               url: "php/digitalizacion/funcdigitalizacion.php",
               data: { function: 'busquedaafiliado', type: tipodocumento, id: documento }
            }).then(function (res) {
               if (res.data.MENSAJE == 1) {
                  $scope.OcultarInformacion = true;
                  $scope.VerDocumentosPendientes = true;
                  $scope.datosrevisarafiliado = res.data;
                  $scope.nombrecompleto = $scope.datosrevisarafiliado.PRIMERNOMBRE+' '+$scope.datosrevisarafiliado.SEGUNDONOMBRE+' '+$scope.datosrevisarafiliado.PRIMERAPELLIDO+' '+$scope.datosrevisarafiliado.SEGUNDOAPELLIDO;
                  var date_formato = res.data.FECHA_NACIMIENTO.split("/");
                  $scope.info.fecha_nacimiento = new Date(date_formato[2], date_formato[1] - 1, date_formato[0]);
               } else {
                  swal('Notificación', res.data.MENSAJE, 'info');
         ...
         $scope.VerDocumentosRevisar = function (documento,tipodocumento) {
            $http({
              method: 'POST',
              url: "php/digitalizacion/funcdigitalizacion.php",
              data: { function: 'VerDocumentosRevision', documento: documento, tipodocumento: tipodocumento}
            }).then(function (response) {
              $scope.listarDocumentosVer = response.data;
              
            })
          }
         $scope.VerSoporte = function (ruta) {
            $scope.vistadelsoporte ="";
            $http({
               method: 'POST',
               url: "php/digitalizacion/funcdigitalizacion.php",
               ...
               $scope.vistadelsoporte = "temp/" + response.data;
               $("#modalversoporte").modal("open");
             });
          }
          $scope.closemodals = function () {
                $("#modalversoporte").modal("close");
               }
               $scope.documentosrevisados = []
            $scope.temprevision = function (documentos,estado) {
               $scope.var_documento = documentos;
               $scope.var_estado = estado;
               if(estado == "R"){
                  $http({
                     method: 'POST',
                     url: "php/digitalizacion/funcdigitalizacion.php",
                     ...
                     swal({
                        ...
                        preConfirm: function(result) {
                           return new Promise(function(resolve, reject) {
                             setTimeout(function() {
                               if (result === '') {
                                 reject('Por favor seleccione un motivo de rechazo');
                               } else {
                                 resolve();
                               }
                             }, 500);
                        ...
                        var datosrecibidos = {"documentos":$scope.var_documento,"estado":$scope.var_estado,"comentario":result};
                        $scope.documentosrevisados = $scope.documentosrevisados.filter(function(item) {
                           return item.documentos !== datosrecibidos.documentos
                       })
                        $scope.documentosrevisados.push(datosrecibidos);
                        if($scope.documentosrevisados.length == $scope.listarDocumentosVer.length){
                           $("#habilitarbotonfinalizar").removeAttr('disabled');
                        }
                        console.log($scope.botonfinalizarhabilitar)
                     })
                  })
               } else {
                  var datosrecibidos = {"documentos":$scope.var_documento,"estado":$scope.var_estado,"comentario":""};
                  $scope.documentosrevisados = $scope.documentosrevisados.filter(function(item) {
                     return item.documentos !== datosrecibidos.documentos
                 })
                  $scope.documentosrevisados.push(datosrecibidos);
               ...
               $scope.FinalizarRevision = function () {
                  ...
                  swal({
                     title: 'Confirmar',
                     text: 'Seguro Desea Finalizar La Revision?',
                     type: 'question',
                     showCancelButton: true,
                     confirmButtonColor: '#3085d6',
                     cancelButtonColor: '#d33',
                     ...
                     if (result) {
                        $http({
                           method: 'POST',
                           url: "php/digitalizacion/funcdigitalizacion.php",
                           data: { function: 'FinalizatodaRevisionDoc', datosenviar: JSON.stringify($scope.documentosrevisados)}
                        }).then(function (response) {
                           if (response.data.coderror == 0){
                              // $scope.VerDocumentosRevisar($scope.datosrevisarafiliado.DOCUMENTO,$scope.datosrevisarafiliado.TIPODOCUMENTO);
                              swal({
                                 title: "Completado!",
                                 text: response.data.mensaje,
                                 type: "success"
                             })
                             $scope.AtrasDocumentosAfiliado();
                             $scope.buscarDocumentosRevisar($scope.fechainicial,$scope.fechafinal)
                           }else{
                              swal({
                                 title: "Completado!",
                                 text: response.data.mensaje,
         ...
         $scope.AtrasDocumentosAfiliado = function() {
            $("#habilitarbotonfinalizar").attr('disabled', 'disabled');
            $scope.documentosrevisados = []
            $scope.OcultarInformacion = false;
            $scope.VerDocumentosPendientes = false;
