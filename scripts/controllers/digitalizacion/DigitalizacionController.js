angular.module('GenesisApp')
	.controller('DigitalizacionController', ['$scope', '$http', 'digitalizacionHTTP', '$compile', '$rootScope', '$sce',
		function ($scope, $http, digitalizacionHTTP, $compile, $rootScope, $sce) {
			...
			$scope.AbrirModal = function () {
				...
				$scope.drawImg = function () {
					...
					$scope.listaImg.push({ img: imgObj, width: imgObj.width, height: imgObj.height, x: 0, y: y, codigo: 0 });
					y += imgH + 15;
				...
				$scope.SubirCarpeta = function () {
					...
					function finalizaScan(successful, mesg, response) {
						swal({ title: 'Cargando Documentos' });
						swal.showLoading();
						if (!successful) {
							console.error('Failed: ' + mesg);
							return;
			...
			$scope.fileToBase64 = function (filesSelected, name, ext, id) {
				...
				reader.onload = function (e) {
					$scope.panelDatos.push({ base64: e.target.result, name: name, ext: ext, id: id });
					$scope.LimpiarCargue();
				};
