'use strict';
angular.module('GenesisApp').controller('reportefinanciera', ['$scope', 'consultaHTTP', 'notification', 'cfpLoadingBar', '$http', '$window', '$filter',
      function ($scope, consultaHTTP, notification, cfpLoadingBar, $http, $window, $filter) {
        ...
        // $scope.tiporeporte = 0;
         ...
         $scope.enviar_anexo3 = function () {
            ...
            $scope.list_Trimetre = function (accion){
               $scope.listTrimetre = [];
               switch (accion) {
                  case "2023":
                     $scope.listTrimetre =[
               ...
               switch ($scope.tiporeporte) {
                  ...
                  case "79": //Capitas Radicadas
                    ...
                    case "95": //Capitas Radicadas
                     ...
                     $scope.tercero = '0';
                     break;
                     case "104": //Radicacion de Facturas (BINT)
                     $scope.shw_annos = false;
                     $scope.shw_nit = false;
                     $scope.shw_fecha_inicio = true;
                     $scope.shw_fecha_final = true;
                     ...
                     $scope.tercero = '0';
                     break;
                     case "108": //Saldo Ips Auditoria
                     $scope.shw_annos = false;
                     $scope.shw_anno_select = true;
                     $scope.shw_nit = true;
                     $scope.shw_periodo_list_trimestre = true;
                     $scope.shw_fecha_inicio = false;
                     $scope.shw_fecha_final = false;
                     $scope.consmayudaaut = false;
                     $scope.consproductos = false;
         ...
         $scope.hideall = function () {
            ...
            $scope.shw_periodo_list_trimestre = false;
            $scope.shw_anno_select = false;
            $scope.selectanno = '';
            $scope.selectperiodo = '';
            $scope.tercero = '';
            $scope.fecha_inicio = '';
            ...
            switch ($scope.tiporeporte) {
               ...
               case "69":
                  ...
                  case "104":
                     ...
                     break;
                     case "108": //Saldo Ips Auditoria
                        if($scope.tercero == '' || $scope.tercero == undefined  || $scope.selectanno == undefined || $scope.selectanno== '' || $scope.selectperiodo == undefined || $scope.selectperiodo == ''){
                           swal('Importante', 'Para poder generar el reporte debe ingresar el año y el nit y seleccionar el periodo', 'info');
                        }else{
                        
                        window.open('php/financiera/reportes/reporte_saldo_ips.php?anno=' + $scope.selectanno + '&periodo=' + $scope.selectperiodo + '&nit=' + $scope.tercero);
                        swal('Importante', 'Generando reporte', 'info');
                        $scope.consmayudaaut = false;
                        $scope.consproductos = false;
                        $scope.consmovempresa = false;
