...
}).controller('informe_validacion_capitaController', ['$scope', '$http', '$location', function ($scope, $http, $location) {
    ...
    if (validar_json($location.search().datos)) {
        $http({
            ...
            data: {
                ...
                codigo: JSON.parse($location.search().datos).codigo
            }
        }).then(function (response) {
            console.log(response.data);
            $scope.datos = response.data.json;
            $scope.detalle = response.data.datos;
