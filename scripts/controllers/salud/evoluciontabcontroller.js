"use strict";
angular.module("GenesisApp").controller("evoluciontabcontroller", [
  "$scope",
  "censoHttp",
  "ngDialog",
  "$controller",
  "Messages",
  "$window",
  function ($scope, censoHttp, ngDialog, $controller, Messages, $window) {
    ...
    $scope.remisionArray = [
      {
        CODIGO: "S",
        NOMBRE: "PENDIENTE POR REMISION",
      },
      {
        CODIGO: "N",
        NOMBRE: "NO TIENE REMISION",
      },
    ];
    $scope.glosa = {
      valorobjecion: 0,
    ...
    $scope.obtenerEvolucionPaciente = function (
      afiliado,
      ubicacion,
      numerocenso,
      estado,
      ...
      if ($scope.estado_procesado === "PROCESADO") {
        $scope.esprocesado = true;
      } else {
        $scope.esprocesado = false;
      }
      censoHttp
        .obtenerEvolucionPaciente(ubicacion, numerocenso)
    ...
    $scope.ObtenerEvtDetalle = function () {
      censoHttp
        .obtenerEventosadvdeta($scope.eventoadv)
        .then(function (response) {
    ...
    $scope.notpaastenot = function () {
      ...
      Evoldesc.onpaste = function (e) {
        ...
        swal(
          "Notificacion",
          "La accion pegar no esta permitida en este campo. ",
          "error"
        );
      };
      ...
      Evoldesc.oncopy = function (e) {
        ...
        swal(
          "Notificacion",
          "La accion copiar no esta permitida en este campo. ",
          "error"
        );
      };
    };
    ...
    $scope.imprimeCertificadoGlosa = function () {
      $window.open(
        "views/salud/formato/formatoglosa.php?censo=" +
          $scope.numerocenso +
          "&ubicacion=" +
          $scope.ubicacion,
        "_blank",
    ...
    $scope.monitor = function () {
      if (
        sessionStorage.getItem("cedula") != "1047234385" &&
    ...
    $scope.insertarEvolucion = function () {
      ...
      if (
        ...
        if ($scope.remision == "S" || $scope.remision == "N") {
          swal({
          ...
          if ($scope.glosa.motivo != "0") {
            $scope.motivoglo = $scope.glosa.motivo.split(" ")[0];
          } else {
            $scope.motivoglo = "0";
          }
          ...
          censoHttp
            .insertarEvolucion(
              ...
              "",
              $scope.motivoglo,
              $scope.valorcerrarauditoria,
              formatDate($scope.fecha_Cierre),
              $scope.motivo_egreso,
              $scope.remision
            )
            .then(function (response) {
          ...
          swal({
            title: "Validar Informacion",
            text: "Debe indicar si existe remisión",
            type: "warning",
            showCancelButton: false,
            confirmButtonColor: "#3085d6",
          })
