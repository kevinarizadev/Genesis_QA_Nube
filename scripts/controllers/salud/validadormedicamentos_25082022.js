  ...
  $scope.ValidarMD = function (x) {
    ...
    $http.post("php/salud/ValidadorMedicamentosIPS/funcValidmedicamentos.php?ano=" + $scope.anno + "&periodo=" + $scope.periodo, file, {
      ...
      if (response.data.substring(0, 3) == '<br' || response.data == 1) {
        ...
        // console.log($scope.anno,'**',$scope.periodo < 10 ? '0'+$scope.periodo : $scope.periodo,'**',$scope.InputNit,'**',$scope.XnombreArch);
        // swal('IMPORTANTE', 'Exito', 'success');
        if (x == 'N') {
          setTimeout(() => {
            $http({
              method: 'POST',
              ...
              if (typeof (res.data) == 'string' && res.data.substring(0, 3) == '<br') {
                ...
                if (res.data.length > 0) {
                  ...
                  list += "<li class='left-align'><div class='collapsible-header blue-text'><i class='material-icons'>error</i>" + $scope.XnombreArch + " <small class='float-right red-text'> Errores: " + res.data.length + "</small>" + "</div><div class='collapsible-body'>";
                  res.data.forEach(function (error, j) {
                    list += "<p style='padding: 1rem;'><span><b>Error: </b>" + error.Error + "</p>";
                  });
                  list += "</div></li>";
                  // });
                  swal({
