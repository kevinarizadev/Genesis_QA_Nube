"use strict";
angular.module("GenesisApp").controller("censotabcontroller", [
  "$scope",
  "censoHttp",
  "$http",
  "ngDialog",
  ...
  function ($scope, censoHttp, $http, ngDialog, $controller) {
    $(document).ready(function () {
      ...
      var min = sysdate.setDate(sysdate.getDate());
      let min_ingreso = new Date();
      let min_egreso = new Date();
      if (cedula == 49760559 || cedula == 1043016498 || cedula == 1083434770) {
        min_ingreso = sysdate.setDate(sysdate.getDate() - 365);
        min_egreso = sysdate.setDate(sysdate.getDate() - 365);
      } else {
        min_ingreso = sysdate.setDate(sysdate.getDate() - 15);
        min_egreso = sysdate.setDate(sysdate.getDate() - 5);
      }
      $scope.minDate = new Date(min).toISOString().split("T")[0];
      $scope.min_egreso = new Date(min_egreso).toISOString().split("T")[0];
      $scope.min_ingreso = new Date(min_ingreso).toISOString().split("T")[0];
    });
    $scope.textarea = true;
    $scope.tipo_doc = "";
    $scope.form = { tipoestancia: "" };
    $scope.regimen = "";
    $scope.sexo = "";
    $scope.servicio = "";
    ...
    $scope.form.fecha_ingreso = new Date();
    $scope.portabilidad = "";
    ...
    $scope.buscarAutorizacion = function (type) {
      if (type == "A") {
        $scope.show_info = false;
        censoHttp
          .obtenercamposautorizacion($scope.numero_au)
          .then(function (response) {
            if (response.data.length != 0) {
              $scope.listautorizacion = response.data;
              $scope.fecha_au = new Date(
                $scope.listautorizacion[0].FECHAAUT.substring(6, 10),
                $scope.listautorizacion[0].FECHAAUT.substring(3, 5) - 1,
                $scope.listautorizacion[0].FECHAAUT.substring(0, 2)
              );
              $scope.fecha_nac = new Date(
                $scope.listautorizacion[0].NACIMIENTO.substring(6, 10),
                $scope.listautorizacion[0].NACIMIENTO.substring(3, 5) - 1,
                $scope.listautorizacion[0].NACIMIENTO.substring(0, 2)
              );
              ...
              $scope.ips = {
                codigo: $scope.listautorizacion[0].IPS,
                nombre: $scope.listautorizacion[0].NOMBREIPS,
              };
              $scope.nomIps();
              $scope.diagnostico = {
                codigo: $scope.listautorizacion[0].CODDIAGNOSTICO,
                nombre: $scope.listautorizacion[0].NOMBREDIAGNOSTICO,
              };
              $scope.nomDiagnosticos();
              $scope.municipio = {
                codigo: $scope.listautorizacion[0].UBICGEO,
                nombre: $scope.listautorizacion[0].NOMUBICGEO.replace(
                  $scope.listautorizacion[0].UBICGEO,
                  ""
                ),
              };
              $scope.nomMun();
              ...
              $scope.edad = Number(
                $scope.listautorizacion[0].EDAD.substring(0, 3)
              );
              $scope.portabilidad = $scope.listautorizacion[0].PORTABILIDAD;
              $scope.show_info = true;
            } else {
              swal(
                "Informacion",
                "datos de autorizacion no encontrado",
                "info"
              );
            }
          });
        ...
        $scope.show_info = false;
        censoHttp
          .obtenercampospordocumento($scope.tipo_doc, $scope.documento)
          .then(function (response) {
            if (response.data.length != 0) {
              $scope.listautorizacion = response.data;
    ...
    $scope.nomDiagnosticos = function () {
      if (
        $scope.diagnostico.codigo === undefined ||
        $scope.diagnostico.codigo == ""
      ) {
        $scope.diagnostico.seleccionC = "SELECCIONAR";
        $scope.diagnostico.codigo = "0";
      } else {
        $scope.diagnostico.seleccionC =
          $scope.diagnostico.codigo + " - " + $scope.diagnostico.nombre;
      }
    };

    $scope.nomDiagnosticosA = function () {
      if (
        $scope.diagnosticoa.codigo === undefined ||
        $scope.diagnosticoa.codigo == ""
      ) {
        $scope.diagnostico.seleccionCA = "SELECCIONAR";
        $scope.diagnosticoa.codigo = "0";
      } else {
        $scope.diagnosticoa.seleccionCA =
          $scope.diagnosticoa.codigo + " - " + $scope.diagnosticoa.nombre;
      }
    };

    $scope.nomDiagnosticosAd = function () {
      if (
        $scope.diagnosticoad.codigo === undefined ||
        $scope.diagnosticoad.codigo == ""
      ) {
        $scope.diagnosticoad.seleccionCAd = "SELECCIONAR";
        $scope.diagnosticoad.codigo = "0";
      } else {
        $scope.diagnosticoad.seleccionCAd =
          $scope.diagnosticoad.codigo + " - " + $scope.diagnosticoad.nombre;
      }
    };
    ...
    $scope.nomMun = function () {
      if (
        $scope.municipio.codigo === undefined ||
        $scope.municipio.codigo == ""
      ) {
        $scope.municipio.seleccion = "SELECCIONAR";
        $scope.municipio.codigo = "0";
      } else {
        $scope.municipio.seleccion =
          $scope.municipio.codigo +
          " - " +
          $scope.municipio.nombre.replace($scope.municipio.codigo, "");
      }
    };
    ...
    $scope.CrearCenso = function () {
      console.log($scope.form.tipoestancia);
      $scope.diferencia_fechas($scope.form.fecha_ingreso);
      setTimeout(() => {
        $scope.$apply();
      }, 500);
      if ($scope.validacion_200dias >= 4) {
        ...
        $scope.showhijode();
        console.log(formatDate($scope.form.fecha_ingreso));
        if (
          $scope.form.tipoestancia == "" ||
          $scope.diagnostico.codigo == "" ||
          $scope.form.fecha_ingreso == "" ||
          $scope.form.tipoestancia == null ||
          $scope.diagnostico.codigo == null ||
          $scope.form.fecha_ingreso == null
        ) {
          swal(
            "Notificación",
          ...
          censoHttp
            .CrearCenso(
              ...
              $scope.form.tipoestancia,
              $scope.cedula,
              formatDate($scope.form.fecha_ingreso),
              $scope.eshijode,
              $scope.eshijonacido,
              $scope.cantidadhijo,
              $scope.diagnostico.codigo,
              ...
              if (response.data.codigo != 0) {
                ...
                if ($scope.form.tipoestancia == "HS") {
                  $http({
                    ...
                    swal("Completado", response.data.mensaje, "success").then(
                      (result) => {
                        censoHttp
