'use strict';
angular.module('GenesisApp').controller('auditoriascuentaspgpController', ['$scope', '$http', '$timeout', '$filter', 'ngDialog', 'notify', function ($scope, $http, $timeout, $filter, ngDialog, notify) {
    // Plantilla funcional
    console.clear();
    $(document).ready(function () {
        ...
        $scope.DetalleGestion = false;
        if (parseInt(cedula) == 56086512) {
            $scope.permisoDevolverDescuento = true;
        }else{
            $scope.permisoDevolverDescuento = false;
        }
        ...
        $http({
            method: 'POST',
            url: "php/pgp/auditoriascuentaspgp.php",
            data: {
                function: 'Obt_Cedula'
            }
    ...
    $scope.Ver_Gestion_Factura = function (X) {
        ...
        let table = document.getElementById("Table_Modal_Gestion_Detalle");
        ...
        $timeout(function () {
            ...
            $scope.HojaGest = {
                ESTADO_FD: X.ESTADO_FD,
                NUMERO_FD: X.NUMERO_FD,
                NUMERO: X.NUMERO,
                ESTADO: X.ESTADO,
                NUMERO_REG: X.NUMERO,
            ...
            if (X.ESTADO == 'A' && $scope.Rol_Procesar == 'S') {
                ...
                let tabla2 = document.getElementById("Table_Modal_Gestion_Detalle")
                if (tabla2) {
                    document.getElementById("Table_Modal_Gestion_Detalle").style.marginTop = '0rem'
                }
            }
            ...
            $timeout(function () {
                ...
                $scope.Btn_Gestionar_Glosa = X.ESTADO == 'P' ? true : false;
            }, 700);
        ...
        console.log($scope.HojaGest_Edi_Mod);
        console.log($scope.DetalleGestion);
    }
    ...
    $scope.vercamb = function (vercamb) {
        $scope.DetalleGestion = vercamb;
    }
    ...
    $scope.HojaGestion_Guardar = function () {
        if ($scope.HojaGlosa.Hoja == 1) {
                                ...
                                if (willDelete) {
                                    ...
                                    $http({
                                        ...
                                        data: {
                                            ...
                                            Num_Doc: $scope.HojaGest.DOCUMENTO, Ambito: $scope.HojaGest.AMBITO,
                                            ...
                                            // v_pproducto: $scope.HojaDet.Array[0].RENGLON,
                                        ...
                                        if (response.data) {
                                            if (response.data.Codigo == 0) {
                                                ...
                                                swal({
                                                    ...
                                                    text: 'Factura en espera de confirmacion',
                                                    type: "success",
                                                    // timer: 1500
                                                }).catch(swal.noop);
                                                $scope.Check_Detalle_Glosa = false;
            ...
            Materialize.toast('¡Por favor, Termine de gestionar el descuento!', 2000); $('.toast').addClass('default-background-dark');
        }
    }

    $scope.Hojagestionar_Descuento = function(){
        if ($scope.HojaGlosa.Array.length > 0) {
            if ($scope.HojaGest.ESTADO == 'P') {
                $http({
                    method: 'POST',
                    url: "php/pgp/auditoriascuentaspgp.php",
                    data: {
                        function: 'P_UI_GLOSA_PGP',
                        Ubi: $scope.HojaGest.UBICACION.toString(),
                        Num: $scope.HojaGest.NUMERO.toString(),
                        recibo: $scope.HojaGest.RECIBO, 
                        factura: $scope.HojaGest.FACTURA,
                        factura2: $scope.HojaGest.NUM_FACTURA, 
                        hora: $scope.HojaGest.FECHA_RADICADO,
                        tipo_doc: $scope.HojaGest.TIPO_DOCUMENTO,
                        afiliado: $scope.HojaGest.DOC_AFILIADO,
                        contrato_capitado: $scope.HojaGest.CONTRATO,
                        contrato: $scope.HojaGest.CONTRATO_ADM,
                        ubicacion_contrato:  $scope.HojaGest.UBICACION_CONTRATO,
                        proyecto: $scope.HojaGest.REGIMEN,
                        datos:  JSON.stringify($scope.HojaGlosa.Array),
                        cantGlosa:  $scope.HojaGlosa.Array.length,
                        tercero:  $scope.HojaGest.NIT,
                        observacion:  '',
                        Observacion_Glosa: '',
                        estado_fac: $scope.HojaGest.ESTADO,
                    }
                }).then(function (response) {
                    if (response.data) {
                        if (response.data.Codigo == 0) {
                            ...
                            swal({
                                title: "¡Acción realizada con exito!",
                                text: 'Descuento aplicado a la factura',
                                type: "success",
                                // timer: 1500
                            }).catch(swal.noop);
                            $scope.Check_Detalle_Glosa = false;
                ...
                Materialize.toast('¡La factura debe estar procesada para poder aplicar el descuento!', 2000); $('.toast').addClass('default-background-dark');
            }         
        }else{
            Materialize.toast('¡No tienes ningun descuento agregado!', 2000); $('.toast').addClass('default-background-dark');
        }
    }

    $scope.devolverDescuento = function(){
        console.log($scope.HojaGlosa.Array);
        if ($scope.HojaGlosa.Array) {
            swal({
                title: '¿Está seguro que desea retractar el descuento',
                type: "info",
                showCancelButton: true,
            }).catch(swal.noop)
                .then((willDelete) => {
                    if (willDelete) {
                        ...
                        $http({
                            ...
                            url: "php/pgp/auditoriascuentaspgp.php",
                            data: {
                                function: 'p_confirma_fac_com_gen',
                                 Num: $scope.Vista2_datos[0].NUMERO_FD.toString(),
                                 Ubi: $scope.Vista2_datos[0].UBICACION.toString(),
                                 empresa: 1,
                                 doc: "FD",
                                 accion: "R",
                            }
                            ...
                            if (response.data) {
                                if (response.data.Codigo == 0) {
                                    swal({
                                        title: "¡Acción realizada con exito!",
                                        text: 'Se retiro el valor del descuento',
                                        type: "success",
                                        // timer: 1500
                                    }).catch(swal.noop);
                    ...
                    if (response.data[0].Codigo != undefined) {
                        ...
                        $timeout(function () {
                            $scope.HojaDet.Array.forEach(i => {
                                ...
                                $scope.totalDescuento = $scope.totalDescuento + parseFloat(i.TOTAL);
    ...
    $scope.habilitarOpcion = true;
    $scope.HojaDetalle_CargarEditarDetalle = function (datos) {
        if (!$scope.habilitarOpcion) {
            $scope.tooltip = 'Seleccionar';
            $scope.HojaDet.Array = $scope.tempArray;
            $scope.habilitarOpcion = true;

            setTimeout(()=>{
                document.querySelectorAll(".iconoAsignar").forEach((ele => {
                    ele.classList.remove("icon-ccw-2");
                    ele.classList.add("icon-ok");
                }))
            },500);
            
        }else if($scope.habilitarOpcion){
            $scope.tooltip = 'Cancelar';
            $timeout(function () {
                datos.TOTAL = $scope.HojaGest.VALOR;
                $scope.HojaDet.Array = [];
                $scope.HojaDet.Array.push({DESCRIPCION: datos.DESCRIPCION, TOTAL: datos.TOTAL, RENGLON: datos.RENGLON, PRODUCTO: datos.PRODUCTO})
                // alert("------------------------REVISAR----------------------");
                // $scope.List_Select.Productos_Detalle.Filtro.push({ "CODIGO": datos.PRODUCTO, "NOMBRE": datos.DESCRIPCION.toUpperCase()})                                                                          
                // $scope.HojaDet.RENGLON = datos.RENGLON;
                // $scope.HojaDet.PRODUCTO = datos.PRODUCTO + ' - ' + datos.DESCRIPCION;
                // $scope.HojaDet.PRODUCTO_COD = datos.PRODUCTO;
                // $scope.HojaDet.AUT = datos.AUT;
                // $scope.HojaDet.TOTAL = $scope.FormatPesoNumero(datos.TOTAL);
                $scope.HojaDet.SUM_TOTAL = datos.TOTAL;
                $scope.List_Select.Productos_Detalle.SAVE = datos.PRODUCTO + ' - ' + datos.DESCRIPCION;
                $scope.List_Select.Productos_Glosa.Listado = [];
                $scope.List_Select.Productos_Glosa.Listado.push({ "CODIGO": datos.PRODUCTO, "NOMBRE": datos.DESCRIPCION.toUpperCase()})
                $scope.habilitarOpcion = false;
            }, 500);
            angular.forEach(document.querySelectorAll('.Hoja_Detalle_Class .red-text'), function (i) { // Limpia campos en rojo antes de buscar
                i.classList.remove('red-text');
            });
            setTimeout(()=>{
                document.querySelectorAll(".iconoAsignar").forEach((ele => {
                        ele.classList.remove("icon-ok");
                        ele.classList.add("icon-ccw-2");
                }))
            },500);
        }
    ...
    $scope.HojaDetalle_EliminarDetalle = function (row) { // Enviar peticion a bd para eliminar
        ...
        if ($scope.HojaDet.Array.length > 1) {
            ...
            if (Exist == 0) {
                ...
                swal({
                    title: 'Ocurrio un error',
                    text: '¡No es posible eliminar este detalle, debe eliminar primero el descuento asociado a este detalle!',
                    type: 'warning',
                }).catch(swal.noop);
            }
        }
    ...
    $scope.HojaGlosa_AgregarGlosa = function () { // Guardar el json pero no hacer bd
        ...
        if ($scope.HojaGlosa.TIPO_GLOSA == undefined || $scope.HojaGlosa.TIPO_GLOSA == null || $scope.HojaGlosa.TIPO_GLOSA == '') {
        ...
        // ****---- MODIFICADO ALEJO 10/07/2024 -----***
        if (Campos_Empty == false) {
            let paso = true;
            if ($scope.HojaGlosa.Array.length == 0) {
                    paso = true;
            }else if($scope.HojaGlosa.Array.length > 0){
                $scope.HojaGlosa.Array.forEach(ele =>{
                    if(ele.DESCRIPCION_GLOSA.trim().toString() == $scope.HojaGlosa.TIPO_GLOSA.split("-")[1].trim().toString()){
                        paso = false;
                    }
                })
            }
            if (paso) {
                    ...
                    if ($scope.HojaGlosa.SUM_TOTAL <= $scope.HojaGest.VALOR) {
                        ...
                        $scope.HojaGlosa.Array.push({
                            RENGLON: Renglon_Max + 1, DESCRIPCION_GLOSA: $scope.HojaGlosa.DESCRIPCION.toString().toUpperCase(), TIPO_GLOSA: $scope.HojaGlosa.TIPO_GLOSA_COD,
                            GLOSA: $scope.List_Select.TipoGlosas.SAVE.split('-')[1], VALOR_GLOSA: parseFloat(($scope.HojaGlosa.VALOR_GLOSA.toString().replace(/\./g, '')).replace(/\,/g, '.')),
                        ...
                        Materialize.toast('¡No es posible ingresar este detalle de descuento porque la sumatoria total excede $' + $scope.FormatPesoNumero($scope.Valores_Factura.HojaGlosa) + ' el valor de la Factura!', 5000); $('.toast').addClass('default-background-dark');
                    }
                }else{
                        document.querySelector('#Glosa_Tipo_Label').classList.add('red-text');
                        Materialize.toast('¡No es posible aplicar el mismo descuento dos veces!', 2000); $('.toast').addClass('default-background-dark');
                }
        } else {
            Materialize.toast('¡Complete todos los campos!', 2000); $('.toast').addClass('default-background-dark');
        }
            ...
            if ($scope.HojaGlosa.SUM_TOTAL + parseFloat(($scope.HojaGlosa.VALOR_GLOSA.toString().replace(/\./g, '')).replace(/\,/g, '.')) <= $scope.HojaGest.VALOR) {
                ...
                for (var i = 0; i < $scope.HojaGlosa.Array.length; i++) {
                    if ($scope.HojaGlosa.Array[i].RENGLON == renglon) {
                        ...
                        $scope.HojaGlosa.Array[i].DESCRIPCION_GLOSA = $scope.HojaGlosa.DESCRIPCION;
                        $scope.HojaGlosa.Array[i].GLOSA = $scope.HojaGlosa.PRODUCTO_COD;
                ...
                $scope.HojaGlosa.Hoja = 1;
                $scope.HojaGlosa.Edt_Act = 0;
                Materialize.toast('Descuento Actualizado!', 2000); $('.toast').addClass('default-background-dark');
            } else {
                Materialize.toast('¡No es posible ingresar este detalle de descuento porque la sumatoria total excede $' + $scope.FormatPesoNumero(($scope.HojaGlosa.SUM_TOTAL + parseFloat(($scope.HojaGlosa.VALOR_GLOSA.toString().replace(/\./g, '')).replace(/\,/g, '.'))) - $scope.HojaGest.VALOR) + ' el valor de la Factura!', 5000); $('.toast').addClass('default-background-dark');
            }
        } else {
            Materialize.toast('¡Complete todos los campos!', 2000); $('.toast').addClass('default-background-dark');
        }
    ...
    $scope.HojaGlosa_EliminarGlosa = function (row) {
        // HojaGlosa.Array
        swal({
            title: '¿Está seguro que desea eliminar el descuento?',
            type: "info",
            showCancelButton: true,
        }).catch(swal.noop)
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    Materialize.toast('Descuento Eliminado!', 2000); $('.toast').addClass('default-background-dark');
                }
            });
    }
    ...
    $scope.HojaGlosa_AgregarObservacion = function (text) {
        ...
        if (text != null) {
            swal({
                title: 'Observación del descuento',
                input: 'textarea',
    ...
    $scope.Reversar_Anular_Factura = function (Accion) {
        swal({
            title: "¿Está seguro que desea " + ((Accion == 'R') ? 'Reversar' : 'Anular') + " el descuento?",
            type: "question",
            showCancelButton: true,
            allowOutsideClick: false
        }).catch(swal.noop)
    ...
    $scope.BlurText = function (LIST, HOJA, MODEL) {
        $timeout(function () {
            if ($scope[HOJA]) {
                if ($scope[HOJA][MODEL] != null && $scope[HOJA][MODEL] != undefined && $scope[HOJA][MODEL] != "") {
            ...
            if (response.data) {
                if (response.data.length == 0 || response.data[0].Codigo != undefined) {
                        if (response.data.length == 0) {
                            ...
                            swal({
                                title: "¡No se encontraron descuentos!",
                                type: "info"
                            }).catch(swal.noop);
                        } else {
                            $scope.Array_ListadoGlosas = [];
                    ...
                    $scope.totalDescuento = 0;
                    $scope.Array_ListadoGlosas.forEach(ele => {
                        $scope.totalDescuento = $scope.totalDescuento + parseFloat(ele.VALOR_GLOSA);
                    })
