...
angular
  ...
  .controller("loginController", [
    ...
    function ($scope, $http, $location, ngDialog) {
            ...
            $http({
              ...
              if (data.Codigo == "0") {
                ...
                if (data.toString().substr(0, 3) == "<br") {
                  ...
                  swal("Notificación", data.Mensaje, "warning").catch(
                    swal.noop
                  );
                }
      ...
      $scope.Asignar_Ambiente = function (ambiente) {
          ...
          .then(function (result) {
            if (result != "") {
              ...
              $http({
                ...
                if (data.codigo == 0) {
                  swal("Notificación", data.mensaje, "success").catch(
                    swal.noop
                  );
                } else {
                  swal("Notificación", data.mensaje, "warning").catch(
      ...
      $scope.loginAfiliado = function () {
        if (
          ...
          swal(
            "Notificación",
            "Ingrese tipo, número de identificación y/o contraseña",
            "info"
          ).catch(swal.noop);
          return;
        }
        if ($scope.acceptTermsAfil == false) {
          swal(
            "Notificación",
            "Debe aceptar haber leido la carta de derecho y deberes",
            "info"
          ).catch(swal.noop);
          return;
        }
            ...
            swal({
              ...
              if (response_0 == true) {
                ...
                $http({
                  ...
                  if (
                    response_1.data.messages != undefined &&
                    response_1.data.messages.length > 0
                  ) {
                    swal({
                      title: "Mensaje enviado!",
                      text:
                        "Ingresa el codigo de verificacion enviado al celular del usuario: " +
                        $scope.userIps,
                      input: "number",
                      inputClass: "input-codigo",
                      inputValue: "",
                      ...
                      if (result > 0) {
                        ...
                        $http({
                          ...
                          if (response_1.data.Codigo == "0") {
                            swal(
                              "Importante",
                              response_1.data.Mensaje,
                              response_1.data.tipo
                            );
                          } else {
                            swal(
                              "Notificación",
      ...
      $scope.loginEmpresa = function () {
        if (
          ...
          swal("Notificación", "Ingrese nit y/o contraseña", "info").catch(
            swal.noop
          );
          return;
        }
