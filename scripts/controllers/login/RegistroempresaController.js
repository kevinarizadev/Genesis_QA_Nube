...
angular.module('GenesisApp')
  .controller('RegistroempresaController', ['$scope', '$http', 'notification', 'ngDialog',
    function ($scope, $http, notification, ngDialog) {
      ...
      $scope.validarform5 = function () {
        if ($scope.codigoradicado == '' || $scope.codigoradicado == undefined) {
          notification.getNotification('info', 'LLenar el formulario antes de seguir', 'Notificacion');
        } else {
          $scope.setTab(6);
        }
        ...
        $http({
          ...
          if (response.data.codigo == 0) {
            ...
            $http({
              ...
              swal.close();
              if (response.data.codigo == 0) {
                $scope.codigoradicado = response.data.formulario;
                $scope.setTab(6);
              } else {
                $scope.validarproceso = false;
      ...
      $scope.obtenerBaseformato = function () {
        if ($("#adjuntofor")[0].files[0].size > 7340032) {
          //swal('Advertencia','El archivo excede el peso limite (7 MB)','warning')
          notification.getNotification('warning', 'El archivo excede el peso limite (7 MB)', 'Notificación');
          $("#adjuntofor")[0].value = "";
          $scope.archivobaseform = "";
          $scope.extensionarchivoform = "";
        } else {
          if ($("#adjuntofor")[0].files && $("#adjuntofor")[0].files[0]) {
            var FR = new FileReader();
            FR.addEventListener("load", function (e) {
              $scope.archivobaseform = e.target.result;
              var name = $("#adjuntofor")[0].files[0].name;
              $scope.extensionarchivoform = name.split('.').pop();
            });
            FR.readAsDataURL($("#adjuntofor")[0].files[0]);
          }
        }
      }
      $scope.Finalizarsubida = function () {
        swal({
          title: 'Cargando información...',
          allowEscapeKey: false,
          allowOutsideClick: false
        ...
        $http({
          ...
          data: {
            ...
            archivobase64: $scope.archivobaseform,
            ext: $scope.extensionarchivoform,
            tipodoc: $scope.formulario.tidentificacion,
            numero: $scope.formulario.nidentificacion,
            codigo:$scope.codigoradicado,
            observacion:"",
          }
        }).then(function (response) {
          ...
          if (response.data.codigo == 0) {
            ...
            swal('Completado', 'proceso de registro Finalizado.', 'success');
            $scope.modalClose();
            $scope.codigoradicado="";
          } else {
            $scope.validarproceso = false;
      ...
      $scope.impriform = function () {
        window.open('views/movilidad/soporte/inforempresa.php?tipo=' + $scope.formulario.tidentificacion +
          '&id=' + $scope.formulario.nidentificacion, '_blank', "width=1080,height=1100");
      }
