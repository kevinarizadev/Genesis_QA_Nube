...
angular.module('GenesisApp')
    .controller('liquidacionEmpresaController', ['$scope', '$filter', 'consultaHTTP', 'notification', '$timeout', '$rootScope', '$http', '$window', 'ngDialog',
        function($scope, $filter, consultaHTTP, notification, $timeout, $rootScope, $http, $window, ngDialog) {
            ...
            $scope.bloquearinputduracion=false;
        ...
        $scope.diascotizados = "";
        $scope.diasgestacion = "";
        $scope.liquidacionorigen_radicacion = "PLATAFORMA";
            // <<<<<<<<<<<<<<<<<<<<<<<<< variables yordis liquidacion >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            ...
            $scope.guardar_devolucion = function() {
                ...
                if (subir_adjunto_devolcuiones == true) {
                    ...
                    $http({
                        ...
                        $scope.validarradicado($scope.liquidacion.ubicacion, $scope.liquidacion.numero);
                        if (response.data == 1) {
            ...
            $scope.diasdeduracion = function (tipoconcepto) {
                if(tipoconcepto =='LP'){
                 $scope.liquidacion.duracion =14;
                 $scope.bloquearinputduracion=true;
                }else if(tipoconcepto =='LM'){
                 $scope.liquidacion.duracion =126;
                 $scope.bloquearinputduracion=true;
                }else{
                 $scope.liquidacion.duracion ="";
                 $scope.bloquearinputduracion=false;
                }
               }
               $scope.validardiaslicenciaa = function(){
                $scope.buscar_afiliado('A');
                 console.log("aaaaaaaaaaaaaaaaa");
                 console.log($scope.liquidacion.concepto);
                 if($scope.liquidacion.concepto =='LA'){
                     if($scope.liquidacion.duracion > 28){
                         $scope.liquidacion.duracion ="";
                         $scope.bloquearinputduracion=false;
                         swal('Información', "La Duracion de Dias de Licencia Por Aborto no debe ser mayor a 28 dias", 'info');
                     }
                 }else{
                 }
               }
...
//  de nacimiento prematuro;   =< 36 semanas de gestacion
            ...
            $scope.mostrar_registro = function(concepto, numero, ubicacion, radicado, X) {
                var fechaprimerdia = new Date(2014, 1 - 1, 1);
                ...
                if (concepto == 0) {
                    ...
                    $scope.liquidacion.tercero_nombre = $scope.sesdata.nit+' - '+$scope.sesdata.nombre;
                    $scope.liquidacion.tercero = $scope.sesdata.nit;
                    swal('Información', "Estimado Aportante, Tenga en cuenta que para garantizar una adecuada radicación de su solicitud de reconocimiento de prestaciones económicas se recomienda utilizar el navegador Google Chrome", 'info');
                } else {
                    swal({
                        title: 'Cargando información...'
                    });
            ...
            $scope.limpiar_liquidacion = function() {
                $scope.liquidacion = {
                    tercero_nombre:$scope.liquidacion.tercero_nombre,
                    tercero:$scope.liquidacion.tercero,
                    concepto: '',
                    motivo: '',
                    fecha_incapacidad: '',
                    fecha_recibido: $scope.fecha,
            ...
            $scope.buscar_afiliado = function(tipo) {
                $scope.buscar_relacion_laboral();
                ...
                if ($scope.liquidacion.documento != '' && $scope.liquidacion.tipo_documento != '') {
                    $http({
                        ...
                        data: {
                            ...
                            fechainicio: tipo == 'A' ? "" : parsedia($scope.liquidacion.fecha_incapacidad),
                        }
                    }).then(function(response) {
                        if (response.data != "0") {
                            if(response.data.SexoCodigo == 'M' && ($scope.liquidacion.concepto == 'LM' || $scope.liquidacion.concepto == 'LA')){
                             swal('Genesis informa','No Puede realizar una Licencia de Maternidad a un Hombre','warning');
                                return;
                            }
                            if(response.data.SexoCodigo == 'F' && $scope.liquidacion.concepto == 'LP'){
                             swal('Genesis informa','No Puede realizar una Licencia de paternidad a una mujer','warning');
                                return;
                            }
                            ...
                            $scope.liquidacion.ubicacionafiliado = response.data.ubicacion;
                            ...
                            if(response.data.codigo == '2'){
                                ...
                                swal({title:"Mensaje", 
                                    html:'Para la fecha de inicio de la prestacion no registra una vinculacion laboral con el empleador <a target="_blank" href="https://genesis.cajacopieps.com/app.php#/movilidad/solicitud">Ingrese a Este Link Para su Registro</a>',
                                    type:"info"});
                            }
                        } else{
                            if(tipo == 'A'){
                            ...
                            // swal('Genesis informa','No existe relacion laboral entre el empleado y el empleador','warning');
                        }
                    });
                }
            }
            ...
            $scope.buscar_relacion_laboral = function() {
                ...
                $http({
                    ...
                    data: {
                        ...
                        documento: $scope.liquidacion.documento,
                        tipo_documento: $scope.liquidacion.tipo_documento,
                        nitdeempresa: $scope.sesdata.nit
                    }
                }).then(function(response) {
                    console.log(response);
                    if(response.data[0]){
                        $scope.liquidacion.tipo_tipo_cotizante = response.data[0].TIPO;
                    }
                    setTimeout(() => {
                        $scope.buscar_archivos();
                    }, 1000);
            ...
            $scope.obtenerBase = function(tipo, ind) {
                ...
                if ($("#adjunto" + ind + "_" + tipo)[0].files[0]) {
                    ...
                    var total_adjunto_Conta = $scope.listar_archivos["ARCHIVOS_CONTABLES"].length;
                    for (var i = 0; i < total_adjunto_Conta; i++) {
                       if($scope.listar_archivos["ARCHIVOS_CONTABLES"][i].codigo =='70' && ind == 0 && tipo =='AC'){
                        swal('Informacion', 'Cargar certificado bancario actualizado inferior a 30 días Decreto 2126 de 2023 art 2.2.3.4.1 de no ser así este será causal de devolución', 'info')
                       }
                    ...
                    // $scope.listar_archivos[ind].extensionarchivo = "";
                    // $scope.listar_archivos[ind].nombreadjunto = "";
                    if(tipo == 'AM'){
                        $scope.listar_archivos.ARCHIVOS_MEDICOS[ind].archivobase = '';
                        $scope.listar_archivos.ARCHIVOS_MEDICOS[ind].extensionarchivo = '';
                    }else{
                        $scope.listar_archivos.ARCHIVOS_CONTABLES[ind].archivobase = '';
                        $scope.listar_archivos.ARCHIVOS_CONTABLES[ind].extensionarchivo = '';
                    }
                ...
                if (subir_adjunto == true) {
                    ...
                    $http({
                        ...
                        data: {
                            ...
                            data: archivosasubir,
                            usuario: $scope.liquidacion.documento,
                            ubicacion: $scope.liquidacion.ubicacionafiliado,
                            numero: ""
                            // ubicacion: ubicacion,
                            // numero: numero
                        }
                        ...
                        if (response.data == 1) {
                            ...
                            if (response.data.codigo == 1) {
                                ...
                                var numeroradi = response.data.numero
                                $scope.insertar_radicado(numeroradi);
                                swal({ title: "Completado", text: response.data.mensaje, type: "success" });
                                // $scope.iniciar_componentes();
                                // $scope.tabla_activas('A');
                                // $scope.check_option_estado = true;
            ...
            $scope.validarradicado = function(ubicacion,numero) {
                $http({
                    method: 'POST',
                    url: "php/prestacionesempresas/liquidacion.php",
                    data: {
            ...
            //     });
            // }
            $scope.insertar_radicado = function(numerorad) {
                $http({
                    method: 'POST',
                    url: "php/prestacionesempresas/liquidacion.php",
                    data: {
                        function: 'p_insertar_prestacion',
                        numeroradicado: numerorad,
                        concepto: $scope.liquidacion.concepto,
                        motivo: $scope.liquidacion.motivo,
                        origen_radicacion: 'P',
                        fecha_incapacidad: $scope.liquidacion.fecha_incapacidad,
                        tipo_documento: $scope.liquidacion.tipo_documento,
                        documento: $scope.liquidacion.documento,
                        ubicacionafi: $scope.liquidacion.ubicacionafiliado,
                        duracion: $scope.liquidacion.duracion,
                        radicado: $scope.liquidacion.radicado,
                        ips: $scope.liquidacion.ips,
                        doc_medico: $scope.liquidacion.doc_medico,
                        ...
                        empleador: $scope.liquidacion.tercero,
                        fecha_accidente: $scope.liquidacion.fecha_accidente,
                        subir_adjunto: $scope.liquidacion.estado_radica_adjunto == true ? 'S': 'N',
                    ...
                    console.log(response.data);
                    if (response.data.codigo == 0) {
                        $scope.validarradicado(response.data.ubicacion, numerorad);
                        // if ($scope.liquidacion.estado_radica_adjunto == true) {
                        //     $scope.subirArchivos(response.data.ubicacion, response.data.numero);
                        // }
                        $window.open('views/aseguramiento/soporte/formato_radicacion.php?numero=' + response.data.numero + '&ubicacion=' + response.data.ubicacion, '_blank', "width=1080,height=1100");
