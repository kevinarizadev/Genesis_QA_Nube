"use strict";angular
  .module("GenesisApp")
  .controller("matrizdeseguimientoController", ["$scope","$http","$window","$filter",function ($scope, $http, $window, $filter) {
      // *********** FUNCTION DE INICIO *************
      $scope.Inicio = function () {
        ...
        $scope.show_vista_de_titulo_tabla = true;
      ...
      $scope.function_p_ui_bitacora_proyectos = function () {
        if($scope.form5.observacion ==  undefined || $scope.form5.observacion == ''){
          swal("Importante", "Complete los campos requeridos (*)", "info");
          let elementosAValidar = [
            {
              Codigo: "1",
              id_label: "id_form5_observacion_label",
              id_input: "id_form5_observacion",
            },
          ];
        ...
        $http({
          ...
          data: {
            ...
            vpcodigo: $scope.form5.codigoSeguimiento,
            vpobservacion: $scope.form5.observacion,
            vpaccion:'I',
          },
        }).then(function ({data}) {
          if (data && data.toString().substr(0, 3) != "<br") {
      ...
      $scope.function_volver = function (){
        $scope.function_consultaReporte('A');
        $scope.show_vista_de_titulo_tabla = true;
        $scope.show_tablaSeguimiento = true;
        $scope.show_tablaObservaciones = false;
      }
      $scope.function_p_lista_obs_proyecto = function (accion) {
        $scope.show_vista_de_titulo_tabla = false;
        $scope.show_tablaSeguimiento = false;
        $scope.show_tablaObservaciones = true;
        $scope.codigoConsulta = accion.CODIGO;
        $scope.list_DatosTemp = [];
        $http({
          method: "POST",
          url: "php/tic/matrizdeSeguimiento.php",
          ...
          if (data && data.toString().substr(0, 3) != "<br") {
            if(data.length == 0){
              swal({
                title: "INFORMACION",
                text: "El seguimiento no tiene minguna observacion",
                type: "info",
              }).catch(swal.noop);

              setTimeout(() => {
                $scope.function_consultaReporte($scope.estadodeReporte);
                $scope.show_vista_de_titulo_tabla = true;
                $scope.show_tablaSeguimiento = true;
                $scope.show_tablaObservaciones = false;
                $scope.$apply();
      ...
      $scope.openmodals = function (tipo,data) {
        ...
        switch (tipo) {
          ...
          case 'observacion':
            ...
            $scope.form5.nombreModulo = data.MODULO;
            $scope.form5.codigoSeguimiento = data.CODIGO;
          break;
          default:
        }
      }
