'use strict';
angular.module('GenesisApp').controller('bitacoraobjetosticController', ['$scope', '$http', '$timeout', '$filter', '$q', function ($scope, $http, $timeout, $filter, $q) {
  // Plantilla funcional
  console.clear();
  ...
  $scope.Obtener_Areas = function () {
    ...
    $http({
      method: 'POST',
      url: "php/tic/bitacoraobjetostic.php",
      data: {
        function: 'Obt_Area'
      }
  ...
  $scope.loadFile = function (SOPORTE, SCOPE, B64, NID, NIDT) {
    ...
    if (ValidarExistente != true) {
      if (fileInput.files.length != 0) {
        ...
        if (x[x.length - 1].toUpperCase() == 'PDF' || x[x.length - 1].toUpperCase() == 'DOCX' || x[x.length - 1].toUpperCase() == 'PNG' || x[x.length - 1].toUpperCase() == 'JPG') {
          if (fileInput.files.length > 0) {
          ...
          swal('Advertencia', '¡El archivo seleccionado deber ser formato PDF, WORD, JPG, PNG!', 'info');
          fileInput.value = '';
          document.querySelector('#' + NIDT).value = '';
          $scope.Form[SOPORTE][SCOPE] = '';
          $scope.Form[SOPORTE][B64] = '';
  ...
  $scope.Abrir_Modal_Soportes = function (ruta) {
    ...
    $http({
      ...
      if ((ruta.split('.')[1]).toUpperCase() == 'PDF' || (ruta.split('.')[1]).toUpperCase() == 'JPG' || (ruta.split('.')[1]).toUpperCase() == 'PNG')
