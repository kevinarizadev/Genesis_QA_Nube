    ...
    function ($scope, $http) {
      ...
      $scope.Obtener_Horas_Disponibilidad = function () {
        ...
        $http({
          ...
          if (data.toString().substr(0, 3) != '<br') {
            // var disponibles = data.data[0].DISPONIBILIDAD;--controlyordis
            var disponibles = 7;
      ...
      $scope.cargarSoportes = function (numeroProyecto) {
        return new Promise((resolve) => {
          // if ($scope.archivosSubir.length == 0) {controlyordis
          //   resolve(true); return
          // }
          swal({
