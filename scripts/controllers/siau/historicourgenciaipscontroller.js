...
.controller('historicourgenciaipscontroller',['$scope','notification','cfpLoadingBar','$http',function($scope,notification,cfpLoadingBar,$http) {
  ...
  $scope.obtenerhistorico = function(){
    if($scope.filtro != "" && $scope.filtro != null && $scope.filtro != undefined){
      $http({
        ...
        if(response.data[0][0].Codigo != "0"){
          $scope.HistoricoUrgencia = response.data[0];
          $scope.Nombre = $scope.HistoricoUrgencia[0].Nombre_afiliado;
