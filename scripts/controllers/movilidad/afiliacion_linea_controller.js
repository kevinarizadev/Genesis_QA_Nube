...
angular.module('GenesisApp').controller('afiliacion_linea_controller', ['$scope', '$http', 'consultaHTTP', 'notification', 'ngDialog', '$filter', function ($scope, $http, consultaHTTP, notification, ngDialog, $filter) {
  $(document).ready(function () {
    ...
    $scope.versubirsoportes = false;
    $scope.nombrearchivo1='';
    document.getElementById('anexo2adj1').value = "";
    document.getElementById('textadj1').value = "";
    $('.tabs').tabs();
    ...
    if (solicitud.RADICADO == $scope.historial.detalle) {
      ...
      $http({
        ...
        if (validar_json(angular.toJson(response.data)) && response.data.datos_basicos != null) {
            ...
            $scope.editarafiestadodeafiliacion=response.data.datos_basicos.ESTADO;
            $scope.editarafiobservaciondev=response.data.datos_basicos.SMVC_OBSERVACION_DEVOL;
            $scope.editarafiliacion = false;
          $scope.editarafi.numeroderadicado=solicitud.RADICADO;
          $scope.numeroderadicadoarchivo=solicitud.RADICADO;
  ...
  // @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@fin control de cambio Yordis@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
      ...
      $scope.cargarbeneficiario = function () {
        ...
        $scope.editarafi.listado_beneficiariosactualizar[j].documento_new=$scope.editarbene.ben_documento;
        ...
        $scope.editarafi.listado_beneficiariosactualizar[j].telefono = $scope.editarbene.telefono == null ? "" : $scope.editarbene.telefono == NaN ? "" : $scope.editarbene.telefono.toString();
          ...
          if (result) {
            ...
            $http({
              ...
              data: {
                ...
                numero_portante: $scope.editarafi.numero_portante,
                ...
                telef: $scope.editarafi.telefono_f_cotizante,
      ...
      $scope.validararchivo1 = function (){
        $scope.archivocargar1="";
        $scope.extencioarchivo="";
      ...
      $scope.changeItem = function (item) {
        $scope.itemafi =[];
        if (item) {
          $scope.itemafi = JSON.parse(item);
        }
      };
      $scope.uploadedFiles = [];
      $scope.uploadFile = function() {
        
        if ($scope.nombrearchivo1) {
          var fileObject = {
            tipo_documento:$scope.itemafi.tipodoc,
            numero_doc:$scope.itemafi.numerodoc,
            nombreafi:$scope.itemafi.nombres,
            parentezco:$scope.itemafi.parentesco,
            nombrearchivo: $scope.nombrearchivo1.name,
            tipo: $scope.selectedDocumentType,
            file: $scope.nombrearchivo1,
            base64:$scope.archivocargar1,
            extension:$scope.extencioarchivo
          };
          $scope.uploadedFiles.push(fileObject);
          $scope.versubirsoportes = true;
          $scope.nombrearchivo1='';
          console.log($scope.uploadedFiles);
        }
      };

      $scope.eliminarsoportetemp= function(datosfile){
        for (let i = 0; i < $scope.uploadedFiles.length; i++) {
          if ($scope.uploadedFiles[i].tipo === datosfile.tipo && $scope.uploadedFiles[i].numero_doc ===datosfile.numero_doc) {
      ...
      $scope.subirdocumentos = function(){
        // swal({
        //   title: '¿Desea Subir los soportes?',
        //   type: 'question',
        //   showCancelButton: true,
        //   confirmButtonColor: '#3085d6',
        //   cancelButtonColor: '#d33',
        ...
        //   if (result) {
            ...
            $http({
              ...
              data: {
                ...
                soportes: JSON.stringify($scope.uploadedFiles),
                cantidad:$scope.uploadedFiles.length,
                codigo_solicitud:$scope.numeroderadicadoarchivo,
                // codigo_solicitud:$scope.editarafi.numeroderadicado,
              }
            }).then(function (response) {
              swal.close();
              $scope.historial.detalle = -1;
              $scope.uploadedFiles = [];
              $scope.valorlistadocoti_bene = "";
              $scope.nombrearchivo1='';
              document.getElementById('anexo2adj1').value = "";
              document.getElementById('textadj1').value = "";
              $scope.versubirsoportes = false;
              $scope.numeroderadicadoarchivo="";
              if(response.data == '0'){
                // swal('Exito', 'Archivos Cargados Correctamente', 'success');
              }else{
                // swal('Error', 'No se Lograron Cargar los Archivos', 'error');
              }
      ...
      $scope.eliminararchivo = function(datosruta){
        swal({
          title: '¿Desea Eliminar este soporte?',
          type: 'question',
          showCancelButton: true,
          confirmButtonColor: '#3085d6',
          cancelButtonColor: '#d33',
          ...
          if (result) {
            ...
            $http({
              ...
              data: {
                ...
                v_pcodigo: datosruta.CODIGO,
                vp_tipo_adj: datosruta.TIPO_ADJ,
                rutasoporte: datosruta.RUTA,
              }
            }).then(function (response) {
              swal.close();
              $scope.nombrearchivo1='';
              ...
              document.getElementById('textadj1').value = "";
              $scope.historial.detalle = -1;
              for (let i = 0; i < $scope.historial.listado_soportes.length; i++) {
                if ($scope.historial.listado_soportes[i].RUTA === datosruta.RUTA) {
                    $scope.historial.listado_soportes.splice(i, 1);
                    i--;
                }
            }
            //   $scope.historial.listado_soportes = $scope.historial.listado_soportes.filter(function(item) {
            //     return item.CODIGO !== datosruta.CODIGO
            // })
            });
          }
        }).catch(swal.noop);
  ...
  $scope.afiliacion = {
    ...
    tipo_documento_beneficiario: "",
    numero_documento_beneficiario: "",
    tipo_documento_beneficiario_CN: "CN",
    numero_documento_beneficiario_CN: "",
    primer_nombre_beneficiario: "",
    segundo_nombre_beneficiario: "",
    primer_apellido_beneficiario: "",
    segundo_apellido_beneficiario: "",
  ...
  $scope.listar_tipo_doc_cargar = function () {
    $http({
      method: 'POST',
      url: "php/movilidad/afiliacion_linea.php",
      data: {
      ...
      if (validar_json(angular.toJson(response.data))) {
        $scope.listadotipo_doc_cargar = response.data;
      } else {
        $scope.listadotipo_doc_cargar = new Array();
      }
    });
  }
  $scope.listar_tipo_doc_cargar();
  ...
  $scope.buscar_aportante = function () {
    if ($scope.afiliacion.tipo_documento_aportante_v && $scope.afiliacion.numero_documento_aportante_v){
      var x = $scope.Tipos_Documentos2.find(e => e.CODIGO == $scope.afiliacion.tipo_documento_aportante_v);
      if (x != -1){
        if( $scope.afiliacion.numero_documento_aportante_v.toString().length < x.VMIN || $scope.afiliacion.numero_documento_aportante_v.toString().length > x.VMAX ){
  ...
  $scope.buscar_cotizante = function () {
    if ($scope.afiliacion.tipo_documento && $scope.afiliacion.numero_documento){
      var x = $scope.Tipos_Documentos.find(e => e.CODIGO == $scope.afiliacion.tipo_documento);
      if (x != -1){
        if( $scope.afiliacion.numero_documento.toString().length < x.VMIN || $scope.afiliacion.numero_documento.toString().length > x.VMAX ){
  ...
  $scope.buscar_beneficiario = function () {
    if ($scope.afiliacion.tipo_documento_beneficiario && $scope.afiliacion.numero_documento_beneficiario){
      var x = $scope.Tipos_Documentos.find(e => e.CODIGO == $scope.afiliacion.tipo_documento_beneficiario);
      if (x != -1){
        if( $scope.afiliacion.numero_documento_beneficiario.toString().length < x.VMIN || $scope.afiliacion.numero_documento_beneficiario.toString().length > x.VMAX ){
                                    ...
                                    if ($scope.afiliacion.parentesco != "" && $scope.afiliacion.parentesco != undefined && $scope.afiliacion.parentesco != null) {
                                      ...
                                      if (i == -1) {
                                        if ($scope.afiliacion.numero_documento != $scope.afiliacion.numero_documento_beneficiario) {
                                          if ($scope.afiliacion.numero_documento_aportante_v != $scope.afiliacion.numero_documento_beneficiario) {
                                            //
                                            if ($scope.afiliacion.tipo_documento_beneficiario != 'RC') {
                                              $scope.incluir_beneficiario_paso2();
                                            } else {
                                              if ($scope.afiliacion.numero_documento_beneficiario_CN == null || $scope.afiliacion.numero_documento_beneficiario_CN == '') {
                                                swal('Advertencia', 'Debe Digitar el Numero de Nacido Vivo', 'warning');
                                              } else if($scope.afiliacion.eps_anterior.slice(0, 6) == '000000'){
  ...
  $scope.incluir_beneficiario_paso2 = function () {
    ...
    if (parentesco) {
      ...
      $scope.afiliacion.listado_beneficiarios.push({
        ...
        tipo_documento_CN: $scope.afiliacion.tipo_documento_beneficiario_CN,
        documento_CN: $scope.afiliacion.numero_documento_beneficiario_CN,
      });
    ...
    return new Promise((resolve) => {
      $http({
        ...
        data: {
          ...
          tipo: $scope.afiliacion.tipo_documento_beneficiario,
          numero: $scope.afiliacion.numero_documento_beneficiario,
          tipo_CN: $scope.afiliacion.tipo_documento_beneficiario == 'RC' ? $scope.afiliacion.tipo_documento_beneficiario_CN : '',
          numero_CN: $scope.afiliacion.tipo_documento_beneficiario == 'RC' ? $scope.afiliacion.numero_documento_beneficiario_CN : ''
        }
      }).then(function ({ data }) {
        // if (data[0].codigo == '0') {
        // } else {
        //   $scope.afiliacion.tipo_documento_beneficiario = '';
        //   $scope.afiliacion.numero_documento_beneficiario = '';
        //   $scope.afiliacion.tipo_documento_beneficiario_CN = '';
        //   $scope.afiliacion.numero_documento_beneficiario_CN = '';
        //   swal('Notificacion', data[0].mensaje, 'warning');
        // }
        resolve(data)//
      });
    });
  ...
  $scope.subir_formato = function () {
    ...
    if ($scope.input_data_default.formato.files != undefined && $scope.input_data_default.formato.files != null &&
      ...
      $scope.obtener_base_64($scope.input_data_default.formato.files).then(function (base64) {
        ...
        $scope.obtener_url_ftp(base64, fecha.getTime(), extencion[extencion.length - 1],
  ...
  $scope.validarLongitudTipoDoc_Ben = function () {
    if ($scope.afiliacion.tipo_documento_beneficiario && $scope.afiliacion.numero_documento_beneficiario){
      var x = $scope.Tipos_Documentos.find(e => e.CODIGO == $scope.afiliacion.tipo_documento_beneficiario);
      if (x != -1){
        if( $scope.afiliacion.numero_documento_beneficiario.toString().length < x.VMIN || $scope.afiliacion.numero_documento_beneficiario.toString().length > x.VMAX ){
  ...
  $scope.validarLongitudTipoDoc_Ben_CN = function () {
    if (!$scope.afiliacion.numero_documento_beneficiario_CN){return;}
    if ($scope.afiliacion.numero_documento_beneficiario_CN.toString().length < 8 || $scope.afiliacion.numero_documento_beneficiario_CN.toString().length > 14) {
      $scope.afiliacion.numero_documento_beneficiario_CN = '';
      const mensaje = "Revise la longitud del numero del documento CN. Min: " + 8 + ", Max:" + 14;
      swal("¡Mensaje!", mensaje, "info").catch(swal.noop);
      setTimeout(() => { $scope.$apply(); }, 500);
    }
  }
  ...
  $scope.validarLongitudTipoDoc_2 = function () {
    if ($scope.afiliacion.tipo_documento_aportante_v && $scope.afiliacion.numero_documento_aportante_v){
      var x = $scope.Tipos_Documentos2.find(e => e.CODIGO == $scope.afiliacion.tipo_documento_aportante_v);
      if (x != -1){
        if( $scope.afiliacion.numero_documento_aportante_v.toString().length < x.VMIN || $scope.afiliacion.numero_documento_aportante_v.toString().length > x.VMAX ){
  ...
  $scope.Event_key = function (keyEvent) {
    if (keyEvent.which === 9) {
      document.querySelector("#afiliacion_primer_nombre_beneficiario").focus();
      setTimeout(() => { document.getElementById("afiliacion_primer_nombre_beneficiario").focus(); }, 500);
      // document.querySelector("#afiliacion_primer_nombre_beneficiario").focus();

      // document.getElementById("afiliacion_primer_nombre_beneficiario").focus();
    }
  }
}]);
