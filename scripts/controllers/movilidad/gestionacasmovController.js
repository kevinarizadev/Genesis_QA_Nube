...
.controller('gestionacasmovController', ['$scope', '$http', 'ngDialog', 'notification', '$timeout', '$q', 'upload', 'communication', '$controller','$rootScope','$window',
function ($scope, $http, ngDialog, notification, $timeout, $q, upload, communication, $controller, $rootScope, $window) {
      ...
      $scope.buscarAcas = function (texto) {
        ...
        $http({
          method: 'POST',
          url: "php/aseguramiento/gestionaseguramiento.php",
          data: { function: 'obtenerAcas', keyword: texto, area: 'M' }
        }).then(function (response) {
          $scope.listacas = response.data;
          $scope.panel = false;
          $scope.tipo = true;
