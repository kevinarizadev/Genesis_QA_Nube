...
.controller('IncumplimientopagoController', ['$scope', '$rootScope', '$http', '$filter', '$window', function ($scope, $rootScope, $http,$filter,$window) {
	...
	$scope.PrintCarta = function (x,estado) {
		if (x.tipo_doc_aportante == 'N') {
			$window.open('views/cartera/print/avisomora.html?estado='+estado+'&tipo='+x.tipo_doc_aportante+'&id='+x.doc_aportante+'&data='+x.fecha_limite,'_blank', "width=1080,height=1100");
		} else {
			$window.open('views/cartera/print/avisomoraindepe.html?estado='+estado+'&tipo='+x.tipo_doc_aportante+'&id='+x.doc_aportante+'&data='+x.fecha_limite,'_blank', "width=1080,height=1100");
		}
	}
	...
	$scope.ListarConsolidadoIncumplimiento = function (estado) {
		...
		$http({
			...
			data: { function: 'ConsolidadoAIncumplimiento' }
	...
	$scope.VerDetalle = function (info) {
		...
		$http({
			method: 'POST',
			url: "php/cartera/funcartera.php",
			data: { function: 'DetalleIncumplimientoDetalle',documento:info.doc_aportante }
		}).then(function (response) {
			if (response.data.length>0){					
