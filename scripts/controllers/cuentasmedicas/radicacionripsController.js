        ...
        function ($scope, notification, consultaHTTP, upload, afiliacionHttp, $http) {
            ...
            $scope.loadfunction = function () {
                $http({
                    ...
                    data: { function: 'obtenerUbicacionHab', nit: $scope.nit, tipo: 'I'}
            ...
            $scope.validarLegalizacion = function(dato){
                $scope.listcontratos.forEach((elem)=>{
                            if (elem.CODIGO == dato) {
                                $scope.legalizacion = elem.LEGALIZADO;
                            }
                })
                if ($scope.legalizacion == 'N') {
                    swal({
                            title: 'IMPORTANTE',
                            html: `Su contrato está pendiente de soportes documentales, <b>ACERCARSE A LA OFICINA DE CONTRATACIÓN.</b><br> Su cooperación es importante para la oportuna gestión de su factura`,
                            type: 'warning',
