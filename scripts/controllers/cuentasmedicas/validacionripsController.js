...
angular.module('GenesisApp')
    .controller('validacionripsController', ['$scope', 'notification', 'consultaHTTP', 'upload', 'afiliacionHttp', '$http',
        function ($scope, notification, consultaHTTP, upload, afiliacionHttp, $http) {
            ...
            $scope.limpiarSolicitud = function(){
                $scope.paneldx = true;
                $scope.activedx = 'active';
                $scope.activead = 'none';
                $scope.inactivepaso2 = true;
            ...
            $scope.ObtenerPrestadores = function (prestador) {
                $scope.numeroNit = "";
                if (prestador) {
                    if (prestador.length > 2) {
                        $http({
                            ...
                            data: { function: 'p_obtener_ips', nit: prestador }
                        }).then(function ({data}) {
                            $scope.listaips = data;
                            if ($scope.listaips.length == 1) {
                                $scope.numeroNit = data.CODIGO;                      
                                $scope.selectPrestador(prestador);
                                $scope.inactiveRips = true;
                            }
                   
                        })
                    }
                }else{
                    notification.getNotification('warning', 'Debe diligenciar el Nit o Razon Social', 'Notificacion');
                }
            }

            $scope.selectPrestador = function(nombre) {
                var selectedPrestador = $scope.listaips.find(function(ips) {
                    return ips.RAZON === nombre || ips.CODIGO == nombre;
                });
                if (selectedPrestador) {
                    $scope.numeroNit = selectedPrestador.CODIGO;                      
                    $scope.loadfunction(selectedPrestador.CODIGO);
                }
            };

            document.addEventListener("input", e => {
                if (e.target.matches("#prestadorId")) {
            ...
            $scope.loadfunction = function (nit) { 
                $http({
                    method: 'POST',
                    url: "php/cuentasmedicas/rips/funcRips.php",
                    data: { function: 'obtenerUbicacionHab', nit, tipo: 'E' }
            ...
            $scope.ObtenerContratos = function () {         
                $http({
                    method: 'POST',
                    url: "php/cuentasmedicas/rips/funcRips.php",
                    data: { function: 'obtenerContratos', nit: $scope.numeroNit, tipo: $scope.rip.tipocontrato_Sel }
                }).then(function (response) {
                    $scope.listcontratos = [];
                    if (!response.data.hasOwnProperty('Codigo')) {
            ...
            $scope.obtenerRegimen = function (contrato) {
                ...
                $http({
                    method: 'POST',
                    url: "php/cuentasmedicas/rips/funcRips.php",
                    data: { function: 'obtenerRegimen', nit: $scope.numeroNit, contrato: contrato }
                }).then(function (response) {
                    setTimeout(function () {
                ...
                $http({
                    method: 'POST',
                    url: "php/cuentasmedicas/rips/funcRips.php",
                    data: { function: 'obtenerYear', nit: $scope.numeroNit, contrato: contrato }
                }).then(function (response) {
                    $scope.listanno = [];
                    if (!response.data.hasOwnProperty('Codigo')) {
                        $scope.listanno = response.data;
            ...
            $scope.obtenerModalidad = function () {
                $http({
                    method: 'POST',
                    url: "php/cuentasmedicas/rips/funcRips.php",
                    data: { function: 'obtenerModalidad', nit: $scope.numeroNit, contrato: $scope.rip.contrato }
                }).then(function (response) {
                    $scope.listModalidad = [];
            ...
            $scope.cargarMes = function (anno) {
                $http({
                    method: 'POST',
                    url: "php/cuentasmedicas/rips/funcRips.php",
                    data: { function: 'obtenerPeriodo', anno: anno, nit: $scope.numeroNit, contrato: $scope.rip.contrato }
                }).then(function (response) {
                    $scope.listmes = [];
                    if (!response.data.hasOwnProperty('Codigo')) {
            ...
            $scope.archivoscargue = function () {
                $http({
                    method: 'POST',
                    url: "php/cuentasmedicas/rips/funcRips.php",
                    data: { function: 'obtenerArchivos', nit: $scope.numeroNit }
                }).then(function (response) {
                ...
                if ($scope.ctlleno == true) {
                    ...
                    if (continuar == true) {
                        ...
                        $http({
                            method: 'POST',
                            url: "php/cuentasmedicas/rips/funcRips.php",
                            data: { function: 'subirAdjuntoValidacion', archivos: archivos, nit: $scope.numeroNit, recibo: $scope.rip.recibo, val_aut: $scope.rip.val_aut }
                        }).then(function (response) {
                            if (response.data.codigo == 0) {
                                ...
                                if (sessionStorage.getItem('cedula') != "") {
                                    $http({
                                        method: 'POST',
                                        url: "php/cuentasmedicas/rips/funcRips.php",
                                        data: { function: 'guardarSolicitud', archivos: archivos, datos: datos, nit: $scope.numeroNit, responsable: sessionStorage.getItem('cedula'), cantidad: $scope.listct.length }
                                    }).then(function (response) {
                                        if (response.data["0"].Codigo == '0') {
                ...
                if ($scope.ctlleno == true) {
                    ...
                    if (continuar == true) {
                        ...
                        $http({
                            method: 'POST',
                            url: "php/cuentasmedicas/rips/funcRips.php",
                            data: { function: 'subirAdjunto', archivos: archivos, nit: $scope.numeroNit, recibo: $scope.rip.recibo }
                        }).then(function (response) {
                            if (response.data != '0') {
