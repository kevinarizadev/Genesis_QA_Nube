...
angular.module('GenesisApp').controller('auditoriacuentasController', ['$scope', '$http', '$timeout', '$filter', 'ngDialog', 'notify', function ($scope, $http, $timeout, $filter, ngDialog, notify) {
  ...
  $scope.Inicio = function () {
    ...
    $http({
      ...
      $http({
        ...
        $scope.obtenerListadoFacturasOCR(1)
      });
      //////////////////////
    });
    /////////////////////////////////////////////
    ...
    if ($scope.HojaGlosa.Hoja == 1) {
      ...
      if (Campos_Empty == false) {
        if (((parseFloat(($scope.HojaGest.COPAGO.toString().replace(/\./g, '')).replace(/\,/g, '.'))) <= ($scope.HojaGest.VALOR * 0.1) && $scope.HojaGest.REGIMEN == '0')
          ...
          const msg = `¡El valor del copago no puede exceder el ${$scope.HojaGest.REGIMEN == '0' ? '10' : '11.5'}% del valor de la Factura!`;
          Materialize.toast(msg, 2000); $('.toast').addClass('default-background-dark');
        }
      } else {
  ...
  $scope.Consulta_Censos = function (tipo, doc, nit, recibo, factura) {
    $scope.Listar_censos = {
      Array: []
    };
    swal({ title: 'Cargando...', allowOutsideClick: false });
    ...
    $http({
      ...
      url: "php/cuentasmedicas/auditoriacuentas.php",
      data: {
        function: 'Consulta_Censos', tipodocumento: tipo, documento: doc, nit, recibo, factura
      }
    }).then(function (response) {
      if (response.data != '') {
        swal.close();
  ...
  $scope.obtenerListadoFacturasOCR = function (x) {
    ...
    $http({
      ...
      if (!x) swal.close();
      if ((data.toString().substr(0, 3) == '<br' || data == 0) && !x) {
  ...
  $scope.descargarListadoFacturasOCR = function () {
    var data = [];

    $scope.modalOCR.listadoFacturas.forEach(e => {
      data.push({
        "PRESTADOR": e.PRESTADOR,
        "RECIBO": e.RECIBO,
        "FACTURA": e.FACTURA,
        "VALOR_FACTURA": e.VALOR_FACTURA,
        "FECHA_RADICACION": e.OCR_FECHA_REGISTRO,
        "DIAS_RESTANTES": e.DIAS_REST,
    ...
    XLSX.writeFile(wb, "Exportado Facturas Digital.xlsx");
    const text = `Facturas ${data.length}`
    swal('¡Completado!', text, 'success').catch(swal.noop);
  }
  ...
  $scope.verFacturaPDF = function (x) {
    ...
    if ($scope.listadoFacturasPDF.length) {
      return
    }
  ...
  $scope.descargarMasivoFacturasPDF = function () {
    // $scope.listadoFacturasPDF_Original.forEach(element => {

    // });
    swal({
      html: '<div class="loading"><div class="default-background"></div><div class="default-background"></div><div class="default-background"></div></div><p style="font-weight: bold;">Descargando facturas...</p>',
      width: 200,
      allowOutsideClick: false,
      allowEscapeKey: false,
      showConfirmButton: false,
    ...
    $http({
      ...
      url: "php/cuentasmedicas/auditoriacuentas.php",
      data: {
        function: "descargarMasivoFacturasPDF",
        nit: $scope.HojaGest.NIT,
        factura: $scope.HojaGest.FACTURA,
        datos: JSON.stringify($scope.listadoFacturasPDF_Original)
      }
    }).then(function ({ data }) {
      swal.close();
      if (data) {
