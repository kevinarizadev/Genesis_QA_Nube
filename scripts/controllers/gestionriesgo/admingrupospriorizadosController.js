  ...
  .controller('admingrupospriorizadosController', ['$scope', '$http', '$filter', '$q', 'afiliacionHttp',
    function ($scope, $http, $filter, $q, afiliacionHttp) {
      $(document).ready(function () {
        ...
        // $scope.Ver_Glosas();
        $scope.Vista = 0;
        $scope.data_modal_gestion_permisos = {};
      ...
      $scope.modal_gestion_permisos = function (usuario) {
        let user = {};
        for (let key in usuario) {
          user[key] = usuario[key];
        }
        $scope.data_modal_gestion_permisos = user;
        const atributosBooleanos = ["VISUALIZAR", "EXPORTAR", "INCLUIR_FUNC", "CONFIRMAR_DX", "INDICADORES"];

        atributosBooleanos.forEach(atributo => {
          $scope.data_modal_gestion_permisos[atributo] = $scope.data_modal_gestion_permisos[atributo] == "S" ? true : false;
        });
        $('#Modal_Gestion_Permisos').modal('open');
      }

      $scope.agingar_permisos_acgs = function () {
        let user = {};
        for (let key in $scope.data_modal_gestion_permisos) {
          user[key] = $scope.data_modal_gestion_permisos[key];
        }
        const atributosBooleanos = ["VISUALIZAR", "EXPORTAR", "INCLUIR_FUNC", "CONFIRMAR_DX", "INDICADORES"];
        atributosBooleanos.forEach(atributo => {
          user[atributo] = user[atributo] == true ? "S" : "N";
        });
        let json = {
          "Documento": user.CODIGO,
          "Visualizar": user.VISUALIZAR,
          "Exportar": user.EXPORTAR,
          "IncluirFunc": user.INCLUIR_FUNC,
          "ConfimarDx": user.CONFIRMAR_DX,
        ...
        $http({
          ...
          if (data && data.toString().substr(0, 3) != '<br') {
            $scope.MPFiltrar_Func = "";
            $scope.filter2("");
            if (data[0].Codigo == 0) {
              $('#Modal_Gestion_Permisos').modal('close');
              swal({
                title: "¡Notificacion!",
                text: data[0].Nombre,
                type: "success",
