...
angular.module('GenesisApp')
   .controller('verfosyga', ['$scope', 'afiliacionHttp', '$http', 'notification', '$location', '$rootScope', '$timeout', 'communication', '$localStorage',
      function ($scope, afiliacionHttp, $http, notification, $location, $rootScope, $timeout, communication, $localStorage) {
            ...
            if (result) {
               ...
               afiliacionHttp.serviceFDC($scope.adresdata_tipo_doc,$scope.adresdata_documento , 'ObtenerFosyga').then(function(response) {
                  ...
                  if ($scope.adres_result && $scope.adres_result.IdRespuesta == 0) {
                     ...
                     //Llena nombres
                     $scope.adres_p_nombre = $scope.adres_result.PrimerNombre;
                     $scope.adres_s_nombre = $scope.adres_result.SegundoNombre;
                     // Llena apellidos
                     $scope.adres_p_apellido = $scope.adres_result.PrimerApellido;
                     $scope.adres_s_apellido = $scope.adres_result.SegundoApellido;
                     // Filtra municipio
                     ...
                     // Establecer entidad
                     if ($scope.adres_result.EPS == "CAJA DE COMPENSACION FAMILIAR  CAJACOPI ATLANTICO" || $scope.adres_result.EPS == "CAJA DE COMPENSACIÓN FAMILIARCAJACOPI ATLÁNTICO" ) {
                        $scope.filtroentidad = 'CAJACOPI';
                     /*}else if ($scope.adres_result.EPS.length=="CAJA DE COMPENSACION FAMILIAR DE CORDOBA - COMFACOR".length){
                       $scope.filtroentidad = 'COMFACOR'; 
                     }*/
                     }else if ($scope.adres_result.EPS.indexOf("COMFACOR") > -1) {
                       $scope.filtroentidad = 'COMFACOR'; 
                     }
                     else{
                        $scope.filtroentidad = $scope.adres_result.EPS; 
                     }
                     // var a = "CAJA DE COMPENSACION FAMILIAR  CAJACOPI ATLANTICO".match(/CAJACOPI.*/);
                     // var b = $scope.adres_result.EPS.match(/CAJACOPI.*/);
                     // if (a[0]==b[0]){
                     // // if ($scope.adres_result.EPS == 'CAJA DE COMPENSACION FAMILIAR  CAJACOPI ATLANTICO'){
                     //    $scope.filtroentidad = 'CAJACOPI';
                     // }else if ($scope.adres_result.EPS == 'CAJA DE COMPENSACION FAMILIAR DE CORDOBA - COMFACOR'){
                     //    $scope.filtroentidad = 'COMFACOR'
                     // }else{
                     //    $scope.filtroentidad = $scope.adres_result.EPS;
                     // }
                     ...
                     if ($scope.adres_result.TipoAfiliado == "ADICIONAL" ) {
         ...
         $scope.guardar = function(existe){
            //if (existe == true || existe === undefined) {
               if ($scope.consulta_beneficiario === undefined || $scope.consulta_beneficiario == false) {
                  $scope.adres_tipo_doc = $scope.filtro.selectdocumento;
                  $scope.adres_documento = $scope.documento;
               }else{
                  $scope.adres_tipo_doc = $scope.bfiltro.bselectdocumento;
                  $scope.adres_documento = $scope.bdocumento;
               }
            //}            
            $http({
               method:'POST',
               ...
               if (response.data.codigo == 0) {
                  ...
                  if (existe == false) {
                     swal('Completado','Se registro que el afiliado no existe en ADRES, puede continuar con el proceso de afiliación', 'success');
                  }else{
