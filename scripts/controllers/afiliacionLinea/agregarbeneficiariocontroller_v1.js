    ...
    function ($scope, $http, ngDialog, afiliacionHttp, notification, $timeout, $q, upload, communication, $rootScope, $localStorage, $controller, $window) {
      ...
      $scope.obtenerDocumento = function () {
        afiliacionHttp.obtenerDocumento().then(function (response) {
          ...
          console.log($scope.Tipos_Documentos);
        })
      }

      $scope.validarLongitudTipoDoc_Ben = function () {
        if ($scope.bfiltro.bselectdocumento && $scope.bdocumento) {
          var x = $scope.Documentos.find(e => e.CODIGO == $scope.bfiltro.bselectdocumento);
          if (x != -1) {
            if ($scope.bdocumento.toString().length < x.VMIN || $scope.bdocumento.toString().length > x.VMAX) {
              $scope.bdocumento = '';
      ...
      $scope.validarLongitudTipoDoc_Ben_CN = function () {
        if (!$scope.bdocumento_CN){return;}
        if ($scope.bdocumento_CN.toString().length < 9 || $scope.bdocumento_CN.toString().length > 14) {
          $scope.bdocumento_CN = '';
      ...
      self.validaform = function () {
        ...
        if ($scope.bfiltro.bselectdocumento == "" || $scope.bdocumento == undefined || $scope.bdocumento == '' ||
          ...
          if ($scope.can_validate == false) {
            ...
            if ($scope.bfiltro.bselectdocumento != 'RC') {
              $scope.afilValidada();
            } else {
              if ($scope.bdocumento_CN == null || $scope.bdocumento_CN == '') {
        ...
        return new Promise((resolve) => {
          $http({
            ...
            data: {
              ...
              tipo: $scope.bfiltro.bselectdocumento,
              numero: $scope.bdocumento,
              tipo_CN: $scope.bfiltro.bselectdocumento == 'RC' ? $scope.bfiltro.bselectdocumento_CN : '',
              numero_CN: $scope.bfiltro.bselectdocumento == 'RC' ? $scope.bdocumento_CN : ''
            }
          }).then(function ({ data }) {
            resolve(data)//
          });
        ...
        if ($scope.filtro.selecttramite == "C") {
          ...
          $http({
            ...
            data: {
              ...
              tipo_documento_CN: ($scope.bfiltro.bselectdocumento == 'RC') ? $scope.bfiltro.bselectdocumento_CN : '',
              documento_CN: ($scope.bfiltro.bselectdocumento == 'RC') ? $scope.bdocumento_CN : '',
            }
          }).then(function (response) {
            if (response.data.toString().substr(0, 3) != '<br') {
              $scope.reporte = true;
              swal('Completado', response.data, 'success').catch(swal.noop);
              //savedForm();
              ...
              swal('Mensaje', response.data, 'warning').catch(swal.noop);
            }
          }, function errorCallback(response) {
            swal('Mensaje', response.data, 'error').catch(swal.noop);
          });
      ...
      $scope.$watch("bfiltro.bselectdocumento", function (newValue, oldValue) {
