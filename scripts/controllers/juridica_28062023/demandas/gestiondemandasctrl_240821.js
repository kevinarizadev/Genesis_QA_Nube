...
angular.module('GenesisApp')
   .controller('demandasController', ['$scope', '$http', 'consultaHTTP', '$filter', 'ngDialog', 'cfpLoadingBar', '$timeout', 'afiliacionHttp',
      function ($scope, $http, consultaHTTP, $filter, ngDialog, cfpLoadingBar, $timeout, afiliacionHttp) {
         ...
         //VALIDA SOLO NUMEROS EN EL INPUT CC DEMANDADOS
         $scope.replaceDato = function(){
            var valorInputDemandado = document.getElementById("doc_demandado").value;
            $scope.iddemandado = valorInputDemandado.replace(/[^0-9]/, '');
         }
         ...
         $scope.Find_Prestador_enter = function (keyEvent) {
            if (keyEvent.which === 13)
               $scope.H2_Buscar_Prestador();
         }

         $scope.H2_Buscar_Prestador = function () {
            if ($scope.nombredemandado == '' || $scope.nombredemandado == null || $scope.nombredemandado == undefined) {
               Materialize.toast('¡Digite el nombre del prestador!', 1500); $('.toast').addClass('default-background-dark');
               $scope.nombredemandado = $scope.prestadorSave;
            } else {
               if ($scope.nombredemandado != $scope.prestadorSave) {
                  if ($scope.nombredemandado == undefined || $scope.nombredemandado.length < 5) {
                     Materialize.toast('Digite al menos 5 caracteres!', 1500); $('.toast').addClass('default-background-dark');
                  } else {
                     $http({
                        method: 'POST',
                        url: "php/juridica/demandas/funcdemandas.php",
                        data: {
                           ...
                           busqueda: $scope.nombredemandado.toUpperCase()
                        }
                     }).then(function (response) {
                        if (response.data == 0) {
                           Materialize.toast('¡No existen registros del prestador!', 1500); $('.toast').addClass('default-background-dark');
                           $scope.nombredemandado = '';
                           $scope.iddemandado = '';
                           $scope.tipoiddemandado = '';
                           $scope.prestadorSave = '';
                           $scope.prestadorCod = '';
                           $scope.Filter_H2_Prestador = null;
                        }
                        if (response.data.length == 1 && response.data != 0) {
                           $scope.nombredemandado = response.data[0].nombre_prestador;
                           $scope.prestadorSave = response.data[0].nombre_prestador;
                           $scope.iddemandado = response.data[0].nit;
                           $scope.tipoiddemandado = response.data[0].tipo_doc;
                           $scope.Filter_H2_Prestador = null;
                        }
                        if (response.data.length > 1) {
                           $scope.H2_Lista_Prestador = response.data;
                           $scope.Complete_H2_Prestador($scope.nombredemandado);
                        }
                     });
                  }
               } else {
                  Materialize.toast('¡El prestador a consultar es el mismo al seleccionado!', 1500); $('.toast').addClass('default-background-dark');
               }
            }
         }

         $scope.Blur_H2_Prestador = function () {
            if ($scope.nombredemandado != null && $scope.nombredemandado != undefined) {
               if ($scope.H2_Lista_Prestador != null && $scope.H2_Lista_Prestador != 0) {
                  $timeout(function () {
                     if ($scope.nombredemandado != $scope.prestadorSave && $scope.nombredemandado != 0) {
                        $scope.Filter_H2_Prestador = null;
                        $scope.nombredemandado = null;
                        $scope.prestadorCod = '';
         ...
         $scope.Complete_H2_Prestador = function (string) {
            $('#list-group-prestador').css({ width: $('#nombre_demandado')[0].offsetWidth });
            if ($scope.H2_Lista_Prestador != null && $scope.H2_Lista_Prestador != 0) {
               if ($scope.nombredemandado != '' && $scope.nombredemandado != null) {
                  var output = [];
                  angular.forEach($scope.H2_Lista_Prestador, function (H2_Lista_Prestador) {
                     if (H2_Lista_Prestador.nombre_prestador.indexOf(string.toUpperCase()) >= 0) {
                        if (H2_Lista_Prestador.nombre_prestador.toString().substr(0,1)==' ') { 
                           H2_Lista_Prestador.nombre_prestador = H2_Lista_Prestador.nombre_prestador.substr(1, H2_Lista_Prestador.nombre_prestador.length)
                        }
                        if (H2_Lista_Prestador.nombre_prestador.toString().substr(H2_Lista_Prestador.nombre_prestador.length - 1, H2_Lista_Prestador.nombre_prestador.length) == ' ') { 
                           H2_Lista_Prestador.nombre_prestador = H2_Lista_Prestador.nombre_prestador.substr(0, H2_Lista_Prestador.nombre_prestador.length-1)
                        }
                        output.push({ "Prestador": ({ "nit": H2_Lista_Prestador.nit, "nombre_prestador": H2_Lista_Prestador.nombre_prestador, "tipo_doc": H2_Lista_Prestador.tipo_doc}) });
                     }
                  });
                  $scope.Filter_H2_Prestador = output;
               }
            }
         }

         $scope.FillTextbox_H2_Prestador = function (codigo, nombre, tipodoc) {
            if (nombre == "") {
               $scope.nombredemandado = codigo;
            } else {
               $scope.nombredemandado = nombre;
            }
            $scope.prestadorSave = nombre;
            $scope.iddemandado = codigo;
            if (tipodoc=='N') {
               $scope.tipoiddemandado = 'NI';
            }else{
               $scope.tipoiddemandado = 'CC';
            }
            $scope.Filter_H2_Prestador = null;
         }
      }
