  ...
  .controller('registrodereferenciacontroller', ['$scope', '$http', '$filter',
    function ($scope, $http, $filter) {
      ...
      $scope.modoValidacion = function (info) {
        ...
        if (info != 'A') {
          $scope.openmodals('procesoGestion', $scope.datoseleccionado, 'M');
          $("#modaldeValidacion").modal("close");
          $scope.buscaripsReceptora('proceso', 'M');
        } else {
          $scope.openmodals('procesoGestion', $scope.datoseleccionado);
          $("#modaldeValidacion").modal("close");
          $scope.buscaripsReceptora('proceso', 'A');
        }
      }
      $scope.buscaripsReceptora = function (proceso, estado, accion) {
        console.log(proceso);
        if ($scope.procesoCancelacion == 'C') {
          $http({
            method: 'POST',
            url: "php/referencia/registroReferencia/registrodereferencia.php",
            data: {
              function: 'obtenerNombreipsReceptora',
              ips: $scope.buscard2 || 'X',
              vpnumeroips: $scope.form3.pnumero || $scope.modal3.pnumero || $scope.modal4.pnumero || $scope.modal5.pnumero || $scope.codigodereferencia || $scope.modal4.pnumero,
              vptipo: 6,
            ...
            if (response.Codigo == 1) {
              ...
              //$("#modalRedirecionamiento").modal("open");
            }
          })
        }
        if (estado == 'M' && $scope.codigodeValidacion == 2) {

          // if ($scope.buscard2 == '' || $scope.buscard2 == null || $scope.buscard2 == undefined) {
          //   // swal('Importante', 'El campo  de texto no puede estar vacio!', 'info');
          // } else {
          $http({
            method: 'POST',
            ...
            data: {
              function: 'obtenerNombreipsReceptora',
              ips: $scope.buscard2 || 'X',
              vpnumeroips: $scope.form3.pnumero || $scope.modal3.pnumero || $scope.modal4.pnumero || $scope.modal5.pnumero || $scope.codigodereferencia,
          ...
          // }
        } else if (estado == 'M' || estado == undefined && $scope.codigodeValidacion == 1) {
          if ($scope.buscard2 == '' || $scope.buscard2 == null || $scope.buscard2 == undefined) {
            // swal('Importante', 'El campo  de texto no puede estar vacio!', 'info');
            return
          } else {
          ...
          $http({
            ...
            //console.log(response);
            $scope.form3.nombreipsRecep = response.data[0].CODIGO;
            $scope.form3.nombreipsreceptoraVista = response.data[0].NOMBRE;
            if (response.data["0"].Codigo == '0') {
              swal('Importante', 'IPS no encontrada', 'info');
            } else {
      ...
      $scope.seleccionaripsReceptora = function (data) {
        ...
        if (data.length > 0) {
          // console.log(data, +'IF');
          $scope.form3.nombreipsRecep = data[0].CODIGO;
          $scope.form3.nombreipsreceptoraVista = data[0].NOMBRE;
          $scope.modal3.nombreipsRecep = data[0].CODIGO;
          $scope.modal4.nombreipsRecep = data[0].CODIGO;
          $scope.modal5.nombreipsRecep = data[0].CODIGO;
          $scope.listipsReceptora = [];
      ...
      $scope.verdetallesreferenciaIPS = function (datosentrada) {
        ...
        //     function: 'obtenerNombreipsReceptora',
        //     ips: 'X',
        //     vpnumeroips: $scope.codigodereferencia,
        //     // vptipo: $scope.procesoActual
        //   }
        // }).then(function (response) {
        //   console.log(response);
        ...
        //     $scope.listipsReceptora = response.data;
        //     setTimeout(() => {
        //       $scope.seleccionaripsReceptora($scope.listipsReceptora)
        //       $scope.$apply();
        //     }, 2500);
        ...
        $http({
          ...
          if (data && data.toString().substr(0, 3) != '<br') {
            ...
            if (data.Codigo == 1) {
              ...
              $scope.listaDireccionamiento = data;
      ...
      $scope.obtenerDireccionamiento = function () {
      }
