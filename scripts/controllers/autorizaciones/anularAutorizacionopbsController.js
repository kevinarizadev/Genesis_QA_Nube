...
angular.module('GenesisApp')
    .controller('anularAutorizacionopbsController', ['$scope', '$http', 'ngDialog', '$filter', function ($scope, $http, ngDialog, $filter) {
        ...
        $scope.searchAutorizaciones = function () {
            ...
            $http({
                ...
                data: {
                    function: 'p_consulta_autorizaciones_nopbs', numero: $scope.numAutorizacion,
                ...
                if (res.data.info.CODIGO == '0') {
                    swal("Autorización", "No encontrada", "info");
                    $scope.showAutorizacion = false;
                } else {
                    swal.close();
                    if (res.data.aut.length > 0) {
                        $scope.dAutorizacion = [];
                        $scope.dAutorizacion = res.data.aut;                            
            ...
            if ($scope.motivo == null || $scope.motivo == '') {
                ...
                if ($scope.tempAccion == 'ANULAR') {
                    $http({
                        method: 'POST',
                        url: "php/autorizaciones/autorizacionprog/funcautorizacion.php",
                        data: { function: 'p_reversa_aut_nopbs', numero: $scope.tempAut.NUMERO, ubicacion: $scope.tempAut.UBICACION, observacion: $scope.motivo, justificacion: $scope.jutificacion }
                    }).then(function (response) {
                        swal.close();
