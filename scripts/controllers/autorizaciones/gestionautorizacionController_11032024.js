    ...
    $scope.buscarAfiliado = function (tipo, tipodocumento, documento) {
      ...
      $http({
        ...
        if (response.data.CODIGO != "0") {
          ...
          var municipio = $scope.datosafiliadoscitasautomaticas.DEPARTAMENTO+$scope.datosafiliadoscitasautomaticas.MUNICIPIO;
        ...
        if ($scope.productosagregadostabI.length == 0 || $scope.productosagregadostabI == undefined) {
          ...
          // if ($scope.pasarproductoaut == true) {
            ...
            setTimeout(function () {
              ...
              $scope.buscarAutorizaciones($scope.autorizacion.tipodocumento,$scope.autorizacion.documento, $scope.autorizacion.numero, $scope.solicitud.ubicacion)
              swal({ title: "Completado", text: 'Autorización Completada', type: "success", timer: 800 });
              $scope.limpiar('4');
              $scope.$apply();
            }, 500);
        ...
        $scope.setParametros = function (opcion, tipodocumento, documento) {
          ...
          if ($scope.solicitud.pqrds == true) {
            $http({
              method: 'POST',
              url: "php/autorizaciones/autorizacionprog/funcautorizacion.php",
              data: { function: 'obtenerPqrds', tipodocumento: tipodocumento, documento: documento }
            }).then(function (response) {
              $scope.listpqrds = [];
              ...
              if ($scope.listpqrds.length > 0) {
                ...
                $scope.solicitud.pqrds = false;
                swal({ title: "Completado", text: 'Afiliado no tiene Pqr.', type: "info" });
              }
                    ...
                    if ($scope.productosagregadostabI.length > 0) {
                      ...
                      $http({
                        ...
                        if (response.data.Codigo == '0') {
                          ...
                          }else if($scope.controlpqr=='S' && response.data.Estado == 'Activa' && $scope.solicitud.numero_pqr !=''){
                            $scope.estadoaut = 'Procesada';
                          }else if($scope.solicitud.ipscodasignada == 900073223){
                            $scope.estadoaut = 'Procesada';
                          }else{
                            $scope.estadoaut = response.data.Estado;
                          ...
                          if(localStorage.getItem('solicitud_solicitud')){
                            $scope.guardargestionyliberar($scope.numautprocesadamanual);
                        }
                        } else {
                          swal.close();
...
$scope.validartabI = function (tipo) {
  ...
  switch (tipo) {
    ...
    $scope.verAutorizacionesEdit = false;
    $scope.inactivebarraproedit = true;
    $scope.buscarAutorizaciones = function (tipodocumento,documento, numero, ubicacion) {
      $scope.check_option_2 = false;
      $scope.switchtable = false;
      ...
      $http({
        method: 'POST',
        url: "php/autorizaciones/autorizacionprog/funcautorizacion.php",
        data: { function: 'obtenerAutorizaciones',tipodocumento:tipodocumento, documento: documento, numero: numero, ubicacion: ubicacion }
      }).then(function (response) {
        $scope.verAutorizaciones = false;
        swal.close();
