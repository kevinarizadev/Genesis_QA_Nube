...
angular.module('GenesisApp')
  .controller('anticiposController', ['$scope', '$http', '$timeout', '$filter', '$window', '$q', '$interval',
    function ($scope, $http, $timeout, $filter, $window, $q, $interval) {
          ...
          if (Campos_Empty == false) {
            ...
            if (Vista_Empty == 2) {
              setTimeout(function () {
                swal({
                  title: '¡Información!',
                  text: 'El valor total del anticipo debe ser mayor a $282.390, pagar por caja menor',
                  type: 'info'
                }).catch(swal.noop);
              }, 1000);
      ...
      $scope.descargarTodosSoportes = function () {
        var datos = []
        $scope.HojaAnt_Soportes.forEach(e => {
          if (e.RUTA)
            datos.push({
              ruta: e.RUTA
            })
        ...
        $http({
          ...
          data: {
            ...
            anticipo: $scope.HojaAnt.Info.Consecutivo,
            afiliado: `${$scope.HojaAnt.Afiliado.TipoDoc}${$scope.HojaAnt.Afiliado.NumeroDoc}`,
            datos: JSON.stringify(datos)
          }
        }).then(function ({ data }) {
          swal("Mensaje", "Soportes descargados", "success").catch(swal.noop);
