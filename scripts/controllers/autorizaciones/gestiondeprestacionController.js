...
angular.module('GenesisApp')
  ...
  .controller('gestiondeprestacionController', ['$scope', '$http', '$location', 'ngDialog', '$filter',
    ...
    if($scope.nitIps!='0'){
      $scope.bloquearinput = true;
     $scope.nitipsdescarga = $scope.nitIps;
    }else{
      $scope.bloquearinput = false;
    }
...
$scope.Obtener_Tipos_Documentos();
        ...
        if ($scope.required_fiels == false) {
          ...
          $http({
            ...
            if (response.data.CODIGO == '0') {
              ...
              if(response.data.aut[0]){
                $scope.variablepqr = response.data.aut[0].AUTN_PQR;
                $scope.cuotamoderadora = response.data.aut[0].cuota_moderadora;
              }else{
                $scope.variablepqr ='0'
              }
              $scope.codigoregimen = response.data.info.CodigoRegimen;
              $scope.calcularEdad($scope.infoafiliadoautedit.FechaNacimiento);
              $scope.initPaginacion($scope.listarAutorizaciones);
      ...
      $scope.openmodals = function (tipo, opcion) {
        $scope.valorcopago = 0;
        $scope.valorcopagocomparar = 0;
        for (let i = 0; i < opcion.DETALLES.length; i++) {
          $scope.valorcopago +=  parseInt(opcion.DETALLES[i].copago);
          $scope.valorcopagocomparar +=  parseInt(opcion.DETALLES[i].copago);
        }
      ...
      $scope.gestionaraut = function () {
        ...
        var archivoobligatorio = false;
          if($scope.valorcopago < $scope.valorcopagocomparar){
            archivoobligatorio = true;
          }else{
            archivoobligatorio = false;
          }
          // if(archivoobligatorio || $scope.variablepqr !='0'){
          if(archivoobligatorio){
            if($scope.archivocargar1 ==''){
              swal('Info', 'El Adjunto es Obligatorio', 'info');
              return;
            }else{
        ...
        if ($scope.fechaprestacion !="") {
          ...
          $http({
            ...
            var inputElement = document.getElementById("anexo2adj1");
            inputElement.value = "";
            $scope.nombrearchivo1 = "";
            $scope.archivocargar1 = "";
            $scope.solicitudext = "";
                $scope.fechaprestacion= new Date();
    ...
    $scope.modalExcelAut = function () {
      $("#modaldescargaregistros").modal("open");
  }
    $scope.descargarreportepss = function () {
      window.open(
        "php/autorizaciones/gprestacion/descargardatosautpss.php?nitips="+$scope.nitipsdescarga+"&fechainicio="+formatDate( $scope.fechainiciodescarga)+"&fechafin="+formatDate( $scope.fechafindescarga));
  }
