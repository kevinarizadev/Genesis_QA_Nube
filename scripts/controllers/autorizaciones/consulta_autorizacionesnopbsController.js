...
angular.module('GenesisApp')
  ...
  .controller('consultanopbsController', ['$scope', '$http', '$location', '$filter', 'ngDialog', '$timeout', 'pqrHttp', 'afiliacionHttp', function ($scope, $http, $location, $filter, ngDialog, $timeout, pqrHttp, afiliacionHttp) {
      ...
      if ($scope.validatefiltros == false) {
        ...
        if ($scope.filterOptions == 'AVANZADO') {
          ...
          $http({
            method: 'POST',
            url: "php/autorizaciones/autorizacionprog/funcautorizacion.php",
            data: { function: 'p_consulta_autorizaciones_nopbs', documento: tempdocumento, numero: tempnumero, ubicacion: tempubicacion, ips: $scope.nit }
          }).then(function (response) {
            if (response.data.info.CODIGO == '0') {
              $scope.verAutorizaciones = true;
              $scope.infoafiliadoautedit = [];
    ...
    if ($scope.responsable == '1052991644' || $scope.responsable == '1129583838' || $scope.responsable=='1080010263') {
