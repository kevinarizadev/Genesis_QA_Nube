    ...
    $scope.buscarAfiliado = function (tipo, tipodocumento, documento) {
      $http({
        ...
        if (response.data.CODIGO != "0") {
          $scope.tipodocumentovalidarpgp = tipodocumento;
          $scope.documentovalidarpgp = documento;
          if (tipo == '3') {
      ...
      if ($scope.buscarpro.length >= 4) {
        ...
        $http({
          ...
          data: { function: 'obtenerProducto', 
                  ...
                  sexo: sexo, 
                  tipo: 'S',
                  tipodocumentoafiliado : $scope.tipodocumentovalidarpgp,
                  documentoafiliado :$scope.documentovalidarpgp }
        }).then(function (response) {
          if (response.data.toString().substr(0, 3) == '<br' || response.data == 0) {
            swal("Error", response.data || 'Sin datos', "warning").catch(swal.noop); return
          }
          $scope.listProductos = [];
          if (response.data["0"].CODIGO == '-1') {
            swal('Importante', response.data["0"].NOMBRE, 'info');
