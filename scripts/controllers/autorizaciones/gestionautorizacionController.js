  ...
  .controller("gestionautorizacionController", [
    ...
    function (
      ...
      $(document).ready(function () {
        ...
        $("#Modal_Celular").modal();
        var myInputmedico = document.getElementById("quitarpastemedico");
        myInputmedico.onpaste = function (e) {
          e.preventDefault();
          alert("esta acción no esta permitida");
        };
        var myInput = document.getElementById("textarea1");
      ...
      $scope.controlpqr = sessionStorage.getItem("controlpqr"); //yordis
      $scope.controlanticipo = sessionStorage.getItem("controlanticipo"); //yordis
      $scope.tabI = true;
      $scope.tabII = false;
      $scope.tabIII = false;
      $scope.tabIV = false;
      ...
      $scope.vertablaordenamiento = false;
      ...
      $scope.formatDateProg = function (addDias) {
        let date = new Date().setDate(new Date().getDate() + addDias);
        if (date) {
          var d = new Date(date),
            month = "" + (d.getMonth() + 1),
            day = "" + d.getDate(),
      ...
      $scope.maxDateProgramacion = $scope.formatDateProg(5);
      $scope.minDateProgramacion = $scope.formatDateProg(0);
      ...
      $scope.listObservacionesAdmin = [
        { Codigo: 1, Nombre: "AUTORIZADO BAJO COTIZACION" },
        { Codigo: 2, Nombre: "INCLUYE MATERIALES MAS INSUMOS" },
        { Codigo: 3, Nombre: "INCLUYE DERECHO DE SALA MAS INSUMOS" },
        { Codigo: 4, Nombre: "SE AUTORIZA PLAN REMEO FEBRERO 2020" },
      ];
      ...
      if (localStorage.getItem("solicitud_solicitud")) {
        const { TIPO_DOC, DOCUMENTO, NUMERO } = JSON.parse(
          localStorage.getItem("solicitud_solicitud")
        );
        $scope.solicitudesaa = {
        ...
        if (TIPO_DOC && DOCUMENTO && NUMERO) {
          $http({
            ...
            data: {
              ...
              data: JSON.stringify($scope.solicitudesaa),
            },
          }).then(function (response) {
            $scope.tempsolicitudesaa = response.data;
            console.log($scope.tempsolicitudesaa);

            var tempAut = response.data[0].ADJUNTO.replace(/\r?\n$/, "");
            $scope.adjuntotabI = tempAut;
      ...
      $scope.buscarAfiliado = function (tipo, tipodocumento, documento) {
        $scope.buscardatosordenamiento(tipodocumento, documento);
        $scope.buscarsolicitudoficionavirtual(tipodocumento,documento);
        ...
        $http({
          ...
          if (response.data.CODIGO != "0") {
            ...
            var municipio =
              $scope.datosafiliadoscitasautomaticas.DEPARTAMENTO +
              $scope.datosafiliadoscitasautomaticas.MUNICIPIO;
            $scope.Agendar_Validar_Municipio(municipio);
            if (tipo == "1") {
              $http({
                method: "POST",
                ...
                if ($scope.datosafiliadoscitasautomaticas.NOV_DIR == "N") {
                // if ($scope.respues_actualizar == "S") {
                  sessionStorage.setItem("tipo", tipodocumento);
                  sessionStorage.setItem("doc", documento);
                  ngDialog.open({
                    template: "views/afiliados/modal/modaldatoscontactoaut.html",
                    className: "ngdialog-theme-plain",
                    controller: "modaldatoscontactoautController",
                    scope: $scope,
                    closeByEscape: false,
              ...
              if ($scope.infoafiliadoaut.EMPLEADOR) {
                $scope.infoafiliadoaut.EMPLEADOR = JSON.parse(
                  $scope.infoafiliadoaut.EMPLEADOR
                );
              }
              ...
              if ($scope.infoafiliadoaut.ADULTO_MAYOR == "true") {
                $scope.afirownumI = $scope.afirownumI + 1;
              }

              if ($scope.infoafiliadoaut.DISCAPACITADO == "true") {
                $scope.afirownumI = $scope.afirownumI + 1;
              }
              ...
              if ($scope.infoafiliadoautedit.Estado != "ACTIVO") {
                $scope.informacionmodaledit =
                  "Afiliado no se encuentra activo en base de datos";

                swal(
                  "Importante",
                  "Afiliado no se encuentra activo en la base de datos.",
                  "info"
                );
              }
              ...
              $scope.calcularEdad(
                $scope.infoafiliadoautedit.FechaNacimiento,
                tipo
              );
              ...
              if ($scope.infoafiliadoautpro.Estado != "ACTIVO") {
                ...
                $scope.calcularEdad(
                  $scope.infoafiliadoautpro.FechaNacimiento,
                  tipo
                );
                ...
                $scope.regimenafitabII =
                  $scope.infoafiliadoautpro.CodigoRegimen;

                $scope.solicitudpro.email = $scope.infoafiliadoautpro.email;

                $scope.solicitudpro.celular =
                  $scope.infoafiliadoautpro.Celular1;
      ...
      $scope.buscardatosordenamiento = function (documento, numero) {
        $http({
          method: "POST",
          url: "php/autorizaciones/autorizacionprog/funcautorizacion.php",
          data: {
            function: "obtenerdatosordenamiento",
            documento: documento,
            numero: numero,
            ordenmedica: "",
            nit: "",
          ...
          $scope.datosordenamiento = response.data;
          if ($scope.datosordenamiento[0].TIPO_DOC) {
            $scope.vertablaordenamiento = true;
          } else {
            $scope.vertablaordenamiento = false;
          }
        });
      ...
      $scope.modalmedicamentos = function (listado) {
        $("#modalmedicamento").modal("open");
        $scope.listadomedicamento = listado.MEDICAMENTO;
        console.log(listado.MEDICAMENTO);
      };

        $scope.buscarsolicitudoficionavirtual = function (documento,numero) {
          $http({
            method: 'POST',
            url: "php/autorizaciones/afiliado/solicitud_autorizacion.php",
            data: { function: 'p_llamar_sol_aut_afiliados_autorizaciones', documento: documento, numero: numero, estado:'P' }
          }).then(function (response) {
            $scope.datosoficinavirtual = response.data;
            if($scope.datosoficinavirtual.length > 0) {
              $scope.vertabla = true;
              $http({
                method: 'POST',
                ...
                $scope.listMotivosAnu = res.data[0];
        ...
        $scope.versolicitudactiva = function(numero_aut_sol,ubicacion_aut_sol){
          $scope.init();
          $scope.autorizacion={
            numero:"",
            ubicacion:""
          ...
          $scope.tipoaut = "4";
          setTimeout(() => {
            $scope.autorizacion.numero = numero_aut_sol.toString();
            $scope.solicitud.ubicacion = ubicacion_aut_sol.toString();
            $scope.buscarAutorizaciones("","",numero_aut_sol,ubicacion_aut_sol)
          }, 800);

        }
        $scope.autorizarporoficina = function (solicitud){
          $scope.vertabla = false;
          $scope.inactiveseccion2tab1 = false;
              $scope.adjuntotabI = solicitud.ADJUNTO;
              $scope.ftptabI = solicitud.FTP;
              $scope.solicitud.numero_solicitud = solicitud.NUMERO;
              $scope.solicitud.tipodocumento = TIPO_DOC;
              $scope.solicitud.documento = DOCUMENTO;
        }
        $scope.saveanulacion = function(solicitud,check_gestion){
          $scope.nuevalistamotivo = $scope.listMotivosAnu;
          if(check_gestion == '2'){
            $scope.nuevalistamotivo = $scope.listMotivosAnu;
            $scope.listadodemotivos = [
              {
                "CODIGO": 100,
                "NOMBRE": "Capitado"
              },
            ...
            $scope.listadodemotivos = $scope.nuevalistamotivo;
          }
          swal({
            title: 'Seleccionar Motivo y Escribir la Justificacion',
            ...
            <div class="input-field col s12" style="padding:0">
                                    ...
                                    <select id="motivoanulacion" value="${this.CODIGO}" class="select-chosen-eps">
                                      <option value="">SELECCIONAR MOTIVO</option>
                                      ${$scope.listadodemotivos.map(ven => `<option value="${ven.CODIGO}">${ven.NOMBRE}</option>`)}
                                    </select>
                                  </div>
                                    <div class="input-field col s12" style="padding:0">
                                    <textarea id="justificacionanulacion" class="materialize-textarea" style="max-height: 10em;
            ...
            if (check_gestion == 2) {
              accionenviar = 'N';
            }else if (check_gestion == 3){
              accionenviar = 'X';
            }
            if (result[0] && result[1]) {
              var json = {
                accion: accionenviar,
                motivo_anulacion: result[0],
                justificacion_anulacion: result[1],
                responsable: sessionStorage.getItem('cedula'),
                numero_solicitud: solicitud.NUMERO,
                ubicacion_solicitud: solicitud.UBICACION
              }
              $http({
                method: 'POST',
                url: "php/autorizaciones/afiliado/solicitud_autorizacion.php",
                ...
                if (response.data.Codigo == 1) {
                  ...
                  setTimeout(() => {
                    $scope.buscarAfiliado('1',solicitud.TIPO_DOC,solicitud.DOCUMENTO);
                  }, 1000);
                }
              })
            } else {
        ...
        switch (tipo) {
          ...
          case "modalservicio":
            ...
            if ($scope.valservicio == true) {
              swal(
                "Importante",
                "La Ips Asignada no puede estar vacia!",
                "info"
              );
            } else {
          ...
          case "modalespecialidad":
            ...
            if ($scope.valespecialidad == true) {
              swal(
                "Importante",
                "La Ips Solicitante no puede estar vacia!",
                "info"
              );
            } else {
      ...
      $scope.closemodals = function (tipo) {
        switch (tipo) {
          ...
          case "modalautsoportes":
            ...
            break;
          case "pqrds":
            if ($scope.solicitud.pqrds == true) {
              if ($scope.solicitud.numero_pqr == "") {
                $scope.solicitud.pqrds = false;
                $scope.solicitud.numero_pqr = "";
                $scope.solicitud.programable = false;
                $scope.solicitud.aut_programable = "";
              }
            }
            $("#modalpqrds").modal("close");
            break;
          default:
        }
      };
      ...
      $scope.buscarnovedades = function () {
        $http({
          ...
          data: {
            ...
            documento: $scope.datosAfiModalNov.Documento,
      ...
      $scope.seleccionarips = function (data, tipo) {
        ...
        if ($scope.tipoaut == "1") {
          if (tipo == "S") {
            ...
            console.log($scope.solicitud.ipscodasignada);
            ...
            $scope.cargarContratoTabI(
              $scope.solicitud.ipscodasignada,
              $scope.regimenafitabI,
              "tab1"
            ); //faltan parametros
          }
        }
        ...
        if ($scope.tipoaut == "2") {
          if (tipo == "S") {
            ...
            text = "Ips Asignada.";
            $scope.listarContratoServicio(
              $scope.solicitudpro.ipscodasignada,
              $scope.regimenafitabII,
              text
            ); //faltan parametros
          }
        ...
        if ($scope.tipoaut == "4") {
          if (tipo == "S") {
            ...
            $scope.cargarContratoTabI(
              $scope.autedit.ipscodasignada,
              $scope.regimenafitabIV,
              "tab4"
            );
        ...
        $http({
          ...
          if (response.data["0"].CODIGO == "1") {
            if ($scope.listacontratost2.length == 1) {
              ...
              $scope.solicitudpro.contratoDocumento =
                response.data["0"].DOCUMENTO;
              $scope.solicitudpro.contratoUbicacion =
                response.data["0"].UBICACION;
              $http({
                method: "POST",
                url: "php/autorizaciones/autorizacionprog/funcautorizacion.php",
                data: {
            ...
            swal({
              title: "Completado",
              text: "IPS Asignada no tiene contrato",
          ...
          $http({
            ...
            data: {
              ...
              tipodocumentoafiliado:
                $scope.datosafiliadoscitasautomaticas.TipoDocumento,
              documentoafiliado:
                $scope.datosafiliadoscitasautomaticas.Documento,
      ...
      $scope.buscarProductoEdit = function (pro) {
        if (pro.length >= 6) {
          ...
          $http({
            ...
            data: {
              function: "obtenerProducto",
              regimen: $scope.regimentabIV,
              contrato: $scope.contratotabIV,
              producto: pro,
              clasificacion: $scope.serviciotabIV,
              tipo: tipo,
              ubicacion: $scope.ubicaciontabIV,
            },
          }).then(function (response) {
      ...
      $scope.seleccionandoelproducto = function (data) {
        $scope.tempProd = data;
        if (
          $scope.solicitud.codservicio == "714" ||
          $scope.autedit.codservicio == "714"
        ) {
          $scope.tempData = data;
          $scope.openmodals("modalcantprod", $scope.tipoaut);
        } else {
            ...
            if (result > 0) {
              ...
              if ($scope.tipoaut == "1") {
                if ($scope.productosagregadostabI.length == 0) {
                  ...
                  for (
                    let index = 0;
                    index < $scope.productosagregadostabI.length;
                    index++
                  ) {
                    const element = $scope.productosagregadostabI[index];
                    if (element.CODIGO == data.CODIGO) {
                  ...
                  if (comps == 0) {
                    ...
                    $scope.productosagregadostabI[sindex].CANTIDAD =
                      data.CANTIDAD;
                    $scope.productosagregadostabI[sindex].CANTIDADN =
                      data.CANTIDAD;
                  }
                }
                if ($scope.productosagregadostabI.length == 0) {
                ...
                if ($scope.tempProd.SUBCLAS == "S") {
                  $http({
                    ...
                    if ($scope.listMotivos.length > 0) {
                      for (var i = 0; i < $scope.listMotivos.length; i++) {
                        var val =
                          $scope.listMotivos[i].NUMERO_H +
                          " - " +
                          $scope.listMotivos[i].NOMBRE_H +
                          " - " +
                          $scope.listMotivos[i].VALOR;
                        $scope.array[val] = val;
                      }
                      ...
                      swal({
                        ...
                        if (result) {
                          const filteredItems =
                            $scope.productosagregadostabI.findIndex(
                              (item) => item.CODIGO == $scope.tempProd.CODIGO
                            );
                          console.log("filteredItems_", filteredItems);
                          if ($scope.tempsubcla[0] == "99999") {
                            $scope.productosagregadostabI[
                            ...
                            const filteredMotivos = $scope.templistMotivos.find(
                              (item) => item.NUMERO_H == $scope.tempsubcla[0]
                            );
                            $scope.productosagregadostabI[
                              filteredItems
                            ].SUBCLASCOD = filteredMotivos.NUMERO_H;
                            $scope.productosagregadostabI[
                              filteredItems
                            ].SUBCLASNOM = filteredMotivos.NOMBRE_H;
                            $scope.productosagregadostabI[
                              filteredItems
                            ].VALORP = filteredMotivos.VALOR;
                            $scope.$apply();
                          }
                          swal({
                            title: "Completado",
                      ...
                      console.log("filteredItems_", filteredItems);
                      $scope.productosagregadostabI[filteredItems].SUBCLASN =
                        "N";
                ...
                if ($scope.productosagregadostabIV.length == 0) {
                  ...
                  for (
                    let index = 0;
                    index < $scope.productosagregadostabIV.length;
                    index++
                  ) {
                    const element = $scope.productosagregadostabIV[index];
                    if (element.CODIGO == data.CODIGO) {
                  ...
                  if (come == 0) {
                    ...
                    $scope.productosagregadostabIV[eindex].CANTIDAD =
                      data.CANTIDAD;
                    $scope.productosagregadostabIV[eindex].CANTIDADN =
                      data.CANTIDAD;
                  }
                }
                if ($scope.productosagregadostabIV.length == 0) {
                    ...
                    if ($scope.listMotivos.length > 0) {
                      ...
                      swal({
                        ...
                        if (result) {
                          console.log("$scope.tempProd_", $scope.tempProd);
                          console.log(
                            "$scope.productosagregadostabI_",
                            $scope.productosagregadostabIV
                          );
                          const filteredItems =
                            $scope.productosagregadostabIV.findIndex(
                              (item) => item.CODIGO == $scope.tempProd.CODIGO
                            );
                          console.log(filteredItems);
                          console.log($scope.productosagregadostabIV);
                      ...
                      console.log("filteredItems_", filteredItems);
                      $scope.productosagregadostabIV[filteredItems].SUBCLASN =
                        "N";
                ...
                if ($scope.productosagregadostabII.length == 0) {
                  ...
                  for (
                    let index = 0;
                    index < $scope.productosagregadostabII.length;
                    index++
                  ) {
                    const element = $scope.productosagregadostabII[index];
                    if (element.CODIGO == data.CODIGO) {
                  ...
                  if (comp == 0) {
                    ...
                    $scope.productosagregadostabII[pindex].CANTIDAD =
                      data.CANTIDAD;
                    $scope.productosagregadostabII[pindex].CANTIDADN =
                      data.CANTIDAD;
                  }
                }
                ...
                if ($scope.productosagregadostabII.length == 0)
                  $scope.nofindproductstabII = false;
                else $scope.nofindproductstabII = true;
                    ...
                    if ($scope.listMotivos.length > 0) {
                      ...
                      swal({
                        ...
                        if (result) {
                          ...
                          const filteredItems =
                            $scope.productosagregadostabII.findIndex(
                              (item) => item.CODIGO == $scope.tempProd.CODIGO
                            );
                          console.log(filteredItems);
                          console.log($scope.productosagregadostabII);
                      ...
                      console.log("filteredItems_", filteredItems);
                      $scope.productosagregadostabII[filteredItems].SUBCLASN =
                        "N";
                      $scope.$apply();
                      swal({
                        title: "Completado",
      ...
      $scope.guardarProdEntreg = function () {
        if ($scope.tempCantidad > 0) {
          ...
          $scope.tempData.CANTIDAD = Math.round(
            $scope.tempData.CANTIDAD / $scope.tempData.ENTREGAS
          );
          $scope.tempData.CANTIDADREST =
            $scope.tempCantidad - $scope.tempData.CANTIDAD;
          if ($scope.tipoaut == "1") {
            if ($scope.productosagregadostabI.length == 0) {
              ...
              if (comps == 0) {
                ...
                $scope.productosagregadostabI[sindex].CANTIDAD =
                  $scope.tempData.CANTIDAD;
              }
            }
            if ($scope.productosagregadostabI.length == 0)
              $scope.nofindproductstabI = false;
            else $scope.nofindproductstabI = true;
          } else if ($scope.tipoaut == "4") {
            $scope.inactivebarraproedit = true;
            if ($scope.productosagregadostabIV.length == 0) {
            ...
            if ($scope.productosagregadostabIV.length == 0)
              $scope.nofindproductstabIV = false;
            else $scope.nofindproductstabIV = true;
          } else {
            if ($scope.productosagregadostabII.length == 0) {
      ...
      $scope.editProducto = function (index, tipo) {
        ...
        if (tipo == "1") {
          if ($scope.solicitud.codservicio == "714") {
            ...
            $scope.tempCantidad = parseInt(
              $scope.productosagregadostabI[index].CANTIDADN
            );
            $scope.tempEntregas = $scope.productosagregadostabI[index].ENTREGAS;
          } else {
            if ($scope.productosagregadostabI[index]) {
              $scope.cantidadinput =
                $scope.productosagregadostabI[index].CANTIDAD;
          ...
          if ($scope.v_encabezado.CLASIFICACION == "714") {
            ...
            $scope.tempCantidad = parseInt(
              $scope.productosagregadostabIV[index].CANTIDADN
            );
            $scope.tempEntregas =
              $scope.productosagregadostabIV[index].ENTREGAS;
          } else {
        ...
        if (
          ($scope.v_encabezado &&
            $scope.v_encabezado.CLASIFICACION == "714" &&
            tipo == "4") ||
          ($scope.solicitud.codservicio == "714" && tipo == "1")
        ) {
          $scope.openmodals("modalcantprod", $scope.tipoaut);
        }
      ...
      $scope.addObsAdmin = function (data) {
        ...
        for (var i = 0; i < $scope.listObservacionesAdmin.length; i++) {
          var val =
            $scope.listObservacionesAdmin[i].Codigo +
            " - " +
            $scope.listObservacionesAdmin[i].Nombre;
          $scope.array[val] = val;
        }
        ...
        if ($scope.tipoaut == "1") {
          swal({
            ...
            if (result) {
              ...
              const filteredItems = $scope.productosagregadostabI.findIndex(
                (item) => item === $scope.tempProdObs
              );
              $scope.productosagregadostabI[filteredItems].OBSADMIN =
                tempresult[0];
              $scope.productosagregadostabI[filteredItems].OBSNOM =
                tempresult[1];
              $scope.$apply();
              swal({
                title: "Completado",
        ...
        if ($scope.tipoaut == "4") {
          swal({
            ...
            if (result) {
              var tempresult = result.split("-");
              const filteredItems = $scope.productosagregadostabIV.findIndex(
                (item) => item === $scope.tempProdObs
              );
              $scope.productosagregadostabIV[filteredItems].OBSADMIN =
                tempresult[0];
              $scope.productosagregadostabIV[filteredItems].OBSNOM =
                tempresult[1];
              $scope.$apply();
              swal({
                title: "Completado",
      ...
      $scope.limpiar = function (tab) {
        switch (tab) {
          ...
          case "5":
            ...
            $scope.inactiveseccion4tab4 = false;
            $scope.check_option = false;
            $scope.autorizacion.tipodocumento = null;
      ...
      $http({
        ...
        console.log($scope.listOrigenes);
      });
      ...
      $scope.pasostabI = function (op) {
        switch (op) {
            ...
            if ($scope.pasarsolicitudaut == true) {
              ...
              swal(
                ...
                $scope.data.requiredFile == true
                  ? "Verifique el peso y/o la extension del adjunto"
                  : $scope.validatext,
                "info"
              );
            }
          ...
          case "6":
            if (
              $scope.productosagregadostabIV.length == 0 ||
              $scope.productosagregadostabIV == undefined
            ) {
              var text =
                "Se creara la autorizacion en estado activa debido a que faltan los productos";
            } else {
          ...
          case "3":
            if (
              $scope.productosagregadostabI.length == 0 ||
              $scope.productosagregadostabI == undefined
            ) {
              var text =
                "Se creara la autorizacion en estado activa debido a que faltan los productos";
            } else {
          ...
          case "t4":
            setTimeout(function () {
              $scope.solicitud.ubicacion = $scope.autorizacion.documento
                ? 0
                : $scope.solicitud.ubicacion;
              $scope.buscarAutorizaciones(
                $scope.autorizacion.tipodocumento,
                $scope.autorizacion.documento,
                $scope.autorizacion.numero,
                $scope.solicitud.ubicacion
              );
              swal({
                title: "Completado",
            ...
            if (tab == "tab1") {
              for (var i = 0; i < $scope.listContratostab1.length; i++) {
                if ($scope.listContratostab1[i].NUMERO == contrato) {
                  $scope.solicitud.ubicacioncontrato =
                    $scope.listContratostab1[i].UBICACION;
                  $scope.solicitud.documentocontrato =
                    $scope.listContratostab1[i].DOCUMENTO;
                }
              }
              $scope.listServicios = response.data;
            } else if (tab == "tab4") {
              for (var i = 0; i < $scope.listContratostab4.length; i++) {
                if ($scope.listContratostab4[i].NUMERO == contrato) {
                  $scope.autedit.ubicacioncontrato =
                    $scope.listContratostab4[i].UBICACION;
                  $scope.autedit.documentocontrato =
                    $scope.listContratostab4[i].DOCUMENTO;
                }
              }
              setTimeout(() => {
                if (
              ...
              for (var i = 0; i < $scope.listacontratost2.length; i++) {
                if ($scope.listacontratost2[i].NUMERO == contrato) {
                  $scope.solicitudpro.ubicacioncontrato =
                    $scope.listacontratost2[i].UBICACION;
                  $scope.solicitudpro.documentocontrato =
                    $scope.listacontratost2[i].DOCUMENTO;
                }
              }
              $scope.listServicios = response.data;
            }
        ...
        switch (opcion) {
            ...
            if ($scope.solicitud.tutela == true) {
              $http({
                ...
                if ($scope.listTutelas[0].Codigo != "0") {
                  ...
                  swal({
                    title: "Completado",
                    text: "Afiliado no tiene tutelas.",
                    type: "info",
                  });
                }
              });
            ...
            if ($scope.solicitud.siniestro == true) {
              $http({
                ...
                if ($scope.listSiniestros["0"].Codigo != "0") {
                  ...
                  swal({
                    title: "Completado",
                    text: "Afiliado no tiene enfermedad de alto costo",
                    type: "info",
                  });
                }
              });
            ...
            if ($scope.solicitud.aut == true) {
              ...
              $http({
                ...
                if (response.data.length == 0) {
                  swal({
                    title: "Completado",
                    text: "Afiliado no tiene Autorizaciones Anuladas!",
                    type: "info",
                  });
                  $scope.solicitud.aut = false;
                } else {
            ...
            if ($scope.autedit.aut == true) {
              ...
              $http({
                ...
                data: {
                  ...
                  tipodocumento: tipodocumento,
                  documento: documento,
                  nit: $scope.autedit.ipscodasignada,
                },
              }).then(function (response) {
                if (response.data.length == 0) {
                  swal({
          ...
          case "autsoporte":
            ...
            if ($scope.solicitud.autsoporte == true) {
              $scope.afiliadoautsoportes.NombreCompleto =
                $scope.infoafiliadoaut.NombreCompleto;
              ...
              $http({
                ...
                if (response.data.length == 0) {
                  swal({
                    title: "Completado",
                    text: "Afiliado no tiene Autorizaciones para mostrar! ",
                    type: "info",
                  });
                  $scope.solicitud.aut = false;
                } else {
            ...
            if ($scope.autedit.autsoporte == true) {
              $scope.afiliadoautsoportes.NombreCompleto =
                $scope.infoafiliadoautedit.NombreCompleto;
              //if ($scope.autedit.ipsasignada == '') {
              // swal({ title: "No Completado", text: 'Ips asignada no puede estar vacia!', type: "info" });
              //  } else {
              $http({
          ...
          case "pqrds":
            if ($scope.solicitud.pqrds == true) {
              $http({
                ...
                if ($scope.listpqrds.length > 0) {
                  // $scope.inactivetagtutela = false;
                  $("#modalpqrds").modal("open");
                } else {
                  $scope.solicitud.pqrds = false;
                  swal({
                    title: "Completado",
                    text: "Afiliado no tiene Pqr.",
                    type: "info",
                  });
                }
              });
      ...
      $scope.selecionarpqrds = function (numero) {
        $scope.solicitud.numero_pqr = numero;
        $("#modalpqrds").modal("close");
        $scope.activarprogramable = false;
      };
      $scope.verPrint = true;
        ...
        swal({
          ...
          if (result) {
            if ($scope.data.requiredFile == true) {
              swal({
                title: "Importante",
                text: "Verificar extesion y/o perso del adjunto!",
                type: "info",
              });
            } else {
              $scope.solicitud.fechasolicitudparseada = formatDate(
                $scope.solicitud.fechasolicitud
              );
              if (
                $scope.solicitud.fecha_cita_normal != "" &&
                $scope.solicitud.fecha_cita_normal != undefined
              ) {
                $scope.solicitud.fecha_cita = formatDate(
                  $scope.solicitud.fecha_cita_normal
                );
              }
              ...
              if ($scope.solicitud.cuotaanticipo == true) {
                $scope.solicitud.valoranticipo = "C";
              } else {
                $scope.solicitud.valoranticipo = "";
                // $scope.solicitud.cuota = 'N';
              }
              ...
              if ($scope.solicitud.programable == true) {
                $scope.solicitud.aut_programable = "S";
              } else {
                $scope.solicitud.aut_programable = "";
              }
              ...
              $http({
                ...
                if ($scope.respuesta.Codigo == "0") {
                  ...
                  $scope.numautprocesadamanual = $scope.respuesta.Autorizacion;
                  $scope.contadorSwal = 0;
                  // ------------------------------------------------------
                  if ($scope.solicitud.ext) {
                    $http({
                      ...
                      $scope.subirAdjunto(
                        JSON.stringify({ ARCHIVO: response.data })
                      );
                    });
                  }
                  ...
                  if ($scope.solicitud.autsoporte == true) {
                    $scope.subirAdjunto(
                      JSON.stringify({ ARCHIVO: $scope.adjuntotabI })
                    );
                  }
                  // ------------------------------------------------------
                  ...
                  setTimeout(() => {
                    if ($scope.productosagregadostabI.length > 0) {
                      var dataproductos = JSON.stringify(
                        $scope.productosagregadostabI
                      );
                      $http({
                        method: "POST",
                        ...
                        data: {
                          ...
                          cantidad: $scope.productosagregadostabI.length,
                        ...
                        if (response.data.Codigo == "0") {
                          ...
                          $scope.estadonombremostrar = response.data.Estado;
                          if (response.data.Estado == "Procesada") {
                            ...
                            $scope.estadoaut = "Procesada";
                          } else if (
                            $scope.solicitud.ipscodasignada == 900073223
                          ) {
                            $scope.estadoaut = "Procesada";
                          } else {
                            $scope.estadoaut = response.data.Estado;
                          ...
                          $http({
                            ...
                            data: {
                              ...
                              autorizacion: JSON.stringify($scope.solicitud),
                              productos: dataproductos,
                              cantidad: $scope.productosagregadostabI.length,
                            },
                          }).then(function (response) {
                          ...
                          if (localStorage.getItem("solicitud_solicitud")) {
                            $scope.guardargestionyliberar(
                              $scope.numautprocesadamanual
                            );
                          }
                        } else {
                          swal.close();
      ...
      $scope.guardargestionyliberar = function (numeroaut) {
        if (localStorage.getItem("solicitud_solicitud")) {
          const { TIPO_DOC, DOCUMENTO, NUMERO } = JSON.parse(
            localStorage.getItem("solicitud_solicitud")
          );
          if (TIPO_DOC && DOCUMENTO && NUMERO) {
            $http({
              ...
              url: "php/autorizaciones/afiliado/solicitud_autorizacion.php",
              data: {
                function: "guardargestionoficinavirtual",
              ...
              console.log(response);
              localStorage.removeItem("solicitud_solicitud");
            });
          }
        }
      };

      $scope.Agendar_Validar_Municipio = function (municipio) {
        $http({
          method: "POST",
          url: "php/autorizaciones/afiliado/solicitud_autorizacion.php",
          data: {
          ...
          console.log(response.data);
          $scope.validaciondelmuni = response.data.codigo;
        });
      };

      $scope.ModalNumeroCelular = function () {
        if (
          $scope.datosafiliadoscitasautomaticas.EdadAnhos >= 25 &&
          $scope.datosafiliadoscitasautomaticas.EdadAnhos <= 49 &&
          $scope.datosafiliadoscitasautomaticas.SexoCodigo == "F" &&
          $scope.validaciondelmuni == 0
        ) {
      ...
      $scope.Agendar_Citas = () => {
        ...
        if (cardnumber.length < 10) {
          swal({
            title: "Importante",
            text: "El Numero de celular minimo debe contener 10 Caracteres",
            type: "info",
          });
        } else {
          if (
            ...
            var nombreapellido =
              $scope.datosafiliadoscitasautomaticas.NombreCompleto.split(" ");
            const bodyParameters = {
              ...
              fecha_nacimiento:
                $scope.datosafiliadoscitasautomaticas.FechaNacimiento,
              ...
              celular: $scope.datosafiliadoscitasautomaticas.CELULAR,
              tienewhatsapp: $scope.cuentaconwhatsapp,
              telefono: $scope.numerodecelular.toString(),
      ...
      $scope.finalizartabIV = function (text) {
        if ($scope.productosagregadostabIV.length == 0) {
          swal({
            title: "Importante",
            text: "Por favor verificar que existan productos en la Autorización!",
            type: "info",
          });
        } else {
          swal({
            ...
            if (result) {
              $scope.autedit.fechasolicitudparseada = formatDate(
                $scope.autedit.fechasolicitud
              );
              if ($scope.autedit.nopos == true) {
                $scope.autedit.valornopos = "N";
                if (
                  $scope.autedit.tutela == true &&
                  $scope.autedit.ctc == false
                ) {
              ...
              if ($scope.autedit.cuotaanticipo == true) {
                $scope.autedit.valoranticipo = "C";
              } else {
              ...
              if ($scope.valcantidad == true) {
                swal(
                  "Importante",
                  "La cantidad de productos no puede estar vacia",
                  "info"
                );
              } else {
                ...
                $http({
                  ...
                  if ($scope.respuesta.Codigo == "0") {
                    if ($scope.productosagregadostabIV.length > 0) {
                      $scope.dataproductosfre = JSON.stringify(
                        $scope.productosagregadostabIV
                      );
                      $http({
                        method: "POST",
                        ...
                        data: {
                          function: "insertarDetalleAut",
                          productos: $scope.dataproductosfre,
                          cantidad: $scope.productosagregadostabIV.length,
                          numero: $scope.respuesta.Numero,
                          ubicacion: $scope.respuesta.Ubicacion,
                        },
                      }).then(function (response) {
                        ...
                        if (response.data.Codigo == "0") {
                          ...
                          if (
                            ($scope.autedit.cod_adjunto == null ||
                              $scope.autedit.cod_adjunto == "") &&
                            $scope.autedit.ext
                          ) {
                            if ($scope.autedit.ext) {
                          ...
                          if ($scope.autedit.autsoporte == true) {
                            $scope.subirAdjunto(
                              JSON.stringify({
                                ARCHIVO: $scope.v_encabezado.ADJUNTO,
                              })
                            );
                          }
                          ...
                          $http({
                            ...
                            data: {
                              function: "insertarAutorizacionEntregas",
                              autorizacion: JSON.stringify($scope.autedit),
                              productos: $scope.dataproductosfre,
                              cantidad: $scope.productosagregadostabIV.length,
                            },
                          }).then(function (response) {
      ...
      $scope.validartabI = function (tipo) {
        ...
        switch (tipo) {
          case "solicitud":
            ...
            if (
              $scope.solicitud.diagnom1 == "" ||
              $scope.solicitud.diagnom1 == undefined
            ) {
              $scope.validatext = "Complete los campos requeridos (*)";
            ...
            if (
              !$scope.solicitud.fechasolicitud ||
              $scope.solicitud.fechasolicitud == undefined
            ) {
              $scope.validatext = "Complete los campos requeridos (*)";
              $scope.pasarsolicitudaut = false;
            }
            if (
              $scope.solicitud.ipssolicita == "" ||
              $scope.solicitud.ipssolicita == undefined
            ) {
              $scope.validatext = "Complete los campos requeridos (*)";
              $scope.pasarsolicitudaut = false;
            }
            if (
              ...
              $scope.solicitud.ipsasignada == undefined
            ) {
              $scope.validatext = "Complete los campos requeridos (*)";
              $scope.pasarsolicitudaut = false;
            }
            if (
              $scope.solicitud.ipscodsolicita == "" ||
              $scope.solicitud.ipscodsolicita == undefined
            ) {
              $scope.validatext = "Complete los campos requeridos (*)";
              $scope.pasarsolicitudaut = false;
            }
            if (
              $scope.solicitud.ipscodasignada == "" ||
              $scope.solicitud.ipscodasignada == undefined
            ) {
              $scope.validatext = "Complete los campos requeridos (*)";
              $scope.pasarsolicitudaut = false;
            }
            if (
              ...
              $scope.solicitud.servicio == undefined
            ) {
              $scope.validatext = "Complete los campos requeridos (*)";
              $scope.pasarsolicitudaut = false;
            }
            if (
              $scope.solicitud.contrato == "" ||
              $scope.solicitud.contrato == undefined
            ) {
              $scope.validatext = "Complete los campos requeridos (*)";
              $scope.pasarsolicitudaut = false;
            }
            if (
              ...
              $scope.solicitud.origen == undefined
            ) {
              $scope.validatext = "Complete los campos requeridos (*)";
              $scope.pasarsolicitudaut = false;
            }
            if (
              ...
              $scope.solicitud.tiposervicio == undefined
            ) {
              $scope.validatext = "Complete los campos requeridos (*)";
              $scope.pasarsolicitudaut = false;
            }
            if (
              $scope.solicitud.ubicacionpaciente == "" ||
              $scope.solicitud.ubicacionpaciente == undefined
            ) {
              $scope.validatext = "Complete los campos requeridos (*)";
              $scope.pasarsolicitudaut = false;
            }
            ...
            if (
              $scope.solicitud.nombremedico == "" ||
              $scope.solicitud.nombremedico == undefined
            ) {
              $scope.validatext = "Complete los campos requeridos (*)";
              $scope.pasarsolicitudaut = false;
            }
            if (
              $scope.solicitud.especialidadmedico == "" ||
              $scope.solicitud.especialidadmedico == undefined
            ) {
              $scope.validatext = "Complete los campos requeridos (*)";
              $scope.pasarsolicitudaut = false;
            }
            ...
            if ($scope.solicitud.aut == true) {
              if (
                ($scope.solicitud.codaut == "" ||
                  $scope.solicitud.codaut == null) &&
                ($scope.solicitud.ubiaut == "" ||
                  $scope.solicitud.ubiaut == null)
              ) {
                $scope.validatext = "Complete los campos requeridos (*)";
                $scope.pasarsolicitudaut = false;
              }
            ...
            if ($scope.solicitud.pqrds == true) {
              ...
              $scope.solicitud.numero_pqr = "";
              $scope.solicitud.aut_programable = "N";
            }
      ...
      if (dias>90) {
        ...
        $scope.pasarsolicitudaut = false;
          ...
          case "autorizacion":
            if (
              $scope.autedit.diagnom1 == "" ||
              $scope.autedit.diagnom1 == undefined
            ) {
              $scope.validatextab4 = "Complete los campos requeridos (*)";
              $scope.pasarsolicitudaut = false;
            }
            if (
              $scope.autedit.ipssolicita == "" ||
              $scope.autedit.ipssolicita == undefined
            ) {
              $scope.validatextab4 = "Complete los campos requeridos (*)";
              $scope.pasarsolicitudaut = false;
            }
            if (
              $scope.autedit.ipsasignada == "" ||
              $scope.autedit.ipsasignada == undefined
            ) {
              $scope.validatextab4 = "Complete los campos requeridos (*)";
              $scope.pasarsolicitudaut = false;
            }
            if (
              $scope.autedit.ipscodsolicita == "" ||
              $scope.autedit.ipscodsolicita == undefined
            ) {
              $scope.validatextab4 = "Complete los campos requeridos (*)";
              $scope.pasarsolicitudaut = false;
            }
            if (
              $scope.autedit.ipscodasignada == "" ||
              $scope.autedit.ipscodasignada == undefined
            ) {
              $scope.validatextab4 = "Complete los campos requeridos (*)";
              $scope.pasarsolicitudaut = false;
            }
            if (
              $scope.autedit.servicio == "" ||
              $scope.autedit.servicio == undefined
            ) {
              $scope.validatextab4 = "Complete los campos requeridos (*)";
              $scope.pasarsolicitudaut = false;
            }
            if (
              $scope.autedit.contrato == "" ||
              $scope.autedit.contrato == undefined
            ) {
              $scope.validatextab4 = "Complete los campos requeridos (*)";
              $scope.pasarsolicitudaut = false;
            }
            if (
              $scope.autedit.origen == "" ||
              $scope.autedit.origen == undefined
            ) {
              $scope.validatextab4 = "Complete los campos requeridos (*)";
              $scope.pasarsolicitudaut = false;
            }
            if (
              $scope.autedit.tiposervicio == "" ||
              $scope.autedit.tiposervicio == undefined
            ) {
              $scope.validatextab4 = "Complete los campos requeridos (*)";
              $scope.pasarsolicitudaut = false;
            }
            if (
              $scope.autedit.ubicacionpaciente == "" ||
              $scope.autedit.ubicacionpaciente == undefined
            ) {
              $scope.validatextab4 = "Complete los campos requeridos (*)";
              $scope.pasarsolicitudaut = false;
            }
            if (
              $scope.autedit.nombremedico == "" ||
              $scope.autedit.nombremedico == undefined
            ) {
              $scope.validatextab4 = "Complete los campos requeridos (*)";
              $scope.pasarsolicitudaut = false;
            }
            if (
              $scope.autedit.especialidadmedico == "" ||
              $scope.autedit.especialidadmedico == "-" ||
              $scope.autedit.especialidadmedico == undefined
            ) {
              $scope.validatextab4 = "Complete los campos requeridos (*)";
              $scope.pasarsolicitudaut = false;
            }
            ...
            if ($scope.autedit.aut == true) {
              if (
                ($scope.autedit.codaut == "" ||
                  $scope.autedit.codaut == null) &&
                ($scope.autedit.ubiaut == "" || $scope.autedit.ubiaut == null)
              ) {
                $scope.validatextab4 = "Complete los campos requeridos (*)";
                $scope.pasarsolicitudaut = false;
              }
            ...
            if ($scope.autedit.autsoporte == true) {
              if (
                $scope.v_encabezado.ADJUNTO == "" ||
                $scope.v_encabezado.ADJUNTO == null
              ) {
      ...
      $scope.printAut = function (tab) {
        setTimeout(() => {
          ...
          if ($scope.sumPrint > 3) {
            swal({
              title: "No Completado",
              text: "No se puede imprimir la autorización mas de 3 veces!",
              showConfirmButton: true,
              type: "warning",
            });
          } else {
            window.open(
              "views/autorizaciones/formatoautorizacionPrint.php?numero=" +
                $scope.numautprocesada +
                "&ubicacion=" +
                $scope.ubicacionPrint,
              "_blank"
            );
          }
        }, 100);
        ...
        swal({
          ...
          if (result) {
            ...
            if ($scope.pasardatosafiliadoprog == true) {
              $scope.solicitudpro.ubicacion =
                sessionStorage.getItem("municipio");
              $scope.solicitudpro.fecsolicitudparseada = formatDate(
                $scope.solicitudpro.fecsolicitud
              );
                ...
                if ($scope.respuesta.Codigo == "1") {
                  ...
                  $http({
                    ...
                    data: {
                      ...
                      productos: dataproductos,
                      cantidad: $scope.productosagregadostabII.length,
                      numero: $scope.respuesta.Numero,
                      ubicacion: $scope.respuesta.Ubicacion,
                      accion: "I",
                    },
                    ...
                    if (response.data.codigo == "1") {
                      $("#btn-finalizartabI")
                        .removeClass("activebtn-step")
                        .addClass("donebtn-step");
                      ...
                      swal({
                        title: "Importante",
                        text: "Autorización no registrada",
                        type: "info",
                      });
                    }
                  });
      ...
      $scope.validartabII = function (tipo) {
        ...
        switch (tipo) {
          case "solicitud":
            if (
              $scope.solicitudpro.diagnom1 == "" ||
              $scope.solicitudpro.diagnom1 == undefined
            ) {
              $scope.pasarsolicitudautprog = false;
            }
            if (
              $scope.solicitudpro.fecsolicitud == "" ||
              $scope.solicitudpro.fecsolicitud == undefined
            ) {
              $scope.pasarsolicitudautprog = false;
            }
            if (
              $scope.solicitudpro.ipssolicitante == "" ||
              $scope.solicitudpro.ipssolicitante == undefined
            ) {
              $scope.pasarsolicitudautprog = false;
            }
            if (
              $scope.solicitudpro.ipsasignada == "" ||
              $scope.solicitudpro.ipsasignada == undefined
            ) {
              $scope.pasarsolicitudautprog = false;
            }
            if (
              $scope.solicitudpro.ipscodsolicitante == "" ||
              $scope.solicitudpro.ipscodsolicitante == undefined
            ) {
              $scope.pasarsolicitudautprog = false;
            }
            if (
              $scope.solicitudpro.ipscodasignada == "" ||
              $scope.solicitudpro.ipscodasignada == undefined
            ) {
              $scope.pasarsolicitudautprog = false;
            }
            if (
              $scope.solicitudpro.servicio == "" ||
              $scope.solicitudpro.servicio == undefined
            ) {
              $scope.pasarsolicitudautprog = false;
            }
            if (
              $scope.solicitudpro.justificacion == "" ||
              $scope.solicitudpro.justificacion == undefined
            ) {
              $scope.pasarsolicitudautprog = false;
            }
            if ($scope.datatb2.requiredFile == true) {
              $scope.textvalidate =
                $scope.solicitudpro.file == null
                  ? "Soporte de la solicitud no puede estar vacio!"
          ...
          case "producto":
            if (
              $scope.productosagregadostabII.length == 0 ||
              $scope.productosagregadostabII == undefined
            ) {
              $scope.pasarproductoautprog = false;
            }
            break;
          case "datosafiliado":
            if (
              $scope.solicitudpro.celular == "" ||
              $scope.solicitudpro.celular == undefined
            ) {
              $scope.pasardatosafiliadoprog = false;
            }
            if (
              $scope.solicitudpro.direccion == "" ||
              $scope.solicitudpro.direccion == undefined
            ) {
              $scope.pasardatosafiliadoprog = false;
            }
            break;
        ...
        if (find != undefined) {
          if (find.length > 4) {
            $http({
              ...
              if (
                response.data["0"].CODIGO != "0" &&
                response.data["0"].CODIGO != "-1"
              ) {
                $scope.listProductos = response.data;
      ...
      $scope.configPages = function () {
        ...
        if (ini < 1) {
          ini = 1;
          if (
            Math.ceil($scope.listProductosTemp.length / $scope.pageSize) >
            $scope.valmaxpag
          )
            fin = 10;
          else
      ...
      $scope.setPage = function (index) {
        ...
        if ($scope.listProductosTemp.length % $scope.pageSize == 0) {
          var tamanomax = parseInt(
            $scope.listProductosTemp.length / $scope.pageSize
          );
        } else {
          var tamanomax =
            parseInt($scope.listProductosTemp.length / $scope.pageSize) + 1;
        }
      ...
      $scope.buscarAutorizaciones = function (
        ...
        $http({
          ...
          setTimeout(function () {
            ...
            document
              .getElementById("tautorizaciones")
          ...
          setTimeout(function () {
            ...
            document
              .getElementById("tautorizacionespro")
              .scrollIntoView({ block: "start", behavior: "smooth" });
...
swal.close();
          ...
          if (accion == "E") {
            ...
            if ($scope.v_detalle.length > 0) {
              for (
                ...
                $scope.productosagregadostabIV[index].CANTIDADN =
                  $scope.productosagregadostabIV[index].CANTIDAD;
              }
            } else {
              $scope.productosagregadostabIV = [];
            }
            ...
            console.log($scope.v_detalle);
            $scope.cargarContratoTabI(
              $scope.v_encabezado.NIT_ASIGNADA,
              $scope.regimentabIV,
              "tab4"
            ); //faltan parametros.
            setTimeout(() => {
              ...
              $scope.autedit.numero = $scope.v_encabezado.NUM_OASIS;
              $scope.autedit.ubicacion = $scope.v_encabezado.UBI_OASIS;
              $scope.autedit.tipodocumento =
                $scope.infoafiliadoautedit.TipoDocumento;
              $scope.autedit.documento = $scope.infoafiliadoautedit.Documento;
              $scope.autedit.diagnom1 =
                $scope.v_encabezado.DX + " - " + $scope.v_encabezado.NOMBRE_DX;
              $scope.autedit.diagnom2 =
                $scope.v_encabezado.DX1 == "N"
                  ? ""
                  : $scope.v_encabezado.DX1 +
                    " - " +
                    $scope.v_encabezado.NOMBRE_DX1;
              $scope.autedit.diagcod1 = $scope.v_encabezado.DX;
              $scope.autedit.diagcod2 = $scope.v_encabezado.DX1;
              $scope.autedit.ipsasignada = $scope.v_encabezado.ASIGNADA;
              $scope.autedit.ipscodasignada = $scope.v_encabezado.NIT_ASIGNADA;
              ...
              $scope.autedit.ubicacioncontrato =
                $scope.v_encabezado.UBICACION_CONTRATO;
              $scope.autedit.documentocontrato = $scope.v_encabezado.CONTRATO;
              $scope.autedit.servicio = $scope.v_encabezado.CLASIFICACION;
              $scope.autedit.codservicio = $scope.v_encabezado.CLASIFICACION;
              $scope.autedit.fechasolicitud = new Date(
                ftemp[2],
                ftemp[1] - 1,
                ftemp[0]
              );
              ...
              $scope.autedit.tiposervicio = $scope.v_encabezado.TIPO_SERVICIO;
              $scope.autedit.ubicacionpaciente =
                $scope.v_encabezado.UBICACION_SOL;
              $scope.autedit.ipssolicita = $scope.v_encabezado.SOLICITANTE;
              $scope.autedit.ipscodsolicita =
                $scope.v_encabezado.NIT_SOLICITANTE;
              $scope.buscarEspecialidades($scope.v_encabezado.NIT_SOLICITANTE);
              $scope.autedit.nombremedico = $scope.v_encabezado.MEDICO;
              $scope.autedit.especialidadmedico =
                $scope.v_encabezado.ESPECIALIDAD +
                " - " +
                $scope.v_encabezado.ESPECIALIDAD_MEDICO;
              $scope.autedit.codespecialidad = $scope.v_encabezado.ESPECIALIDAD;
              $scope.autedit.observacion = $scope.v_encabezado.OBSERVACION;
              $scope.autedit.codaut =
                $scope.v_encabezado.AUTORIZACION_ANTERIOR == 0
                  ? ""
                  : $scope.v_encabezado.AUTORIZACION_ANTERIOR;
              $scope.autedit.nopos =
                $scope.v_encabezado.POSS == "TRUE" ? true : false;
              $scope.autedit.valornopos = "";
              $scope.autedit.valortipo = "";
              $scope.autedit.cod_adjunto = $scope.v_encabezado.COD_ADJUNTO;
              ...
              // $scope.autedit.valorctc = $scope.v_encabezado.NUM_CTC
              // $scope.autedit.valormipres= $scope.autedit.valorctc;
              $scope.autedit.tutela =
                $scope.v_encabezado.TUTELA == "TRUE" ? true : false;
              $scope.autedit.valortutela = $scope.v_encabezado.NUM_TUTELA;
              $scope.autedit.anticipo =
                $scope.v_encabezado.ANTICIPO == "TRUE" ? true : false;
              $scope.autedit.valoranticipo = "";
              $scope.autedit.siniestro =
                $scope.v_encabezado.SINIESTRO == "TRUE" ? true : false;
              $scope.autedit.valorsiniestro = "";
              $scope.autedit.altocosto = "";
              $scope.autedit.prioridad =
                $scope.v_encabezado.PRIORIDAD == "TRUE" ? true : false;
              $scope.autedit.valorprioridad = "";
              $scope.autedit.accion = "U";

              $scope.autedit.cuotaanticipo = $scope.v_encabezado.CAJA_SALUD == 'TRUE' ? true : false;


              $scope.setParametros(
                "noposp",
                $scope.infoafiliadoautedit.TipoDocumento,
                $scope.infoafiliadoautedit.Documento
              );
            }, 100);
          }
          swal.close();
      ...
      $scope.accionesAutorizacion = function (
        opcion,
        operacion,
        numero,
        ubicacion
        ...
        $scope.busquedaXdetalles.closePromise.then(function (response) {
          ...
          if (response.value != "$closeButton") {
            ...
            if ($scope.tipoaut == "2") {
              ...
              $scope.buscarAfiliado(
                "2",
                $scope.solicitudpro.tipodocumento,
                $scope.solicitudpro.documento
              );
            }
            if ($scope.tipoaut == "4") {
              $scope.buscarAutorizaciones($scope.type, $scope.id, "", "");
            }
          }
        });
      };
      ...
      $scope.seleccionaranodias = function () {
        if ($scope.tipoaut == "1") {
          ...
          if (
            $scope.solicitud.anopass == "0" ||
            $scope.solicitud.anopass == undefined ||
            $scope.solicitud.mespess == "0" ||
            $scope.solicitud.mespess == undefined
          ) {
            ...
            swal({
              title: "Completado",
              text: "Año y Mes de control selecionado Correctamente!",
              showConfirmButton: false,
              type: "success",
              timer: 1500,
            });
        ...
        if ($scope.tipoaut == "4") {
          ...
          if (
            $scope.autedit.anopass == "0" ||
            $scope.autedit.anopass == undefined ||
            $scope.autedit.mespess == "0" ||
            $scope.autedit.mespess == undefined
          ) {
            swal("Importante", "No pueden haber campos vacios!", "info");
          } else {
            setTimeout(() => {
      ...
      //Functions in jquery
      $("#autfile").change(function () {
        //Detecta los cambios que sufre el input file
        $timeout(function () {
          ...
          $scope.data.formato = "";
          if (file) {
            //Valida si existe el archivo en el input file
            if (file.size <= 5000000) {
              //valida que el size del file sea <= 5 mb
              if (file.name.split(".").pop().toLowerCase() == "pdf") {
                //Obtiene la extension del file y valida que sea un pdf
                $scope.data.formato =
                  "Dentro Del Peso Limite y Formato Validado";
                $scope.data.requiredFile = false;
                var reader = new FileReader();
                reader.onload = function (event) {
                  $timeout(function () {
                    $scope.solicitud.file = event.target.result; //Asigna el file al ng-model autFile
                    $scope.solicitud.ext = file.name
                      .split(".")
                      .pop()
                      .toLowerCase(); //Asigna la extension del autFile
                    $scope.solicitud.namefile = file.name.substr(
                      0,
                      file.name.lastIndexOf(".")
                    ); //Asigna el nombre del autFile
                  });
                };
                reader.readAsDataURL(file);
              } else {
                $scope.data.formato =
                  "Formato Invalido solo puedes adjuntar archivos con extensión .pdf";
                $scope.solicitud.namefile = null; //Asigna null al ng-model autFile
                $scope.solicitud.ext = null; //Asigna null a la extension autFile
                $scope.data.requiredFile = true;
              }
              ...
              if (file.name.split(".").pop().toLowerCase() == "pdf") {
                ...
                reader.onload = function (event) {
                  $timeout(function () {
                    ...
                    $scope.autedit.ext = file.name
                      .split(".")
                      .pop()
                      .toLowerCase(); //Asigna la extension del autFile
                    $scope.autedit.namefile = file.name.substr(
                      0,
                      file.name.lastIndexOf(".")
                    ); //Asigna el nombre del autFile
                  });
              ...
              if (file.name.split(".").pop().toLowerCase() == "pdf") {
                ...
                reader.onload = function (event) {
                  $timeout(function () {
                    ...
                    $scope.solicitudpro.ext = file.name
                      .split(".")
                      .pop()
                      .toLowerCase(); //Asigna la extension del autFile
                    $scope.solicitudpro.namefile = file.name.substr(
                      0,
                      file.name.lastIndexOf(".")
                    ); //Asigna el nombre del autFile
                  });
      ...
      $scope.abrirModalDireccion = function () {
        $scope.dialogDiagreg = ngDialog.open({
          template:
            "views/consultaAfiliados/nucleofamiliar/modal/modalDireccion.html",
          className: "ngdialog-theme-plain",
      ...
      $scope.buscarprescripcion = function () {
        ...
        $http({
          ...
          data: {
            ...
            tipodocumento: temptipodocumento,
            documento: tempdocumento,
            valormipres: $scope.numero_prescripcion,
          },
        }).then(function (response) {
          console.log(response.data);
      ...
      $scope.saltarcontrolfrecuencia = function (tipo) {
        if ($scope.motivo == null || $scope.motivo == "") {
          swal({
            title: "Motivo para saltar contrl de frencuencia",
      ...
      $scope.avisoFechaCita = function () {
        if ($scope.contadorSwal == 0) {
          swal({
            title: "NOTIFICACION",
            text: "ESTA FECHA NO DEBE SUPERAR LOS 5 DIAS",
            showConfirmButton: true,
            type: "warning",
          });
        }
