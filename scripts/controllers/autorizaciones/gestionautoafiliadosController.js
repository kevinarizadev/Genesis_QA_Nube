...
angular.module('GenesisApp')
  ...
  .controller('gestionautoafiliadosController', ['$scope', '$http', '$filter', 'ngDialog', '$timeout', 'pqrHttp', function ($scope, $http, $filter, ngDialog, $timeout, pqrHttp) {
    ...
    $scope.ubicacion = $scope.viewseccional.toString().substr(0, 2);
    $scope.ubicaciontemporalmunicipio = $scope.ubicacion + '00';
    ...
    $scope.solicitudesVencidas = function (seccional) {
      $http({
        method: 'POST',
        url: "php/autorizaciones/afiliado/solicitud_autorizacion.php",
        data: {
          function: 'Obtener_Seguimiento', municipio: seccional.codigo
        }
      }).then(function (response) {
        if (response.data && response.data.toString().substr(0, 3) != '<br') {
          $scope.datovencidas = response.data[0];
    ...
    $scope.validatecallIps = function (seccional) {
      ...
      if (seccional) {
        $scope.solicitudesVencidas(seccional);
    ...
    $scope.openmodals = function (tipo, opcion, other) {
      ...
      switch (tipo) {
        ...
        case 'modalgestionpro':
          $scope.tempAut = other;
          $scope.archivocargadosolicitud = other.ADJUNTO.replace(/\r?\n$/, '');
          swal({
            html: '<div class="loading"><div class="default-background"></div><div class="default-background"></div><div class="default-background"></div></div><p style="font-weight: bold;">Cargando...</p>',
            width: 200,
            allowOutsideClick: false,
          ...
          $http({
            method: 'POST',
            url: "php/autorizaciones/afiliado/solicitud_autorizacion.php",
            data: { function: 'descargarfile', ruta: $scope.archivocargadosolicitud }
          }).then(function (response) {
    ...
    $scope.verdocumentoabjunto = function (ruta, ftp) {
      if (ftp == 1) {
        $http({
          method: 'POST',
    ...
    $scope.VerPendientes = function () {
          ...
          $http({
            method: 'POST',
            url: "php/autorizaciones/afiliado/solicitud_autorizacion.php",
            data: { function: 'llamar_Listado_Pendientes', municipio: $scope.tempSeccionalips.codigo }
          }).then(function (response) {
            $scope.listaSolicitudesPendientes= response.data;
            swal.close();
            $("#modalseguimiento").modal("open");
          });
      }
      ...
      if (state == 'P' && aut.activo == 0) {
        ...
        if (state == 'G' || state == 'X') {
          ...
          $http({
            ...
            console.log($scope.listaSolicitudesprog.length);
    ...
    // $scope.seleccionar_gestion = function () {
    //   if($scope.check_gestion == 1){
    //     $scope.check_gestionselec = 1;
    //   }else if($scope.check_gestion == 2){
    //     $scope.check_gestionselec = 2;
    //   }else {
    //     $scope.check_gestionselec = 3;
    //   }
    // }
    $scope.cambiarmotivos = function(tipomotivo){
      $scope.listMotivos =[];
        if(tipomotivo == 2){
          // $scope.nuevalistamotivo = $scope.listMotivos;
          $scope.listMotivos = [
            {
              "CODIGO": 100,
              "NOMBRE": "Capitado"
    ...
    $scope.saveanulacion = function () {
      let accionenviar;
      if ($scope.check_gestion == 2) {
        accionenviar = 'N';
       
      }else if ($scope.check_gestion == 3){
    ...
    $scope.guardargestionrechazo = function(solicitud,check_gestion){
      $scope.nuevalistamotivo = $scope.listMotivos2;
      if(check_gestion == '2'){
        $scope.nuevalistamotivo = $scope.listMotivos2;
      ...
      swal({
        ...
        <div class="input-field col s12" style="padding:0">
                                ...
                                <select id="motivoanulacion2" value="${this.CODIGO}" class="select-chosen-eps">
                                  <option value="">SELECCIONAR MOTIVO</option>
                                  ${$scope.listadodemotivos.map(dat => `<option value="${dat.CODIGO}">${dat.NOMBRE}</option>`)}
                                </select>
                              </div>
                                <div class="input-field col s12" style="padding:0">
    ...
    $scope.VerPendientesPorAnular = function () {
      swal({
        html: '<div class="loading"><div class="default-background"></div><div class="default-background"></div><div class="default-background"></div></div><p style="font-weight: bold;">Cargando...</p>',
        width: 200,
        allowOutsideClick: false,
    ...
    $scope.savegenerarauto = function (datos) {
      $scope.tempAut = datos;
      swal({
        title: 'Confirmar',
        text: '¿Esta seguro de haber verificado la documentacion perfectamente?',
        ...
        if ($scope.check_gestion == 1) {
          localStorage.setItem('solicitud_solicitud', JSON.stringify($scope.tempAut));
          $scope.closemodals('modalseguimiento');
          location.href = "#/gestionautorizacion";
        } else {
          $scope.saveanulacion();
        }
    ...
    $scope.retroceder = function () {
      $scope.validatecall($scope.ubicaciontemporalmunicipio);
