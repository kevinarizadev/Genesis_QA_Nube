...
angular.module('GenesisApp')
  ...
  .controller('gestionautprogramadaController', ['$scope', '$http', '$filter', 'ngDialog', '$timeout', 'pqrHttp', function ($scope, $http, $filter, ngDialog, $timeout, pqrHttp) {
    ...
    $scope.buscarAutorizaciones = function (tipo_documento,documento, numero, ubicacion, nit) {
      var vtext = "";
      if ($scope.filterOptions == 'AFILIADO') {
        ...
        ubicacion = 0;
        nit = 0;
        if (documento == undefined || documento == null ||tipo_documento== undefined || tipo_documento == null) {
      ...
      if ($scope.validatesearch == true) {
        ...
        $http({
          method: 'POST',
          url: "php/autorizaciones/autorizacionprog/funcautorizacion.php",
          data: { function: 'obtener_aut_auditor',tipo_documento:tipo_documento, documento: documento, numero: numero, ubicacion: ubicacion, nit: nit, usu_ubicacion: sessionStorage.getItem('municipio') }
        }).then(function (response) {
