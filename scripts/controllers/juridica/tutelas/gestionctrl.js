...
angular.module('GenesisApp')
  .controller('gestionctrl', ['$scope', '$http', 'consultaHTTP', 'ngDialog', 'cfpLoadingBar',
    function ($scope, $http, consultaHTTP, ngDialog, cfpLoadingBar) {
      ...
      $scope.agregarAfiliados = function () {

        if (!$scope.registro.nombreafiliado) {
          return
        }

        $scope.listadoAfiliados.push({
          tipoidafiliado: $scope.registro.tipoidafiliado,
          idafiliado: $scope.registro.idafiliado,
          nombreafiliado: $scope.registro.nombreafiliado
        })

        $scope.registro.tipoidafiliado = ''
        $scope.registro.idafiliado = ''
        $scope.registro.nombreafiliado = ''
      ...
      $scope.quitarAfiliados = function (index) {
        $scope.listadoAfiliados.splice(index, 1);
        setTimeout(() => { $scope.$apply(); }, 500);
      }
      ...
      $scope.registraTutela = function () {
        ...
        if (!$scope.hdeIdafiliado && $scope.listadoAfiliados.length == 0 && !$scope.registro.tipoidafiliado && !$scope.registro.idafiliado && !$scope.registro.nombreafiliado) {
          swal('Información', 'Diligencia el accionante', 'info').catch(swal.noop); $scope.btn.GuardaRegistro = false; return
        }
        if ($scope.juzgado.codigo == undefined || $scope.juzgado.codigo == "") {
        ...
        if ($scope.listadoAfiliados.length == 0) {
          ...
          for (let i = 0; i < $scope.listadoAfiliados.length; i++) {
            $scope.registro.tipoidafiliado = $scope.listadoAfiliados[i].tipoidafiliado;
            $scope.registro.idafiliado = $scope.listadoAfiliados[i].idafiliado;
      ...
      $scope.changeFallo = function () {
        ...
        $scope.detalles.decicion_primera_instancia = ''
        // if ($scope.detalles.fallotutela == true) {
        // 	$scope.hdeFechaFallo = true;
        // } else {
        // 	$scope.hdeFechaFallo = false;
          ...
          if (result) {
            ...
            $http({
              ...
              data: {
                ...
                constutela: $scope.registro.codigotutela,
                ubicacion: $scope.registro.ubicacion,
                type: $scope.archivorespapertpruext,
      ...
      $scope.registradecisionincidente = function () {
        ...
        $scope.registro.decision_incidente = {
          sancionar_decision: $scope.registro.chkDecisionInci_Sancionar == true ? 'S' : 'N',
          cuantos_dias: decisionInci_CuantosDias,
          valor_multa: decisionInci_ValorMulta
        }

        // $scope.registro.decision_incidente.sancionar_decision = $scope.registro.chkDecisionInci_Sancionar;
        // $scope.registro.decision_incidente.cuantos_dias = decisionInci_ValorMulta;
        // $scope.registro.decision_incidente.valor_multa = decisionInci_CuantosDias;
      ...
      $scope.chkconsultaDecision_Sancionar = function () {
        if ($scope.detalles.decision_decisioninc == '1' || $scope.detalles.decision_decisioninc == '3') {
          if ($scope.registro.decision_incidente == null) {
            return
          }
          $scope.registro.chkConsultaInci_Sancionar = $scope.registro.decision_incidente.sancionar_decision == 'S' ? true : false;
          $scope.registro.ConsultaInci_CuantosDias = parseInt($scope.registro.decision_incidente.cuantos_dias);
          $scope.registro.ConsultaInci_ValorMulta = $scope.registro.decision_incidente.valor_multa;

          if ($scope.registro.ConsultaInci_CuantosDias) {
            $scope.chkConsultaInci_CuantosDias = true;

          }
          if ($scope.registro.ConsultaInci_ValorMulta) {
            $scope.chkConsultaInci_ValorMulta = true;
          }
        } else {
          $scope.registro.chkConsultaInci_Sancionar = false;
          $scope.registro.ConsultaInci_CuantosDias = ''
          $scope.registro.ConsultaInci_ValorMulta = ''
          $scope.chkConsultaInci_CuantosDias = false;
          $scope.chkConsultaInci_ValorMulta = false;
        }
        setTimeout(() => { $scope.$apply(); }, 500);
      }
      ...
      $scope.registraconsultaincidente = function () {
        if ($("#FechaConInc").val() == "") {
          ...
          const consultaInci_CuantosDias = $scope.chkConsultaInci_CuantosDias ? $scope.registro.ConsultaInci_CuantosDias : ''
          const consultaInci_ValorMulta = $scope.chkConsultaInci_ValorMulta ? ($scope.registro.ConsultaInci_ValorMulta.toString().replace(/\./g, '')).replace(/\,/g, '.') : ''
            ...
            if (result) {
              ...
              $http({
                ...
                data: {
                  ...
                  sancionar_decision: $scope.registro.chkConsultaInci_Sancionar ? 'S' : 'N',
                  CuantosDias: consultaInci_CuantosDias,
                  ValorMulta: consultaInci_ValorMulta
