...
.controller('admndatosempleadoscontroller', ['$scope', '$http', 'altocostoHttp','ngDialog', 'notification', '$timeout', '$q', 'upload', 'communication', '$controller','$rootScope','$window','$filter',
  function ($scope, $http, altocostoHttp, ngDialog, notification, $timeout, $q, upload, communication, $controller, $rootScope, $window,$filter) {
...
$scope.fechaingreso = "";
$scope.fechaegreso =  "";
//Jair Molina Control Cambio
$scope.puesto="0";
$scope.observacion="";
$scope.causalegreso="0";
$scope.causalingreso="0";
...
$scope.changeempresarial = function(){
  if($scope.cardempresarial == false){
    ...
    $scope.OcultarCausalIngreso=false;
    $scope.OcultarCausalEgreso=true;
...
$scope.setNewData =  function(){
  ...
  if($scope.newData.puesto_trabajo == null){$scope.puesto = "0";}else{$scope.puesto = $scope.newData.puesto_trabajo;}
  if($scope.newData.causal_ingreso == null){$scope.causalingreso = "0";}else{$scope.causalingreso = $scope.newData.causal_ingreso;}
  if($scope.newData.causal_egreso == null){$scope.causalegreso = "0";}else{$scope.causalegreso = $scope.newData.causal_egreso;}

  $scope.observacion=$scope.newData.observacion;
}
...
$scope.cargardatosbasicos =  function(desicion){
  ...
  $scope.documento  = $scope.qrnumerob;
  //$scope.tipoDoc=$scope.qrtipotemp;
  //$scope.documento=$scope.qrnumerobtemp;
  setTimeout(function () {
  ...
  if($scope.camposvalidos == true){
    ...
    $http({
     ...
     data: {
       ...
       tipo:tipo,
       puestotrabajo:$scope.puesto,
       causalingreso:$scope.causalingreso,
       causalegreso:$scope.causalegreso,
...
$scope.limpiartodo2 =  function(x){
  ...
  $("#-municipios").removeClass("requerido");

  $("#-departamentoes").removeClass("requerido");
  $("#-municipioes").removeClass("requerido");
  ...
  $("#-banco").removeClass("requerido");
    $("#-puesto").removeClass("requerido");
  $("#-causalingreso").removeClass("requerido");
  $("#-causalegreso").removeClass("requerido");
  $("#-observacion").removeClass("requerido");
  $("#-fechaegreso").removeClass("requerido");
  $("#-fechaingreso").removeClass("requerido");
...
$scope.validarcampo =  function(opcion){
  ...
  switch (opcion) {
    case "1":
    if($scope.departamentoe == "" || $scope.departamentoe == "0" || $scope.departamentoe == null || $scope.departamentoe == undefined || $scope.departamentoe == " "){$scope.camposvalidos = false; $("#-departamentoes").addClass("requerido");}else{$("#-departamentoes").removeClass("requerido");};
    if($scope.municipioe == "" || $scope.municipioe == "0" || $scope.municipioe == null || $scope.municipioe == undefined || $scope.municipioe == " "){$scope.camposvalidos = false; $("#-municipioes").addClass("requerido");}else{$("#-municipioes").removeClass("requerido");};
    ...
    if($scope.puesto == "" || $scope.puesto == "0" || $scope.puesto == null || $scope.puesto == undefined || $scope.puesto == " "){$scope.camposvalidos = false; $("#-puesto").addClass("requerido");}else{$("#-puesto").removeClass("requerido");};
    if($scope.causalingreso == "" || $scope.causalingreso == "0" || $scope.causalingreso == null || $scope.causalingreso == undefined || $scope.causalingreso == " "){$scope.camposvalidos = false; $("#-causalingreso").addClass("requerido");}else{$("#-causalingreso").removeClass("requerido");};
    ...
    case "2":
          ...
          if($scope.salario == "" || $scope.salario == "0" || $scope.salario == null || $scope.salario == undefined || $scope.salario == " "){$scope.camposvalidos = false; $("#-salario").addClass("requerido");}else{$("#-salario").removeClass("requerido");};
          if($scope.puesto == "" || $scope.puesto == "0" || $scope.puesto == null || $scope.puesto == undefined || $scope.puesto == " "){$scope.camposvalidos = false; $("#-puesto").addClass("requerido");}else{$("#-puesto").removeClass("requerido");};
          if($scope.estado=="R" && $scope.causalegreso == "" ||
            $scope.estado=="R" && $scope.causalegreso == "0" ||
            $scope.estado=="R" && $scope.causalegreso == null ||
            $scope.estado=="R" && $scope.causalegreso == undefined ||
            $scope.estado=="R" && $scope.causalegreso == " "){$scope.camposvalidos = false; $("#-causalegreso").addClass("requerido");}else{$("#-causalegreso").removeClass("requerido");};
          if($scope.tiporemuneracion == "" || $scope.tiporemuneracion == "0" || $scope.tiporemuneracion == null || $scope.tiporemuneracion == undefined || $scope.tiporemuneracion == " "){$scope.camposvalidos = false; $("#-tiporemuneracion").addClass("requerido");}else{$("#-tiporemuneracion").removeClass("requerido");};
          if($scope.estado == "" || $scope.estado == "0" || $scope.estado == null || $scope.estado == undefined || $scope.estado == " "){$scope.camposvalidos = false; $("#-estado").addClass("requerido");}else{$("#-estado").removeClass("requerido");};
          //if($scope.estado == "R"){$scope.camposvalidos = false; $("#-fechaegreso").addClass("requerido");}else{$("#-fechaegreso").removeClass("requerido");};
          if($scope.estado == "R"){$scope.validarfecha($scope.fechaingreso,$scope.fechaegreso);}else{$("#-fechaegreso").removeClass("requerido");};
      ...
      $scope.validarfecha=function(fechainicial, fechafinal){

       fechainicial = new Date(fechainicial.split("/").reverse().join("-"));
       fechafinal = new Date(fechafinal.split("/").reverse().join("-"));
       if (fechafinal==null || fechafinal==undefined || fechafinal=='' || fechafinal=='Invalid Date') {
          $("#-fechaegreso").addClass("requerido");
          notification.getNotification('warning', 'Debe Ingresar La Fecha De Egreso', 'Notificacion');
       } else {
        if (fechafinal < fechainicial) {
          $scope.camposvalidos = false;
          $("#-fechaegreso").addClass("requerido");
          swal('Notificacion','La fecha de egreso debe ser superior a la fecha de ingreso','warning');
        }else{
          $("#-fechaegreso").removeClass("requerido");
        }
      }
    ...
    $scope.CambioEstado=function(caso){
      if (caso=='R'){
        $scope.OcultarCausalIngreso=true;
        $scope.OcultarCausalEgreso=false;
      ...
      $scope.DescargarReporte = function () {
        swal({
          ...
          window.open("php/talentohumano/ReportesNovedades.php?periodo=" + result);
        })
      }
