...
angular.module('GenesisApp')
  .controller('datosempleadoscontroller', ['$scope', '$http', 'afiliacionHttp', '$window',
    function ($scope, $http, afiliacionHttp, $window) {
        ...
        $http({
          ...
          if (response.data.toString().substr(0, 3) != '<br') {
            if (response.data.estado == '1') {
              $scope.Tipo_de_Contrato = response.data.tipo_contrato;
              ...
              if(response.data.nacimiento.length > 1){
                ...
                $scope.Hoja1.Nacimiento = new Date();
              }
      ...
      $scope.generaCertLaboral = function(){
          if($scope.Tipocertificado == "" || $scope.Tipocertificado == undefined || $scope.Tipocertificado ==null ){
              swal('Notificacion', "Por Favor Seleccione Tipo de Certificado. ", 'info');
            } else {
                $window.open('views/talentohumano/certificados/imprimircertificadol.php?documento='+$scope.Rol_Cedula+'&tipocertificado='+$scope.Tipocertificado+'&inter='+$scope.interesado,'_blank', "width=1080,height=1100");
            }
      }
