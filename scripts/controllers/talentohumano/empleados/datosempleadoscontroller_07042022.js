...
.controller('datosempleadoscontroller', ['$scope', '$http', 'consultaHTTP','afiliacionHttp','altocostoHttp','ngDialog', 'notification', '$timeout', '$q', 'upload', 'communication', '$controller','$rootScope','$window',
function ($scope, $http,consultaHTTP,afiliacionHttp, altocostoHttp, ngDialog, notification, $timeout, $q, upload, communication, $controller, $rootScope, $window) {
  (function() {
	    $("form").on("change", ".file-upload-field", function(){
         ...
         if($(this)["0"].files.length > 0){
           $(this).parent(".file-upload-wrapper").attr("data-text", nombre+'.'+$scope.ext);
           var fileToLoad = $(this)["0"].files[0];
           var fileReader = new FileReader();
           fileReader.onload = function(fileLoadedEvent) {
              $scope.base64 = fileLoadedEvent.target.result;
              $scope.inactiveadjunto = false;
              $scope.$apply();
           }; 
           fileReader.readAsDataURL(fileToLoad);
         }else{
               $(this).val("");
               $scope.inactiveadjunto = true;
               notification.getNotification('info', 'Por favor subir un adjunto!', 'Notificacion');
           }
      });
  ...
  //  funciones del modal  para el estudio
  $scope.filtrar_datosBasicos = function (tipo) {
    $scope.Listardiagnosticos = '';
    $scope.tipo = tipo;
    if (tipo == 1) {
      $scope.nombre_tipo = "Selecciona la institución "
    } else {
      $scope.nombre_tipo = "Selecciona el Programa"
    }
...
$scope.cargarDiagnosticos = function (texto) {
    ...
    if (($scope.coincidencia1 != "" && $scope.coincidencia1.length > 3)) {
      ...
      var tipo ='instituciones';
        $http({
          method:'POST',
          url:"php/talentohumano/datosbasicos/Rdatosbasicos.php",
          data: {function:"obtenerselects",tipo:tipo,valor:texto}
        }).then(function(response){
        ...
        //$scope.Listardiagnosticos = $scope.estudio.programa;
        angular.forEach($scope.estudio.programa, function (programa) {

          if (programa.Nombre.toUpperCase().indexOf(texto.toUpperCase()) >= 0 || programa.Codigo.toString().indexOf(texto) >= 0) {
          
          output.push ({ "Codigo": programa.Codigo, "Nombre": programa.Nombre.toUpperCase() }) ;
          
          }
        });

        $scope.Listardiagnosticos = output;
  ...
  $scope.removeSeleccion = function () {
    ...
    if ($scope.tipo == 1) {
      $('#DM' + $scope.institucion).removeClass('eleacti');
      $scope.institucion = "0";
      $scope.institucion_nombre = "";
    } else {
      $('#DM' + $scope.programa).removeClass('eleacti');
      $scope.programa = "0";
  ...
  $scope.elegir_diagnostico = function (codigo, nombre) {
    ...
    // $scope.hovering=true;
    if ($scope.tipo == 1) {
      $scope.institucion = codigo;
      $scope.institucion_nombre = nombre;
    } else {
      $scope.programa = codigo;
      $scope.programa_nombre = nombre;
    }
  }
  ...
  $scope.GuardarDireccion=function(ViaPrincipal,NumViaPrincipal,Letra,Numero,Bis,Cuadrante,NumeroVG,SelectLetraVG,NumeroPlaca,CuadranteVG,Complemento){
    ...
    $scope.closeThisDialog($('#barrios').val());
  }
  //
  // $scope.openmodaldireccion =  function(){
  //   ngDialog.open({
  ...
  $scope.accionesestudio =  function(data){
    ...
    if(document.getElementById('est-'+data.renglon).checked == true){
      ...
      $scope.programa_nombre = data.programa;
      $scope.institucion_nombre = data.institucion;
      $scope.nivel = data.codnivel;
      $scope.tipoperiodoe = data.codtipoperiodo;
      $scope.periodo = data.codperiodo;
      $scope.ubicacione = data.codubicacion.length==4?'0'+data.codubicacion:data.codubicacion;
    ...
    if(tipo == 'insert'){
      ...
      if($scope.campobenvalido == true){
        ...
        ngDialog.open({
		  ...
		  $scope.docbtemp = $scope.tipodocb;
          $scope.idbtemp = $scope.documentob;
		  $('#modalaadjunto').modal('open');
      }else{
...
$scope.closemodal =  function(){
  $('#modalaadjunto').modal("close");
}
$scope.subiradjuntotemp =  function(){
  var today = new Date();
  ...
  $scope.nuevonombre = '131_'+$scope.docbtemp+"_"+$scope.idbtemp+"_"+ dd + mm + yyyy+ hora + min + seg;
  $scope.doct = 131;
  $scope.obs = 'beneficiario de '+$scope.idbtemp;
  $http({
      method: 'POST',
      url: "php/talentohumano/datosbasicos/Cdatosbasicos.php",
      data: {function:'cargarAdjunto',
             file:$scope.base64,
             ext:$scope.ext,
             name:$scope.nuevonombre,
             documento:$scope.doct,
             obs:$scope.obs,
             cedula:$scope.documentoa,
             tipodocumento:$scope.tipodocumentoa,
             tipodoc:$scope.docbtemp,
             doc:$scope.idbtemp}
  }).then(function(response) {
    ...
    if(response.data == "1"){
      ...
      notification.getNotification('error', 'Documento no anexado!', 'Notificacion');
    }
  })
}
}])
