...
Session_Start();
	...
	function ObtenerVariables(){
		...
		// $cedula = 1000774553;
		$res=273;	
		$consulta = oci_parse($c,'BEGIN PQ_GENESIS_CAC.P_OBTENER_DATOS_CAC(:v_pempresa,:v_ptipodocumento,:v_pdocumento,:v_presolucion,:v_json_row); end;');
	...
	function InsertarDatos(){
		...
		$consulta = oci_parse($c,'begin PQ_GENESIS_CAC.P_INSERTA_CAC_VIH(:v_pjson,:v_parea,:v_prespuesta,:v_json_row,:v_pvalidacion); end;');
...
// 	if (ftp_mkdir($con_id, $path3)) {					
// 		if (ftp_mkdir($con_id, $path2)) {
// 			$subio=ftp_put($con_id, $path3.'/'.$tmpfile, $tmpfile, FTP_BINARY);
// 			if ($subio) {
// 							unlink($tmpfile);
// 							return $db_name;
...
// 	global $request;
// 	$nomadj= $request->nomadj;
// 	$BaseArchivo= $request->BaseArchivo;
// 	$ext= $request->ext;	
// 	$cedula = '1143450658';		
// 	$ruta = "/cargue_ftp/Digitalizacion/Genesis/CAC/0273/";
// 	$day = date("dmY");
// 	$path_of_storage = "/cargue_ftp/Digitalizacion/Genesis/CAC/0273/".$day.'/'.$cedula.'/';
// 	if (is_dir('ftp://ftp_genesis:Senador2019!@192.168.50.10/'.$path_of_storage) == TRUE) {
// 		subirdo($ext,$cedula,$con_id,$nomadj,$BaseArchivo,$ruta,$path_of_storage);
// 	}else{
