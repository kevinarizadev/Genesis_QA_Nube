...
function Obtener_Codigo()
{
	...
	global $request;
	$consulta = oci_parse($c, 'begin oasis.pq_genesis_tic.p_valida_datos_certificados( :v_proceso,:v_usuario,:v_json,:V_TIPODOC,:V_IDDOC,:v_resp);end;');
	...
	oci_bind_by_name($consulta, ':V_TIPODOC', $request->tipo_documento);
	oci_bind_by_name($consulta, ':V_IDDOC', $request->numero_documento);
