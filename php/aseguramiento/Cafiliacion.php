<?php
  ...
  //  $grupopoblacional = (isset($request->grupopoblacional) ? $request->grupopoblacional : '');
   $metodogpoblacional = (isset($request->metodogpoblacional) ? $request->metodogpoblacional : '');
   $gruposisben = (isset($request->gruposisben) ? $request->gruposisben : '');
   $subgruposisben = (isset($request->subgruposisben) ? $request->subgruposisben : '');
   $causalafi_oficio = (isset($request->causalafi_oficio) ? $request->causalafi_oficio : '');

   $tipo_documento_CN = (isset($request->tipo_documento_CN) ? $request->tipo_documento_CN : '');
   $documento_CN = (isset($request->documento_CN) ? $request->documento_CN : '');
   ...
   $consulta = oci_parse($c,'BEGIN PQ_GENESIS_AL.P_EPRI_AFILIACION(:v_pempresa,:v_estado_afiliado,:v_pdocumento,:v_pnumero,:v_pubicacion,:v_ptipo_documento,:v_pnumerodocumento,:v_ptipo_cbf,:v_pparentesco,:v_pcabezatipodocumento,:v_pcabezanumerodocumento,:v_pprimer_nombre,:v_ppsegundo_nombre,:v_pprimer_apellido,
                                                        ...
                                                        :v_renglon,:v_f_afiliacion,:v_responsable,:v_metodologia_grupo_pob,:v_grupo_sisben_iv,
                                                        :v_subgrupo_sisben_iv,:v_causal_oficio,:v_ptipodocumento_CN,:v_pnumerodocumento_CN,:v_salida,:v_mensaje); end;');
   oci_bind_by_name($consulta,':v_pempresa',$empresa);
   ...
   oci_bind_by_name($consulta,':v_f_afiliacion',$fecha_afiliacion);
   oci_bind_by_name($consulta,':v_responsable',$request->responsable);
  //  oci_bind_by_name($consulta,':v_pgrupo_poblacional',$grupopoblacional);
   oci_bind_by_name($consulta,':v_metodologia_grupo_pob',$metodogpoblacional);
   oci_bind_by_name($consulta,':v_grupo_sisben_iv',$gruposisben);
   oci_bind_by_name($consulta,':v_subgrupo_sisben_iv',$subgruposisben);
   oci_bind_by_name($consulta,':v_causal_oficio',$causalafi_oficio);
   oci_bind_by_name($consulta,':v_ptipodocumento_CN',$tipo_documento_CN);
   oci_bind_by_name($consulta,':v_pnumerodocumento_CN',$documento_CN);
