...
function radicado()
{
  ...
  $cantRegistros = $request->cantRegistros;

  require_once('../../config/dbcon_prod.php');
  $consulta = oci_parse($c, 'BEGIN pq_genesis_medicamentos.P_INSERTAR_GESTION_MEDICAMENTOS (:V_PTERCERO,:V_PANNO,:V_PPERIODO, :v_pnomarch, :v_pregistro, :v_PRADICADO, :V_JSON_ROW); end;');
  oci_bind_by_name($consulta, ':v_ptercero', $nit);
  oci_bind_by_name($consulta, ':v_panno', $ano);
  oci_bind_by_name($consulta, ':v_pperiodo', $periodo);
  oci_bind_by_name($consulta, ':v_pnomarch', $archivo);
  oci_bind_by_name($consulta, ':v_pregistro', $cantRegistros);
...
function actualizarCargue()
{
  ...
  require_once('../../config/dbcon_prod.php');
  $consulta = oci_parse($c, 'BEGIN oasis.pq_genesis_medicamentos_2.p_re_cargar_archivo(:v_ptercero,:v_panno,:v_pperiodo,:v_pcodigo_proceso,:v_json_row); end;');
