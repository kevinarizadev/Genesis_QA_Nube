    ...
    if (mb_check_encoding($contenido, 'UTF-8') && strpos($contenido, "\xEF\xBB\xBF") !== 0) { // Validamos que el formato de codificacion sea utf-8
        if ($_FILES['archivo']['size'] > 0) { //validamos que el archivo no este vacio
            ...
            if (is_dir('ftp://genesis_ftp:Cajacopi2022!@192.168.50.36/' . $rutaFtp) == TRUE) {
                ...
                if ($subio) {
                    unlink($rutaTemp);
                    $resp = validarComas($nombreArchivo, $rutaFtp, $nit, $año, $mes, $dia);
                ...
                if (ftp_mkdir($con_id, $rutaFtp)) {
                    ...
                    if ($subio) {
                        ...
                        echo $tipo;
                        if ($tipo == 1) {
                            validarComas($nombreArchivo, $rutaFtp . '/' . $nombreArchivo, $nit, $año, $mes, $dia);
                            echo "insertarRuta";
                        } else {
                            validarComas($nombreArchivo, $rutaFtp . '/' . $nombreArchivo, $nit, $año, $mes, $dia);
                            ///actualizarCargue(intval($nit), intval($año), intval($mes), intval($dia), intval($_POST['codProceso']));
...
function validarComas($nombrArchivo, $rutaFTP, $nit, $anno, $periodo, $dia)
{

    require_once('../../config/dbcon_prod.php');
    $consulta = oci_parse($c, 'BEGIN oasis.pq_genesis_medicamentos_2.p_valida_archivo_medicamentos(:v_pruta,:v_pnomarch,:v_json_error); end;');
