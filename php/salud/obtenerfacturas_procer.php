<?php
	...
	$consulta = oci_parse($c,"SELECT *  FROM r_traza_rips_20162_facpnv D
              WHERE --e.CUPS_HOMOLOGADO IS NULL AND 
                    EXISTS (SELECT 1 FROM  r_traza_rips_20162_facdpnv e
                         where d.nit = e.terv_codigo 
                         and trim(d.num_factura) = trim(e.num_factura) 
                         and d.fcdn_recibo = e.fcdn_recibo 
                         and trim(d.afic_documento) = trim(e.afic_documento)
                         and e.nuevo_producto is null and e.cambio_sexo is null
                         )

							and ROWnUM <= 10 order by dbms_random.value");	
