<?php
  ...
  $request = json_decode($postdata);
	...
	function insertarurgencia(){
		...
		global $request;
		$informacioncheck          = $request->informacioncheck;
		...
		$consulta = oci_parse($c,'BEGIN PQ_GENESIS_CD.P_INSERTA_COD_URGENCIA(:v_pcoddiag1,:v_pcoddiag2,:v_pubicacion,:v_pdocsolicitante,:v_pnitips,:v_ptipodocpaciente,:v_pdocpaciente,:v_pobservacion,:v_pfechaingreso,:v_prol,:v_phijo,:v_pruta,:v_paccion,:v_pnegacion,:v_pmotivo_rechazo,:v_pcontrol,:v_pinformacion); end;');
		...
		oci_bind_by_name($consulta,':v_pcontrol',$informacioncheck);
