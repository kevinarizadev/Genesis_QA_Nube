<?php
  ...
  function obtenerPqrds(){
      ...
      $tipodocumento = $request->tipodocumento;
      $documento     = $request->documento;
      $consulta = oci_parse($c,'BEGIN PQ_GENESIS_AUTPRO.P_OBTENER_PQR(:v_ptipodocumento,:v_pdocumento,:v_json_row); end;');
      oci_bind_by_name($consulta,':v_ptipodocumento',$tipodocumento);
      oci_bind_by_name($consulta,':v_pdocumento',$documento);
      $clob = oci_new_descriptor($c,OCI_D_LOB);
      oci_bind_by_name($consulta, ':v_json_row', $clob,-1,OCI_B_CLOB);
  ...
  function obtenerAutorizaciones(){
      ...
      $numero     = $request->numero;
      $ubicacion     = $request->ubicacion;
      $consulta = oci_parse($c,'BEGIN PQ_GENESIS_AUTPRO.P_OBTENER_AUTORIZACIONES_DEMO(:v_ptipo_doc,:v_pdocumento,:v_pnumero,:v_pubicacion,:v_json_row); end;');
  ...
  function p_mostrar_cups_vs_dx(){
   ...
   function p_consulta_autorizaciones(){
      require_once('../../config/dbcon.php');
      global $request;
      $consulta = oci_parse($c,'BEGIN PQ_GENESIS_AUTPRO.P_CONSULTA_AUTORIZACIONES(:v_ptipo_doc,:v_pdocumento,:v_pnumero,:v_pips,:v_pubicacion,:v_json_row); end;');
  ...
  function p_consulta_autorizaciones_2(){
    require_once('../../config/dbcon.php');
    global $request;
    $consulta = oci_parse($c,'BEGIN PQ_GENESIS_AUTPRO.P_CONSULTA_AUTORIZACIONES_2(:v_ptipo_doc,:v_pdocumento,:v_pnumero,:v_pips,:v_pubicacion,:v_json_row); end;');
    oci_bind_by_name($consulta,':v_ptipo_doc',$request->tipodocumento);
    oci_bind_by_name($consulta,':v_pdocumento',$request->documento);
    oci_bind_by_name($consulta,':v_pnumero',$request->numero);
    oci_bind_by_name($consulta,':v_pips',$request->ips);
...
function Generar_Agenda()
{
	...
	curl_setopt_array($curl, array(
		...
		CURLOPT_HTTPHEADER => array(
			// 'API-KEY: 2VwtfEVxx4dxmql8D?jOvazsUx1pHeYwlS8NlMotBV3',
			'API-KEY: 1Go8l9k9nJ8IkO01?hUp4c6nAmQiTxPhItQvT9NqP94Lk',
...
function obtener_listado_aut_pqrds(){
  require_once('../../config/dbcon.php');
  global $request;
  $consulta = oci_parse($c,'BEGIN oasis.PQ_GENESIS_AUTPRO.P_LISTA_AUTORIZACIONES_ACTIVAS_pqr(:v_pestado,:v_pseccional,:v_pjson_row); end;');
  oci_bind_by_name($consulta,':v_pestado',$request->tipoaut);
...
function obtenerdatosordenamiento(){
  require_once('../../config/dbcon.php');
  global $request;
  $consulta = oci_parse($c,'BEGIN oasis.pq_webservices_ips.P_consulta_ordenamiento(:p_tipodocafiliado,
                                                                              :p_num_documento,
                                                                              :p_orden_medico,
                                                                              :p_nit,
                                                                              :v_pjson_row); end;');
  oci_bind_by_name($consulta,':p_tipodocafiliado',$request->documento);
  oci_bind_by_name($consulta,':p_num_documento',$request->numero);
  oci_bind_by_name($consulta,':p_orden_medico',$request->ordenmedica);
