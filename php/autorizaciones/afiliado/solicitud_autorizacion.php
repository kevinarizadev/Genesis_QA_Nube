<?php 
  ...
  function p_ui_esaa(){
    ...
    $consulta = oci_parse($c,'BEGIN PQ_GENESIS_ESOA.P_UI_ESAA(:v_psolicitudesoa, 
                                                              :v_json_row); end;');
  ...
  function p_llamar_sol_aut_afiliados_autorizaciones (){    
      require_once('../../config/dbcon_prod.php');
      global $request;    
      $consulta = oci_parse($c,'BEGIN PQ_GENESIS_ESOA.p_llamar_sol_aut_afiliados_doc(:v_ptipo_doc,:v_pdocumento,:vp_estado,:v_json_row); end;');
      oci_bind_by_name($consulta,':v_ptipo_doc',$request->documento);
  ...
  function llamar_Listado_Pendientes (){    
    require_once('../../config/dbcon_prod.php');
    global $request;    
    $consulta = oci_parse($c,'BEGIN PQ_GENESIS_ESOA.p_listar_detalle_municipio_sol_aut(:v_municipio,:v_json_res); end;');
    oci_bind_by_name($consulta,':v_municipio',$request->municipio);
    $clob = oci_new_descriptor($c,OCI_D_LOB);
    oci_bind_by_name($consulta, ':v_json_res', $clob,-1,OCI_B_CLOB);
    oci_execute($consulta,OCI_DEFAULT);
  ...
  function guardargestionoficinavirtual (){    
    require_once('../../config/dbcon_prod.php');
    global $request;    
    $consulta = oci_parse($c,'BEGIN PQ_GENESIS_ESOA.p_marcar_solicitud_activas(:v_saan_numero_solicitud,
                                                                                :v_autn_autorizacion_manual,
                                                                                :v_presposable_oficial,
                                                                                :v_json_row); end;');
    oci_bind_by_name($consulta,':v_saan_numero_solicitud',$request->numeroradicado);
    oci_bind_by_name($consulta,':v_autn_autorizacion_manual',$request->numeroaut);
    oci_bind_by_name($consulta,':v_presposable_oficial',$request->responsable);
    $clob = oci_new_descriptor($c,OCI_D_LOB);
    oci_bind_by_name($consulta, ':v_json_row', $clob,-1,OCI_B_CLOB);
    oci_execute($consulta,OCI_DEFAULT);
    if (isset($clob)) {
  ...
  function validarmunicipio (){    
    require_once('../../config/dbcon_prod.php');
    global $request;    
    $consulta = oci_parse($c,'BEGIN PQ_GENESIS_AUTPRO.p_valida_ubicacion_capamnna_ca(:v_pmunicipio,
                                                                                :v_json_row); end;');
    oci_bind_by_name($consulta,':v_pmunicipio',$request->municipio);
    $clob = oci_new_descriptor($c,OCI_D_LOB);
    oci_bind_by_name($consulta, ':v_json_row', $clob,-1,OCI_B_CLOB);
    oci_execute($consulta,OCI_DEFAULT);
    if (isset($clob)) {
