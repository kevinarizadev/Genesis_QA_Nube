  ...
  $request = json_decode($postdata);
	...
	function listarResumen(){
		...
		function actualiza_evento(){
			...
			$consulta = oci_parse($c, 'BEGIN PQ_GENESIS_PC.P_ACTUALIZA_EVENTO(:v_numero,
			:v_ubicacion,
			:v_tema,
			:v_fecha,
			:v_lugar,
			:v_area_solicita,
			--:v_objetivo_estrategico,
			--:v_objetivo_actividad,
			:v_duracion,
			:v_res); end;');
			oci_bind_by_name($consulta,':v_numero',$request->numero);
			oci_bind_by_name($consulta,':v_ubicacion',$request->ubicacion);
			oci_bind_by_name($consulta,':v_tema',$request->tema);
			oci_bind_by_name($consulta,':v_fecha',$request->fecha);
			oci_bind_by_name($consulta,':v_lugar',$request->lugar);
			oci_bind_by_name($consulta,':v_area_solicita',$request->area_solicita);
			//oci_bind_by_name($consulta,':v_objetivo_estrategico',$request->objetivo_estrategico);
			//oci_bind_by_name($consulta,':v_objetivo_actividad',$request->objetivo_actividad);
			oci_bind_by_name($consulta,':v_duracion',$request->duracion);
