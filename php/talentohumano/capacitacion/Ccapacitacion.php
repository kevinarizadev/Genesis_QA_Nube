...
function crearEvento(){
	...
	$facilitadores = $request->data1;
	$objetivos = $request->data2;
	$cantidadfacilitador = $request->cantidadfacilitador;
	$cantidadobjetivo = $request->cantidadobjetivo;

	$consulta = oci_parse($c, 'BEGIN PQ_GENESIS_PC.P_INSERTAR_EVENTO(
																	:v_pubicacion,            
																	:v_pprogramacion,
																	:v_ptema,                 
																	:v_ptipoactividad,        
																	...
																	:v_pareasolicita,         
																	:v_phorasprogramadas,
																	:v_json_facilitadores,    
																	:v_cantidad_facilitadores,
																	:v_json_objetivos,        
																	:v_cantidad_objetivos,
																	:v_ptipolugar,            
																	:v_plugar,                
																	:v_psede,                 
																	:v_pvalor,               
	...
	//Json Fcilitadores
	$jsonafacilitadores= oci_new_descriptor($c, OCI_D_LOB);
	oci_bind_by_name($consulta, ':v_json_facilitadores', $jsonafacilitadores, -1, OCI_B_CLOB);
	$jsonafacilitadores->writeTemporary($facilitadores);

	//Cantidad De Facilitadores
	oci_bind_by_name($consulta,':v_cantidad_facilitadores', $cantidadfacilitador);
	
	//Json Objetivos
	$jsonobjetivos= oci_new_descriptor($c, OCI_D_LOB);
	oci_bind_by_name($consulta, ':v_json_objetivos', $jsonobjetivos, -1, OCI_B_CLOB);
	$jsonobjetivos->writeTemporary($objetivos);
	
	//Cantidad Objetivos
	oci_bind_by_name($consulta,':v_cantidad_objetivos',	$cantidadobjetivo);
	...
	oci_bind_by_name($consulta,':v_pvalor', $dataEvento->valor);
