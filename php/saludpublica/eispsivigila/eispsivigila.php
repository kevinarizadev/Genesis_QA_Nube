...
function obtenerRol()
{
	...
	global $request;
	$consulta = oci_parse($c, 'BEGIN oasis.PQ_SIVIGILA.p_obtener_rol(:v_pcedula,:v_json_row);end;');
...
function cargarSoporte()
{
  ...
  global $request;
  // $hoy = date("dmY");
  $path_of_storage = '/cargue_ftp/Digitalizacion/Genesis/SaludPublica/Sivigila/' . $request->nombreCarpeta;
  $hoy = date('dmY_His');
...
function p_ui_cargue()
{
  ...
  $consulta = oci_parse($c, 'BEGIN oasis.PQ_SIVIGILA.p_insertar_registros_sivigila(:v_pjson_in,:v_pjson_out); end;');
...
//   global $request;
//   require_once('../../config/dbcon_prod.php');
//   $consulta = oci_parse($c, 'BEGIN oasis.PQ_EISP_SIVIGILA.p_insertar_registros_sivigila(:v_pjson_in,:v_idconsulta,:v_pjson_out); end;');
//   oci_bind_by_name($consulta, ':v_pjson_in', $request->datos);
//   oci_bind_by_name($consulta, ':v_idconsulta', $request->vidconsulta);
//   $clob = oci_new_descriptor($c, OCI_D_LOB);
//   oci_bind_by_name($consulta, ':v_pjson_out', $clob, -1, OCI_B_CLOB);
//   oci_execute($consulta, OCI_DEFAULT);
//   if (isset($clob)) {
...
function p_obtener_funcionario()
{
  global $request;
  require_once('../../config/dbcon_prod.php');
  $consulta = oci_parse($c, 'BEGIN oasis.PQ_SIVIGILA.p_obtener_funcionario(:v_pnit,:v_json_row); end;');
...
function p_ver_permisos_funcs()
{
  global $request;
  require_once('../../config/dbcon_prod.php');
  $consulta = oci_parse($c, 'BEGIN oasis.PQ_SIVIGILA.p_ver_permisos_funcs(:v_pcedula,:v_json_row); end;');
...
function p_listar_funcs()
{
  // global $request;
  require_once('../../config/dbcon_prod.php');
  $consulta = oci_parse($c, 'BEGIN oasis.PQ_SIVIGILA.p_listar_funcs(:v_json_row); end;');
  $clob = oci_new_descriptor($c, OCI_D_LOB);
  oci_bind_by_name($consulta, ':v_json_row', $clob, -1, OCI_B_CLOB);
  oci_execute($consulta, OCI_DEFAULT);
  if (isset($clob)) {
...
function p_ui_funcs()
{
  global $request;
  require_once('../../config/dbcon_prod.php');
  $consulta = oci_parse($c, 'BEGIN oasis.PQ_SIVIGILA.p_ui_funcs(:v_pcedula,:v_paccion,:v_pestado,:v_pjson_out); end;');
  oci_bind_by_name($consulta, ':v_pcedula', $request->cedula);
...
function p_consulta_cargue()
{
  // global $request;
  require_once('../../config/dbcon_prod.php');
  $consulta = oci_parse($c, 'BEGIN oasis.PQ_SIVIGILA.p_consulta_cargue(:v_pjson_out); end;');
  $clob = oci_new_descriptor($c, OCI_D_LOB);
  oci_bind_by_name($consulta, ':v_pjson_out', $clob, -1, OCI_B_CLOB);
  oci_execute($consulta, OCI_DEFAULT);
  if (isset($clob)) {
