...
function subir_soporte_denustricion()
{
	...
	if ($arrlength == 0) {
		...
		for ($i = 0; $i < count($data); $i++) {
			// echo $name;
			$name = $data[$i]->tiposop . '_' . $data[$i]->name . '_' . $id . '.' . $data[$i]->ext;
			list(, $data[$i]->src) = explode(';', $data[$i]->src); // Proceso para traer el Base64
			list(, $data[$i]->src) = explode(',', $data[$i]->src); // Proceso para traer el Base64
			$base64 = base64_decode($data[$i]->src);
