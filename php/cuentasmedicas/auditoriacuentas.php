...
function Consulta_Censos(){
	...
	$consulta = oci_parse($c,'begin pq_genesis_censo_hospitalario.p_obtener_censo_documento(:v_tipo_documento,:v_documento,:v_pnit,:v_precibo,:v_pfactura,:v_response); end;');
...
function descargarMasivoFacturasPDF()
{
  ...
  $sftp = ssh2_sftp($con_id); // Creamos la conexion sftp
    ...
    ssh2_scp_recv($con_id, $Array[$i]->ruta, $local_file);// Traemos el archivo del servidor y lo enviamos a la carpeta temp de Genesis
	}
