<?php
  ...
  $request = json_decode($postdata);
	...
	function obteneracasenvivo(){
			...
			$consulta = oci_parse($c,'BEGIN PQ_GENESIS_TIC.P_OBTENER_ACAS_ENVIVO(:v_pestado,:v_json_row); end;');
	...
	function obteneracasconcepto(){
	    require_once('../../config/dbcon_prod.php');
	    global $request;
	    $consulta = oci_parse($c,'BEGIN PQ_GENESIS_TIC.P_OBTENER_ACAS_PORCONCEPTO(:v_json_row); end;');
	    $clob = oci_new_descriptor($c,OCI_D_LOB);
	    oci_bind_by_name($consulta, ':v_json_row', $clob,-1,OCI_B_CLOB);
	    oci_execute($consulta,OCI_DEFAULT);
	    if (isset($clob)) {
	...
	function obteneracaspropios(){
	    require_once('../../config/dbcon_prod.php');
	    global $request;
	    $consulta = oci_parse($c,'BEGIN PQ_GENESIS_TIC.P_OBTENER_ACAS_PROPIOS(:v_json_row); end;');
	    $clob = oci_new_descriptor($c,OCI_D_LOB);
	    oci_bind_by_name($consulta, ':v_json_row', $clob,-1,OCI_B_CLOB);
	    oci_execute($consulta,OCI_DEFAULT);
	    if (isset($clob)) {
		...
		function obteneracaspropioshoy(){
			require_once('../../config/dbcon_prod.php');
			global $request;
			$consulta = oci_parse($c,'BEGIN PQ_GENESIS_TIC.P_OBTENER_ACAS_SOPORTE_HOY(:v_json_row); end;');
			$clob = oci_new_descriptor($c,OCI_D_LOB);
			oci_bind_by_name($consulta, ':v_json_row', $clob,-1,OCI_B_CLOB);
			oci_execute($consulta,OCI_DEFAULT);
			if (isset($clob)) {
	  ...
	  function obteneracashistorico(){
	    require_once('../../config/dbcon_prod.php');
	    global $request;
	    $consulta = oci_parse($c,'BEGIN PQ_GENESIS_TIC.P_OBTENER_ACAS_HISTORICO(:v_json_row); end;');
	    $clob = oci_new_descriptor($c,OCI_D_LOB);
	    oci_bind_by_name($consulta, ':v_json_row', $clob,-1,OCI_B_CLOB);
	    oci_execute($consulta,OCI_DEFAULT);
	    if (isset($clob)) {
	  ...
	  function obteneracasengenerales(){
	    require_once('../../config/dbcon_prod.php');
	    global $request;
	    $consulta = oci_parse($c,'BEGIN PQ_GENESIS_TIC.P_OBTENER_ACAS_GENERALES(:v_json_row); end;');
	    $clob = oci_new_descriptor($c,OCI_D_LOB);
	    oci_bind_by_name($consulta, ':v_json_row', $clob,-1,OCI_B_CLOB);
	    oci_execute($consulta,OCI_DEFAULT);
	    if (isset($clob)) {
	...
	function obteneracastic(){
		...
		global $request;
		$tipo = $request->tipo;
		//$consulta = oci_parse($c,'BEGIN PQ_GENESIS_TIC.P_OBTENER_ACAS_TIC(:v_json_row); end;');
		$consulta = oci_parse($c,'BEGIN PQ_GENESIS_TIC.P_OBTENER_ACAS_TIC(:v_ptipo,:v_json_row); end;');
...
/* select bt.terc_nombre as nombre,
...
");
	...
	function obtenerDptoIPS(){
			require_once('../../config/dbcon_prod.php');
			global $request;
			$consulta = oci_parse($c,'BEGIN PQ_GENESIS_TIC.P_OBTENER_DPTO_IPS(:v_json_row); end;');
			$clob = oci_new_descriptor($c,OCI_D_LOB);
			oci_bind_by_name($consulta, ':v_json_row', $clob,-1,OCI_B_CLOB);
			oci_execute($consulta,OCI_DEFAULT);
			if (isset($clob)) {
