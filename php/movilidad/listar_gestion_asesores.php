<?php
	if($_GET["funcion"]==1){
		...
		$consulta = oci_parse($c,'begin pq_genesis_mov.cantidad_solicitudes_asesores(:v_pestado,:v_pregional,:v_prespuesta); end;');
		oci_bind_by_name($consulta,':v_pestado',$_GET["estado"]);
		oci_bind_by_name($consulta,':v_pregional',$_GET["regional"]);
		...
		require_once('../config/dbcon_empresa.php');
		$consulta = oci_parse($c,'begin pq_genesis_mov.cantidad_afiliaciones_asesores(:v_pestado,:v_pregional,:v_prespuesta); end;');
		oci_bind_by_name($consulta,':v_pestado',$_GET["estado"]);
		oci_bind_by_name($consulta,':v_pregional',$_GET["regional"]);
		$clob = oci_new_descriptor($c,OCI_D_LOB);
		oci_bind_by_name($consulta, ':v_prespuesta', $clob,-1,OCI_B_CLOB);
		...
		require_once('../config/dbcon_empresa.php');
		$consulta = oci_parse($c,'begin pq_genesis_mov.cantidad_solicitudes_empresa_asesores_detalle_descarga(:v_pestado,:v_pregional,:v_json_row); end;');
		// $estado = 'A';
		...
		require_once('../config/dbcon_empresa.php');
		// $estado = 'A';
		$consulta = oci_parse($c,'begin pq_genesis_mov.cantidad_solicitudes_afiliaciones_asesores_detalle_descarga(:v_pestado,:v_pregional,:v_json_row); end;');
		// oci_bind_by_name($consulta,':v_estado',$estado);
		oci_bind_by_name($consulta,':v_pestado',$_GET["estado"]);
		oci_bind_by_name($consulta,':v_pregional',$_GET["regional"]);
		$cursor = oci_new_cursor($c);
