...
function List_TipoGlosas()
{
	require_once('../config/dbcon_prod.php');
	global $request;
	$consulta = oci_parse($c, 'BEGIN PQ_GENESIS_FACTURACION.P_LISTA_TIPO_DESCUENTO_PGP(:v_json_row); end;');
	$clob = oci_new_descriptor($c, OCI_D_LOB);
	oci_bind_by_name($consulta, ':v_json_row', $clob, -1, OCI_B_CLOB);
	oci_execute($consulta, OCI_DEFAULT);
	if (isset($clob)) {
...
function Vista2_List_Facturas()
{
	require_once('../config/dbcon_prod.php');
	global $request;
	$consulta = oci_parse($c, 'BEGIN PQ_GENESIS_FACTURACION.P_LISTA_FACTURA_PGP(:v_nit,:v_recibo,:v_tipo,:v_json_row); end;');
...
function Factura_Gestion()
{
	...
	global $request;
	$datos = $request->datos;
	$consulta = oci_parse($c, 'BEGIN OASIS.PQ_GENESIS_FACTURACION.P_U_FACTURAS_PGP(:v_pdocumento,:v_pnumero,:v_pubicacion,:v_pfinicial,:v_pffinal,:v_ptipo_doc,:v_pdoc_afi,:v_pambito,:v_pdiagnostico,
	:v_pregimen,:v_ptipofact,:v_paltocosto,:v_pcopago,:v_pcuota_mod,:v_pjson_glosa,:v_pcant_glosa,
	:v_pobservacion_glosa,:v_pconcepto,:v_pfechapss,:v_pno_copago,:v_pproducto,:v_json_row); end;');
...
function P_UI_GLOSA_PGP()
{
	...
	global $request;
	$datos = $request->datos;
	$consulta = oci_parse($c, 'BEGIN OASIS.PQ_GENESIS_FACTURACION.P_UI_GLOSA_PGP(:v_pubicacion,
																					:v_pnumero,
																					:v_phora,
																					:v_pjson_glosa,
																					:v_cantidad_glosa,
	...
	// oci_bind_by_name($consulta, ':v_pfactura2', $request->factura2);
	oci_bind_by_name($consulta, ':v_phora', $request->hora);
	// oci_bind_by_name($consulta, ':v_ptipo_doc_afiliado', $request->tipo_doc);
	// oci_bind_by_name($consulta, ':v_pafiliado', $request->afiliado);
	// oci_bind_by_name($consulta, ':v_pcontrato_capitado', $request->contrato_capitado);
	// oci_bind_by_name($consulta, ':v_pcontrato', $request->contrato);
	// oci_bind_by_name($consulta, ':v_pubicacion_contrato', $request->ubicacion_contrato);
	// oci_bind_by_name($consulta, ':v_pproyecto', $request->proyecto);
	oci_bind_by_name($consulta, ':v_pjson_glosa', $json_parametros, -1, OCI_B_CLOB);
	oci_bind_by_name($consulta, ':v_cantidad_glosa', $request->cantGlosa);
	// oci_bind_by_name($consulta, ':v_ptercero', $request->tercero);
	oci_bind_by_name($consulta, ':v_pobservacion', $request->observacion);
	// oci_bind_by_name($consulta, ':v_pobservacion_glosa', $request->Observacion_Glosa);
	// oci_bind_by_name($consulta, ':v_estado_fac', $request->estado_fac);
	$json_parametros->writeTemporary($datos);
	$clob = oci_new_descriptor($c, OCI_D_LOB);
	oci_bind_by_name($consulta, ':v_json_row', $clob, -1, OCI_B_CLOB);
	oci_execute($consulta, OCI_DEFAULT);
...
function p_confirma_fac_com_gen()
{
	require_once('../config/dbcon_prod.php');
	global $request;
	$consulta = oci_parse($c, 'BEGIN oasis.p_confirma_fac_com_gen(:v_pempresa,:v_pdocumento,:v_numero,:v_pubicacion,:v_poperacion,:v_json_row); end;');
	oci_bind_by_name($consulta, ':v_pempresa', $request->empresa);
	oci_bind_by_name($consulta, ':v_pdocumento', $request->doc);
	oci_bind_by_name($consulta, ':v_numero', $request->Num);
	oci_bind_by_name($consulta, ':v_pubicacion', $request->Ubi);
	oci_bind_by_name($consulta, ':v_poperacion', $request->accion);
	$clob = oci_new_descriptor($c, OCI_D_LOB);
	oci_bind_by_name($consulta, ':v_json_row', $clob, -1, OCI_B_CLOB);
	oci_execute($consulta, OCI_DEFAULT);
	if (isset($clob)) {
...
function Vista2_List_Glosa_Factura()
{
	require_once('../config/dbcon_prod.php');
	global $request;
	$consulta = oci_parse($c, 'BEGIN PQ_GENESIS_FACTURACION.P_LISTA_DESCUENTO_PGP_FACTURA(:v_numero,:v_ubicacion,:v_pfecha ,:v_json_row); end;');
	oci_bind_by_name($consulta, ':v_numero', $request->Num);
	oci_bind_by_name($consulta, ':v_ubicacion', $request->Ubi);
