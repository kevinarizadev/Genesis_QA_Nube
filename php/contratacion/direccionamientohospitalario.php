<?php
    ...
    function P_MODIFICACION_DIRECCIONAMIENTO(){
        ...
        global $request;
        $consulta =  oci_parse($c,'BEGIN PQ_DIRECCIONAMIENTO_SERVICIOS.P_MODIFICACION_DIRECCIONAMIENTO_HOSP( :V_PJSON_IN,
                                                                                                        :V_PCANTIDAD_ESC,
                                                                                                        :v_pjson_out ); end;');
    ...
    function P_MODIFICACION_ESTRATEGIA(){
        require_once('../config/dbcon_prod.php');
        global $request;
        $consulta =  oci_parse($c,'BEGIN PQ_DIRECCIONAMIENTO_SERVICIOS.P_MODIFICACION_ESTRATEGIA_HOSP( :V_PJSON_IN,
                                                                                                        :V_PCANTIDAD_ESC,
                                                                                                        :v_pjson_out ); end;');
        $clob = oci_new_descriptor($c,OCI_D_LOB);
        oci_bind_by_name($consulta,':V_PJSON_IN',$request->V_PJSON_IN);
    ...
    function P_LISTA_IPS_ESTRATEGIA(){
		require_once('../config/dbcon_prod.php');
		global $request;
		$consulta =  oci_parse($c,'BEGIN PQ_DIRECCIONAMIENTO_SERVICIOS.P_LISTA_IPS_ESTRATEGIA_HOSP(  :v_pcod_mpio,
																								:v_pregimen,
                                                                                                :v_pjson_out ); end;');
		$clob = oci_new_descriptor($c,OCI_D_LOB);
    ...
    function P_REGISTRO_UBIC_ESCENARIOS(){
		require_once('../config/dbcon_prod.php');
		global $request;
        $consulta =  oci_parse($c,'BEGIN PQ_DIRECCIONAMIENTO_SERVICIOS.P_REGISTRO_UBIC_ESCENARIOS_HOSP(  :v_pjson_in,
                                                                                                    :v_pcantidad_esc,
                                                                                                    :v_pjson_out ); end;');
		$clob = oci_new_descriptor($c,OCI_D_LOB);
		oci_bind_by_name($consulta,':v_pjson_in',$request->v_pjson_in);
...
function P_LISTA_DIR_BASICO()
{
    require_once('../config/dbcon_prod.php');
    global $request;
    $consulta =  oci_parse($c, 'BEGIN PQ_DIRECCIONAMIENTO_SERVICIOS.P_LISTA_DIR_BASICO_HOSP( :v_pregimen,
                                                                                        :v_pjson_row ); end;');
    $clob = oci_new_descriptor($c, OCI_D_LOB);
    oci_bind_by_name($consulta, ':v_pregimen', $request->v_pregimen);
    oci_bind_by_name($consulta, ':v_pjson_row', $clob, -1, OCI_B_CLOB);
...
function P_LISTA_DETALLE_DIR_BASICO (){
    require_once('../config/dbcon_prod.php');
    global $request;
    $consulta =  oci_parse($c, 'BEGIN PQ_DIRECCIONAMIENTO_SERVICIOS.P_LISTA_DETALLE_DIR_BASICO_HOSP( :v_pregimen,
                                                                                                :v_pmpio_origen,
                                                                                                :v_pjson_row ); end;');
    $clob = oci_new_descriptor($c, OCI_D_LOB);
    oci_bind_by_name($consulta, ':v_pregimen', $request->v_pregimen);
...
function P_LISTA_ESTRATEGIA_DIR_BASICO ()
{
    require_once('../config/dbcon_prod.php');
    global $request;
    $consulta =  oci_parse($c, 'BEGIN PQ_DIRECCIONAMIENTO_SERVICIOS.p_lista_estrategia_dir_basico_HOSP( :v_pregimen,
                                                                                                :v_pmpio_origen,
                                                                                                :v_pmpio_destino,
                                                                                                :v_pjson_row ); end;');
    $clob = oci_new_descriptor($c, OCI_D_LOB);
