...
header("Content-Disposition: attachment; filename=Matriz de incumplimiento de actividades"."_".date("dmY").".xls");
...
if ($regimen == 'S'){
$consulta = oci_parse ($c,"SELECT PER_CAPITACION,
       NIT_PRESTADOR,
       NOM_PRESTADOR,
       D.NOM_DEPARTAMENTO,
       M.NOM_CIUDAD,
       ...
       COD_CENTROCOSTOS,
       '' DATOMOD
  FROM oasis.CAPITACIONMES
  LEFT JOIN oasis.DEPARTAMENTO D
    ON SUBSTR(NUM_CAUSACION, 1, 2) = D.COD_DEPARTAMENTO
  LEFT JOIN oasis.CIUDAD M
    ON NUM_CAUSACION = M.COD_DEPARTAMENTO || M.COD_CIUDAD
  
  INNER JOIN oasis.ocnt_contrato CO 
    ON CO.CNTN_NUMERO = NUM_CONTRATO
   AND CO.CNTC_DOCUMENTO = 'KS'
   AND CO.CNTC_CONCEPTO in ('CA')
  AND CO.CNTN_MOTIVO = '2'
   AND CO.CNTC_ESTADO = 'P' 

WHERE PER_CAPITACION = :v_periodo
 GROUP BY PER_CAPITACION,
          NIT_PRESTADOR,
          NOM_PRESTADOR,
          D.NOM_DEPARTAMENTO,
          M.NOM_CIUDAD,
